...
(function ($) {
  $.easyui = {
    ...
    forEach: function forEach(data, deep, callback) {
      var nodes = [];
  ...
  $.parser = {
    ...
    onComplete: function onComplete(context) {},
    ...
    parseValue: function parseValue(property, value, parent, delta) {
      delta = delta || 0;
     ...
     */
    parseOptions: function parseOptions(target, properties) {
      var t = $(target);
      var options = {};
...
(function ($) {
  ...
  function onTouchStart(e) {
    ...
    if (!isDblClick) {
      ...
      isDblClick = false;
      fire(e, 'dblclick'); //			e.preventDefault();
    }
...
(function ($) {
  ...
  $.fn.draggable.defaults = {
    proxy: null,
    // 'clone' or a function that will create the proxy object, 
...
(function ($) {
  ...
  $.fn.droppable.defaults = {
    accept: null,
    disabled: false,
    onDragEnter: function onDragEnter(e, source) {},
    onDragOver: function onDragOver(e, source) {},
    onDragLeave: function onDragLeave(e, source) {},
    onDrop: function onDrop(e, source) {}
  };
})(jQuery);
  ...
  $.fn.resizable = function (options, param) {
    ...
    return this.each(function () {
      ...
      $(this).bind('mousemove.resizable', {
        ...
        $(e.data.target).css('cursor', type ? type + '-resize' : '');
...
(function ($) {
  ...
  function createButton(target) {
    ...
    t.attr('id', opts.id || ''); //modify: 2017-09-04 haibin.long 增加扩展属性的绑定

    if (opts.extend) {
      //扩展自定义属性
      $.each(opts.extend, function (key, val) {
        t.attr(key, val);
      });
    }
    ...
    if (opts.extendCls) {
      //扩展自定义样式类
      t.addClass(opts.extendCls);
    } //modfify: 2017-09-04 haibin.long 扩展结束
    ...
    if (opts.iconCls) {
      var iconStr = opts.iconCls; // 默认是粗的字体图标

      var isExistFal = iconStr.substring(0, 3) == "fal";
      var faStr = "fas";
      ...
      var icon = $('<i class="' + faStr + ' l-btn-icon" aria-hidden="true"></i>');
      icon.addClass(opts.iconCls);

      if (opts.iconCls.substring(0, 2) == "fa") {
        icon.addClass("fa-font-icon");
      }
  ...
  $.fn.linkbutton.parseOptions = function (target) {
    var t = $(target);
    return $.extend({}, $.parser.parseOptions(target, ['id', 'iconCls', 'iconAlign', 'group', 'size', 'text', {
      plain: 'boolean',
      toggle: 'boolean',
      selected: 'boolean',
      outline: 'boolean'
...
(function ($) {
   ...
   */
  // tangbangguo
  var rightIcon = 'fal fa-chevron-right';
  var downIcon = 'fal fa-chevron-down';

  function wrapTree(target, opts) {
    var tree = $(target);
    tree.addClass('tree');
    tree.addClass('tree-build-' + opts.mode);

    if (opts.mode === 'page') {
      rightIcon = 'fa fa-angle-caret-down';
      downIcon = 'fa fa-angle-caret-up';
    }
  ...
  function bindTreeEvents(target) {
    ...
    $(target).unbind().bind('mouseover', function (e) {
      ...
      if (tt.hasClass('tree-hit')) {
        if (tt.hasClass('tree-expanded ' + downIcon)) {//tt.addClass('tree-expanded-hover');
        } else {//tt.addClass('tree-collapsed-hover');
          }
      }

      opts.onMouseover.call(target, e, _getNode(target, node[0]));
      e.stopPropagation();
      ...
      if (tt.hasClass('tree-hit')) {
        if (tt.hasClass('tree-expanded ' + downIcon)) {//tt.removeClass('tree-expanded-hover');
        } else {//tt.removeClass('tree-collapsed-hover');
          }
      }

      opts.onMouseout.call(target, e, _getNode(target, node[0]));
      e.stopPropagation();
    }).bind('click', function (e) {
      //mousedown换成click,layout树点击无法触发click,201910mikahuang
      var tt = $(e.target);
      var node = tt.closest('div.tree-node');
      ...
      if (tt.hasClass('tree-hit')) {
        ...
        var nodeData = _getNode(target, node[0]);

        if (nodeData && (!nodeData.isCombotreePageMore || nodeData.isCombotreePageMore === false)) {
          opts.onClick.call(target, nodeData, e);
        }
      }
      ...
      selectNode(target, node[0]);
      opts.onDblClick.call(target, _getNode(target, node[0]), e);
      e.stopPropagation();
  ...
  function _enableDnd(target) {
    ...
    tree.find('div.tree-node').draggable({
      ...
      proxy: function proxy(source) {
        var p = $('<div class="tree-node-proxy"></div>').appendTo('body'); //				var p = $('<div class="tree-node-proxy tree-dnd-no"></div>').appendTo('body');
      ...
      deltaY: 15,
      onBeforeDrag: function onBeforeDrag(e) {
        if (opts.onBeforeDrag.call(target, _getNode(target, this)) == false) {
          return false;
        }
        ...
        $(this).next('ul').find('div.tree-node').droppable({
          accept: 'no-accept'
        }); // the child node can't be dropped
      ...
      onStartDrag: function onStartDrag() {
        ...
        opts.onStartDrag.call(target, _getNode(target, this));

        var node = _getNode(target, this);
      ...
      onStopDrag: function onStopDrag() {
        ...
        }); // restore the accept property of child nodes

        for (var i = 0; i < state.disabledNodes.length; i++) {
          $(state.disabledNodes[i]).droppable('enable');
        }

        state.disabledNodes = []; // get the source node
      ...
      onDragEnter: function onDragEnter(e, source) {
        if (opts.onDragEnter.call(target, this, getSourceData(source)) == false) {
          allowDrop(source, false); //					$(source).draggable('proxy').removeClass('tree-dnd-yes').addClass('tree-dnd-no');
      ...
      onDragOver: function onDragOver(e, source) {
        ...
        allowDrop(source, true); //				$(source).draggable('proxy').removeClass('tree-dnd-no').addClass('tree-dnd-yes');
    ...
    function append(source, dest) {
      if (_getNode(target, dest).state == 'closed') {
  ...
  function checkNode(target, nodeEl, checked) {
    ...
    var nodedata = _getNode(target, nodeEl);
    ...
    if (opts.onBeforeCheck.call(target, nodedata, checked) == false) {
      return;
    }
    ...
    if (_isLeaf(target, nodeEl)) {
      ...
      if (opts.onlyLeafCheck) {
        ...
        if (ck.hasClass('tree-checkbox1')) {
          ...
          var children = _getChildren(target, nodeEl);
  ...
  function request(target, ul, param, callback) {
    var opts = $.data(target, 'tree').options;
    param = $.extend({}, opts.queryParams, param || {}); //		param = param || {};

    var nodedata = null;

    if (target != ul) {
      var node = $(ul).prev();
      nodedata = _getNode(target, node[0]);
    }
  ...
  function expandNode(target, nodeEl, callback) {
    ...
    if (hit.hasClass('tree-expanded ' + downIcon)) return; // has expanded

    var node = _getNode(target, nodeEl);
    ...
    if (ul.length) {
      ...
      hit.removeClass('tree-collapsed ' + rightIcon + ' tree-collapsed-hover');

      if (node.hasChildren) {
        hit.removeClass("tree-leaf").addClass("tree-expanded " + downIcon);
      } else {
        hit.addClass("tree-leaf");
      }
    } else {
      var subul = $('<ul style="display:none"></ul>').insertAfter(nodeEl); // request children nodes data

      request(target, subul[0], {
        id: node.id
      }, function () {
        if (subul.is(':empty')) {
  ...
  function collapseNode(target, nodeEl) {
    ...
    if (hit.hasClass('tree-collapsed ' + rightIcon)) return; // has collapsed

    var node = _getNode(target, nodeEl);

    if (opts.onBeforeCollapse.call(target, node) == false) return;
    hit.removeClass('tree-expanded ' + downIcon + ' tree-expanded-hover').addClass('tree-collapsed ' + rightIcon);
  ...
  function toggleNode(target, nodeEl) {
    ...
    if (hit.hasClass('tree-expanded ' + downIcon)) {
  ...
  function expandAllNode(target, nodeEl) {
    var nodes = _getChildren(target, nodeEl);

    if (nodeEl) {
      nodes.unshift(_getNode(target, nodeEl));
    }
  ...
  function appendNodes(target, param) {
    ...
    _loadData(target, ul[0], data, true);
  ...
  function updateNode(target, param) {
    ...
    var data = _getNode(target, param.target);
  ...
  function _getChildren(target, nodeEl) {
    var nodes = [];

    var n = _getNode(target, nodeEl);
  ...
  function getCheckedNode(target, state) {
    ...
    $(target).find(selectors.join(',')).each(function () {
      ...
      nodes.push(_getNode(target, node[0]));
    });
    return nodes;
  }
  /**
  ...
  function getSelectedNode(target) {
    ...
    return node.length ? _getNode(target, node[0]) : null;
  }
  /**
  ...
  function _getData(target, nodeEl) {
    var data = _getNode(target, nodeEl);
  ...
  function _getNode(target, nodeEl) {
  ...
  function attachProperties(node) {
    ...
    if (node.children && node.children.length > 0) node.hasChildren = true;
    return node;
  }
  ...
  function _isLeaf(target, nodeEl) {
    return $(nodeEl).children('span.tree-hit').length == 0;
  }

  function _beginEdit(target, nodeEl) {
    var opts = $.data(target, 'tree').options;
    ...
    editor.bind('click', function (e) {
      ...
      if (e.keyCode == 13) {
        // enter
        _endEdit(target, nodeEl);

        return false;
      } else if (e.keyCode == 27) {
        // esc
        _cancelEdit(target, nodeEl);
  ...
  function _endEdit(target, nodeEl) {
    var opts = $.data(target, 'tree').options;
    $(nodeEl).css('position', '');
  ...
  function _cancelEdit(target, nodeEl) {
    var opts = $.data(target, 'tree').options;
    $(nodeEl).css('position', '');
  ...
  $.fn.tree = function (options, param) {
    ...
    return this.each(function () {
      ...
      if (opts.data) {
        _loadData(this, this, $.extend(true, [], opts.data));
      }
  ...
  $.fn.tree.methods = {
    ...
    getNode: function getNode(jq, nodeEl) {
      // get the single node
      return _getNode(jq[0], nodeEl);
    },
    getData: function getData(jq, nodeEl) {
      // get the specified node data, include its children
      return _getData(jq[0], nodeEl);
    },
    reload: function reload(jq, nodeEl) {
      return jq.each(function () {
        if (nodeEl) {
          var node = $(nodeEl);
    ...
    getRoot: function getRoot(jq, nodeEl) {
      // if specify 'nodeEl', return its top parent node, otherwise return the first root node.
      return getRootNode(jq[0], nodeEl);
    },
    getRoots: function getRoots(jq) {
      return getRootNodes(jq[0]);
    },
    getParent: function getParent(jq, nodeEl) {
      return getParentNode(jq[0], nodeEl);
    },
    getChildren: function getChildren(jq, nodeEl) {
      return _getChildren(jq[0], nodeEl);
    },
    getChecked: function getChecked(jq, state) {
      // the state available values are: 'checked','unchecked','indeterminate', default is 'checked'.
    ...
    isLeaf: function isLeaf(jq, nodeEl) {
      return _isLeaf(jq[0], nodeEl);
    },
    find: function find(jq, id) {
      return findNode(jq[0], id);
    },
    select: function select(jq, nodeEl) {
      return jq.each(function () {
        selectNode(this, nodeEl);
      });
    },
    check: function check(jq, nodeEl) {
      return jq.each(function () {
        checkNode(this, nodeEl, true);
      });
    },
    uncheck: function uncheck(jq, nodeEl) {
      return jq.each(function () {
        checkNode(this, nodeEl, false);
      });
    },
    collapse: function collapse(jq, nodeEl) {
      return jq.each(function () {
        collapseNode(this, nodeEl);
      });
    },
    expand: function expand(jq, nodeEl) {
      return jq.each(function () {
        expandNode(this, nodeEl);
      });
    },
    collapseAll: function collapseAll(jq, nodeEl) {
      return jq.each(function () {
        collapseAllNode(this, nodeEl);
      });
    },
    expandAll: function expandAll(jq, nodeEl) {
      return jq.each(function () {
        expandAllNode(this, nodeEl);
      });
    },
    expandTo: function expandTo(jq, nodeEl) {
      return jq.each(function () {
        expandToNode(this, nodeEl);
      });
    },
    scrollTo: function scrollTo(jq, nodeEl) {
      return jq.each(function () {
        scrollToNode(this, nodeEl);
      });
    },
    toggle: function toggle(jq, nodeEl) {
      return jq.each(function () {
    ...
    remove: function remove(jq, nodeEl) {
      return jq.each(function () {
        removeNode(this, nodeEl);
      });
    },
    pop: function pop(jq, nodeEl) {
      var node = jq.tree('getData', nodeEl);
      jq.tree('remove', nodeEl);
      return node;
    },
    update: function update(jq, param) {
      return jq.each(function () {
    ...
    beginEdit: function beginEdit(jq, nodeEl) {
      return jq.each(function () {
        _beginEdit(this, nodeEl);
      });
    },
    endEdit: function endEdit(jq, nodeEl) {
      return jq.each(function () {
        _endEdit(this, nodeEl);
      });
    },
    cancelEdit: function cancelEdit(jq, nodeEl) {
      return jq.each(function () {
        _cancelEdit(this, nodeEl);
      });
    },
  ...
  var nodeIndex = 1;
  var defaultView = {
    render: function render(target, ul, data) {
      var opts = $.data(target, 'tree').options;
      ...
      function getTreeData(depth, children) {
        ...
        for (var i = 0; i < children.length; i++) {
          ...
          */
          //modify by cq.taiqi 2017年12月15日 18:12:00 树里面团队

          if (item.iconCls && item.iconCls == 'tree-team' && item.id && item.ParentFolderId && item.id != item.ParentFolderId) {
            if (item.state == 'closed') {
              item.TeamIcon = !item.TeamIcon ? item.TeamId : item.TeamIcon;
              cc.push('<span class="tree-hit tree-collapsed ' + rightIcon + '"></span>');
              cc.push('<span class="tree-icon tree-folder ' + item.iconCls + '" style="background:url(external/teamicon/' + item.TeamIcon + '.jpg) no-repeat;background-size:16px !important;"></span>');
            } else {
              if (item.children && item.children.length) {
                item.TeamIcon = !item.TeamIcon ? item.TeamId : item.TeamIcon;
                cc.push('<span class="tree-hit tree-expanded ' + downIcon + '"></span>');
                cc.push('<span class="tree-icon tree-folder ' + '" style="background:url(external/teamicon/' + item.TeamIcon + '.jpg) no-repeat;background-size:16px;"></span>');
              } else {
                if (item.treeType && item.treeType == "searchTree") {
                  cc.push('<span class="tree-indent" style="width: 10px;"></span>');
                } else {
                  cc.push('<span class="tree-indent"></span>');
                }
            ...
            if (item.state == 'closed') {
              ...
              if (item.children && item.children.length) {
                ...
                //ldb 智能标签子节点去掉展开图标
                if (item.iconCls == "tree-intag-children" || item.iconCls == "tree-metaObjTypeItem") {
                  cc.push('<span class="tree-indent"></span>');
          ...
          if (item.iconCls == "tree-metaObjTypeItem") {
            //元数据类别选择组件,无图标子节点向右移
            cc.push('<span class="tree-title" style="margin-left: 8px;">' + opts.formatter.call(target, item) + '</span>');
          } else {
  ...
  $.fn.tree.defaults = {
    ...
    loader: function loader(param, _success, _error) {
    ...
    loadFilter: function loadFilter(data, parent) {
      return data;
    },
    view: defaultView,
    ...
    onBeforeCheck: function onBeforeCheck(node, checked) {},
    ...
    onMouseover: function onMouseover(e, node) {},
    onMouseout: function onMouseout(e, node) {},
    ...
    onDragEnter: function onDragEnter(target, source) {},
    // return false to deny drop
    onDragOver: function onDragOver(target, source) {},
    // return false to deny drop
    onDragLeave: function onDragLeave(target, source) {},
    onBeforeDrop: function onBeforeDrop(target, source, point) {},
    onDrop: function onDrop(target, source, point) {},
...
(function ($) {
  function init(target) {
    ...
    $(target).html('<div class="progressbar-bar"><div class="progressbar-text"></div></div>');
  ...
  function setSize(target, width) {
    ...
    if (opts.color) {
      bar.find('div.progressbar-bar').addClass('progressbar-' + opts.color);
    }

    bar.find('div.progressbar-text,div.progressbar-bar').css({
  ...
  $.fn.progressbar.methods = {
    ...
    setValue: function setValue(jq, value) {
      ...
      return jq.each(function () {
        ...
        opts.value = value;
        $(this).find('div.progressbar-bar').width(value + '%');
        $(this).find('div.progressbar-bar').width(value + '%');
...
(function ($) {
  ...
  $.fn.tooltip.methods = {
    ...
    update: function update(jq, content) {
      return jq.each(function () {
  ...
  $.fn.tooltip.defaults = {
    ...
    valign: 'middle',
    // possible values are: 'middle','top'
    content: null,
    trackMouse: false,
    deltaX: 0,
    deltaY: 0,
...
(function ($) {
  ...
  function setSize(target, param) {
    ...
    opts.hasResized = true; // pheader.add(pbody)._outerWidth(panel.width());
    ...
    if (!isNaN(parseInt(opts.height))) {
      if (isHorizontal) {
        ...
        pfooter._outerHeight(headerHeight); // pbody.css({left:'',right:''}).css(opts.halign, (pheader.position()[opts.halign]+headerWidth)+'px');
    ...
    $(target).panel('doLayout'); //		$(target).find('>div:visible,>form>div:visible').triggerHandler('_resize');
  }
    ...
    function _addHeader() {
      if (opts.noheader || !opts.title && !opts.header) {
        ...
        if (opts.header) {
          ...
          header.empty();
          var htitle; // 布局控件的title单独处理

          if (panel.hasClass("layout-panel")) {
            htitle = $('<div class="panel-title layout-title"></div>').html(opts.title).appendTo(header);
          } else {
            htitle = $('<div class="panel-title"></div>').html(opts.title).appendTo(header);
          }
          ...
          if (opts.collapsible) {
            _buildTool(tool, 'panel-tool-collapse', function () {// debugger;
              // layout布局 需要刷新按钮，注释掉原有的折叠功能
          ...
          if (opts.minimizable) {
            _buildTool(tool, 'panel-tool-min fal fa-window-minimize panel-close', function () {
              minimizePanel(target);
            });
          }

          if (opts.maximizable) {
            _buildTool(tool, 'panel-tool-max fal fa-window-maximize', function () {
          ...
          if (opts.closable) {
            _buildTool(tool, 'panel-tool-close edoc2-icon edoc2-icon-close', function () {
              closePanel(target);
            });
          }
        }
  ...
  function doLayout(target) {
    $(target).panel('doLayout', true); //		$(target).find('div.panel:visible,div.accordion:visible,div.tabs-container:visible,div.layout:visible').each(function(){
  ...
  function maximizePanel(target) {
    ...
    tool.addClass('panel-tool-restore fal fa-window-restore');
  ...
  function restorePanel(target) {
    ...
    panel.show();
    tool.removeClass('panel-tool-restore fa-window-restore');
    $.extend(opts, $.data(target, 'panel').original); //		var original = $.data(target, 'panel').original;
  ...
  $(window).unbind('.panel').bind('resize.panel', function () {
    ...
    resizeTimer = setTimeout(function () {
      ...
      if (layout.length) {
        layout.layout('resize'); //				$('body').children('.easyui-fluid:visible').trigger('_resize');

        $('body').children('.easyui-fluid:visible').each(function () {
          $(this).triggerHandler('_resize');
        });
      } else {
        $('body').panel('doLayout'); //				$('body').children('div.panel:visible,div.accordion:visible,div.tabs-container:visible,div.layout:visible').triggerHandler('_resize');
      }
  ...
  $.fn.panel.methods = {
    ...
    setTitle: function setTitle(jq, title) {
      return jq.each(function () {
        _setTitle(this, title);
      });
    },
    open: function open(jq, forceOpen) {
      return jq.each(function () {
        openPanel(this, forceOpen);
      });
    },
    close: function close(jq, forceClose) {
      return jq.each(function () {
        closePanel(this, forceClose);
      });
    },
    destroy: function destroy(jq, forceDestroy) {
      return jq.each(function () {
    ...
    clear: function clear(jq, type) {
      return jq.each(function () {
        clearPanel(type == 'footer' ? $(this).panel('footer') : this); // clearPanel(this);
      });
    },
    ...
    collapse: function collapse(jq, animate) {
      return jq.each(function () {
        collapsePanel(this, animate);
      });
    },
    expand: function expand(jq, animate) {
      return jq.each(function () {
  ...
  $.fn.panel.defaults = (_$$fn$panel$defaults = {
    ...
    noheader: false,
    content: null,
    // the body content if specified
    halign: 'top',
    // the header alignment: 'top','left','right'
    titleDirection: 'down',
    // up,down
    collapsible: false,
    minimizable: false,
    ...
    queryParams: {},
    method: 'get'
  }, _defineProperty(_$$fn$panel$defaults, "href", null), _defineProperty(_$$fn$panel$defaults, "loadingMessage", 'Loading...'), _defineProperty(_$$fn$panel$defaults, "loader", function loader(param, _success, _error) {
    ...
    // define how to extract the content from ajax response, return extracted data
    var pattern = /<body[^>]*>((.|[\n\r])*)<\/body>/im;
...
(function ($) {
  ...
  function _hcenter(target, tomove) {
  ...
  function _vcenter(target, tomove) {
    var opts = $.data(target, 'window').options;
    var pp = $(target).window('panel');
  ...
  function create(target) {
    ...
    state.window = win.panel('panel'); // create mask
  ...
  $.fn.window.defaults = $.extend({}, $.fn.panel.defaults, {
    ...
    modal: false,
    border: true,
    // possible values are: true,false,'thin','thick'
    inline: false,
...
(function ($) {
  ...
  function setContentSize(target, param) {
    ...
    bb.insertAfter(target); // bb.insertAfter(target).css({
    // 	position:'relative',
    // 	top: -1
    // });
...
(function ($) {
  ...
  function createDialog(options) {
    ...
    dlg.dialog($.extend({}, options, {
      ...
      onClose: function onClose() {
        ...
        dlg.dialog('destroy'); // setTimeout(function(){
        // }, 100);
      }
    }));
  ...
  $.messager = {
    ...
    alert: function alert(title, msg, icon, fn) {
      var opts = _typeof(title) == 'object' ? title : {
        title: title,
        msg: msg,
        icon: icon,
        fn: fn
      ...
      switch (opts.icon) {
        case 'error':
          cls = cls + ' fal fa-times-circle';
          break;

        case 'info':
          cls = cls + ' fal fa-info-circle';
          break;

        case 'question':
          cls = cls + ' fal fa-question-circle';
          break;

        case 'warning':
          cls = cls + ' fal fa-exclamation-circle';
          break;
    ...
    confirm: function confirm(title, msg, fn) {
      var opts = _typeof(title) == 'object' ? title : {
        title: title,
        msg: msg,
        fn: fn
      ...
      opts = $.extend({}, $.messager.defaults, {
        content: '<div><div class="messager-icon messager-question"></div>' + opts.msg + '</div>' + '<div style="clear:both;"/>'
      }, opts);
    ...
    prompt: function prompt(title, msg, fn) {
      var opts = _typeof(title) == 'object' ? title : {
        title: title,
        msg: msg,
        fn: fn
      ...
      opts = $.extend({}, $.messager.defaults, {
        content: '<div class="messager-prompt">' + '<div><div class="messager-icon messager-question"></div>' + opts.msg + '</div>' + '<br/>' + '<div style="clear:both;"/>' + '<div><input class="messager-input" type="text"/></div></div>'
      }, opts);
      ...
      if (!opts.buttons) {
        opts.buttons = [{
          text: opts.ok,
          onClick: function onClick() {
            closeDialog(dlg, dlg.find('.messager-input').val());
          }
        }, {
          text: opts.cancel,
          important: false,
...
(function ($) {
  ...
  // 	}
  // }
  function setSize(container, param) {} // var state = $.data(container, 'accordion');
  // var opts = state.options;
  // var panels = state.panels;
  // var cc = $(container);
  // var isHorizontal = (opts.halign=='left' || opts.halign=='right');
  ...
  function _getSelections(container) {
    return findBy(container, 'collapsed', false, true);
  }

  function _getSelected(container) {
    var pp = _getSelections(container);
  ...
  function _getPanelIndex(container, panel) {
  ...
  function _getPanel(container, which) {
    var panels = $.data(container, 'accordion').panels;
  ...
  function setProperties(container) {
    ...
    if (opts.border) {
      ...
      cc.append('<span class="easyui-nav-bar" />');
    }
  }
  ...
  function init(container) {
    ...
    cc.addClass('accordion-build-' + state.options.mode);
    state.panels = [];
  ...
  function createPanel(container, pp, options) {
    ...
    pp.panel($.extend({}, {
      ...
      onBeforeExpand: function onBeforeExpand() {
        ...
        if (!opts.multiple) {
          ...
          var all = $.grep(_getSelections(container), function (p) {
            return p.panel('options').collapsible;
          });

          for (var i = 0; i < all.length; i++) {
            _unselect(container, _getPanelIndex(container, all[i]));
          }
        }

        if (header.siblings(".accordion-body").children().length == 0) {
          header.addClass('accordion-header-selected');
        }

        header.closest(".easyui-accordion").find("div.panel").find(".accordion-header .panel-icon").attr("select", false);
        header.closest(".easyui-accordion").find("div.panel").find(".accordion-header").attr("select", false);
        header.attr("select", true);
        header.find(".panel-icon").attr("select", true);
        header.addClass('accordion-header-selected');
        header.find('.accordion-collapse').removeClass('accordion-expand fa-plus').addClass('fa-angle-caret-up fa-minus');
      },
      onExpand: function onExpand() {
        ...
        opts.onSelect.call(container, $(this).panel('options').title, _getPanelIndex(container, this));
      },
      onBeforeCollapse: function onBeforeCollapse() {
        ...
        header.find('.accordion-collapse').addClass('accordion-expand fa-plus').removeClass('fa-angle-caret-up fa-minus');
      },
      onCollapse: function onCollapse() {
        ...
        opts.onUnselect.call(container, $(this).panel('options').title, _getPanelIndex(container, this));
      }
    }));
    var header = pp.panel('header'); // var header = pp.parent();

    pp.parent().on({
      mouseover: function mouseover() {
        var current = $(this);
        var bar = current.closest(".easyui-accordion").find(".easyui-nav-bar");
        bar.css("opacity", 1);
        bar.offset({
    ...
    var t = $('<a href="javascript:;"></a>').addClass('accordion-collapse accordion-expand fal fa-angle-caret-down fa-plus').appendTo(tool);
    ...
    pp.parent().addClass(options.panelExtendCls);
    ...
    function togglePanel(p) {
      ...
      if (popts.collapsible) {
        var index = _getPanelIndex(container, p);

        if (popts.collapsed) {
          _select2(container, index);
        } else {
          _unselect(container, index);
        }
      }
    }
  }
  ...
  function _select2(container, which) {
    var p = _getPanel(container, which);
    ...
    var opts = $.data(container, 'accordion').options; // p.panel('expand', opts.animate); anxue

    p.panel('expand', true);
  }

  function _unselect(container, which) {
    var p = _getPanel(container, which);
  ...
  function doFirstSelect(container) {
    ...
    if (p) {
      _select(_getPanelIndex(container, p));
    } else {
  ...
  function _add(container, options) {
    var state = $.data(container, 'accordion');
    var opts = state.options;
    var panels = state.panels;
    ...
    if (options.selected) {
      _select2(container, panels.length - 1);
    }
  }

  function _remove(container, which) {
    var state = $.data(container, 'accordion');
    var opts = state.options;
    var panels = state.panels;
    stopAnimate(container);

    var panel = _getPanel(container, which);

    var title = panel.panel('options').title;

    var index = _getPanelIndex(container, panel);
    ...
    if (panels.length) {
      ...
      var curr = _getSelected(container);

      if (!curr) {
        _select2(container, 0);
      }
    }
  ...
  $.fn.accordion.methods = {
    ...
    //设置滚动条 by tanyo
    setScroll: function setScroll(jq) {},
    ...
    getPanel: function getPanel(jq, which) {
      return _getPanel(jq[0], which);
    },
    getPanelIndex: function getPanelIndex(jq, panel) {
      return _getPanelIndex(jq[0], panel);
    },
    select: function select(jq, which) {
      return jq.each(function () {
        _select2(this, which);
      });
    },
    unselect: function unselect(jq, which) {
      return jq.each(function () {
        _unselect(this, which);
      });
    },
    add: function add(jq, options) {
      return jq.each(function () {
        _add(this, options);
      });
    },
    remove: function remove(jq, which) {
      return jq.each(function () {
        _remove(this, which);
      });
    }
  ...
  $.fn.accordion.defaults = {
    ...
    onUnselect: function onUnselect(title, index) {},
    onAdd: function onAdd(title, index) {},
    onBeforeRemove: function onBeforeRemove(title, index) {},
    onRemove: function onRemove(title, index) {}
  };
})(jQuery);
...
(function ($) {
  ...
  function wrapTabs(container) {
    ...
    $('<div class="tabs-header">' + '<div class="tabs-scroller-left fal fa-angle-double-left"></div>' + '<div class="tabs-scroller-right fal fa-angle-double-right"></div>' + '<div class="tabs-wrap">' + '<ul class="tabs"></ul>' + '</div>' + '</div>').prependTo(container);
  ...
  function setProperties(container) {
    ...
    if (opts.plain == true) {
      ...
      panels.addClass('tab-panels-plain');
    } else {
      header.removeClass('tabs-header-plain');
      panels.removeClass('tab-panels-plain');
    }

    if (opts.brief == true) {
      header.addClass('tabs-header-brief');
      panels.addClass('tab-panels-brief');
    } else {
      header.removeClass('tabs-header-brief');
      panels.removeClass('tab-panels-brief');
    }
    ...
    tabs.removeClass('tabs-brief').addClass(opts.brief ? 'tabs-brief' : '');
  ...
  function createTab(container, options, pp) {
    ...
    pp.panel($.extend({}, options, {
      ...
      onBeforeOpen: function onBeforeOpen() {
        ...
        if (p) {
          if (p[0] != this) {
            $(container).tabs('unselect', _getTabIndex(container, p));
        ...
        var popts = $(this).panel('options');
        popts.tab.addClass('tabs-selected'); // scroll the tab to center position if required.
      ...
      onOpen: function onOpen() {
        ...
        var index = _getTabIndex(container, this); // state.selectHis.push(popts.title);
      ...
      onClose: function onClose() {
        ...
        var popts = $(this).panel('options');
        state.options.onUnselect.call(container, popts.title, _getTabIndex(container, this));
      }
  ...
  function updateTab(container, param) {
    ...
    if (param.type == 'all' || param.type == 'header') {
      ...
      if (opts.header) {
        ...
        if (opts.closable) {
          s_title.closest(".tabs-inner").addClass('tabs-closable'); //<i class="edoc2-icon tabs-close">&#x1006;</i>

          $('<a href="javascript:;" class="edoc2-icon tabs-close">&#x1006;</a>').appendTo(tab);
        } else {
          s_title.closest(".tabs-inner").removeClass('tabs-closable');
        }
    ...
    setSize(container);
    $.data(container, 'tabs').options.onUpdate.call(container, opts.title, _getTabIndex(container, pp));
  }
  /**
  ...
  function closeTab(container, which) {
    ...
    if (!_exists(container, which)) return;

    var tab = _getTab(container, which);

    var title = tab.panel('options').title;

    var index = _getTabIndex(container, tab);

    if (opts.onBeforeClose.call(container, title, index) == false) return;

    var tab = _getTab(container, which, true);

    tab.panel('options').tab.remove();
    tab.panel('destroy');
    opts.onClose.call(container, title, index); //		setScrollers(container);
  ...
  function _getTab(container, which, removeit) {
    var tabs = $.data(container, 'tabs').tabs;
    var tab = null;
  ...
  function _getTabIndex(container, tab) {
    var tabs = $.data(container, 'tabs').tabs;
  ...
  function _exists(container, which) {
    return _getTab(container, which) != null;
  }

  function _showHeader(container, visible) {
    var opts = $.data(container, 'tabs').options;
    opts.showHeader = visible;
    $(container).tabs('resize');
  }

  function _showTool(container, visible) {
  ...
  $.fn.tabs.methods = {
    options: function options(jq) {
      ...
      opts.selected = s ? _getTabIndex(cc, s) : -1;
      return opts;
    },
    ...
    close: function close(jq, which) {
      return jq.each(function () {
        closeTab(this, which);
      });
    },
    getTab: function getTab(jq, which) {
      return _getTab(jq[0], which);
    },
    ...
    exists: function exists(jq, which) {
      return _exists(jq[0], which);
    },
    update: function update(jq, options) {
      return jq.each(function () {
        updateTab(this, options);
      });
    },
    enableTab: function enableTab(jq, which) {
      return jq.each(function () {
    ...
    disableTab: function disableTab(jq, which) {
      return jq.each(function () {
        var opts = $(this).tabs('getTab', which).panel('options');
        opts.tab.addClass('tabs-disabled');
        opts.disabled = true;
    ...
    showHeader: function showHeader(jq) {
      return jq.each(function () {
        _showHeader(this, true);
      });
    },
    hideHeader: function hideHeader(jq) {
      return jq.each(function () {
        _showHeader(this, false);
      });
    },
    ...
    hideTool: function hideTool(jq) {
      return jq.each(function () {
        _showTool(this, false);
      });
    },
    scrollBy: function scrollBy(jq, deltaX) {
  ...
  $.fn.tabs.parseOptions = function (target) {
    return $.extend({}, $.parser.parseOptions(target, ['tools', 'toolPosition', 'tabPosition', {
      fit: 'boolean',
      border: 'boolean',
      plain: 'boolean'
    }, {
  ...
  $.fn.tabs.defaults = {
    ...
    // the tab height
    selected: 0,
    // the initialized selected tab index
    showHeader: true,
    plain: false,
    fit: false,
    border: true,
    ...
    onSelect: function onSelect(title, index) {},
    onUnselect: function onUnselect(title, index) {},
    onBeforeClose: function onBeforeClose(title, index) {},
    onClose: function onClose(title, index) {},
    onAdd: function onAdd(title, index) {},
    onUpdate: function onUpdate(title, index) {},
    onContextMenu: function onContextMenu(e, title, index) {}
  };
})(jQuery);
...
(function ($) {
  ...
  function init(container) {
    var cc = $(container);
    var options = $(container).layout('options');
    cc.addClass('layout');
    cc.addClass('layout-mode-' + options.mode);
    ...
    opts.onAdd = onAdd; // cc.children('form').length ? _add(cc.children('form')) : _add(cc);
  ...
  function addPanel(container, param, el) {
    ...
    var popts = $.extend({}, $.fn.layout.paneldefaults, {
      ...
      // indrive to edoc2ui
      onOpen: function onOpen() {
        var tool = $(this).panel('header').children('div.panel-tool'); // tool.children('a.panel-tool-collapse').hide();	// hide the old collapse button
        ...
        var iconCls = 'layout-button-' + buttonDir[dir]; // add collapse tool to panel header
  ...
  function collapsePanel(container, region, animateSpeed) {
    ...
    var p = panels[region];
    var popts = p.panel('options');
    if (popts.onBeforeCollapse.call(p) == false) return; // expand panel name: expandNorth, expandSouth, expandWest, expandEast
  ...
  $.fn.layout.methods = {
    ...
    panel: function panel(jq, region) {
      return $.data(jq[0], 'layout').panels[region];
    },
    collapse: function collapse(jq, region) {
      return jq.each(function () {
        collapsePanel(this, region);
      });
    },
    expand: function expand(jq, region) {
      return jq.each(function () {
    ...
    remove: function remove(jq, region) {
      return jq.each(function () {
        removePanel(this, region);
        setSize(this);
      });
    },
    split: function split(jq, region) {
      return jq.each(function () {
        setSplit(this, region, true);
      });
    },
    unsplit: function unsplit(jq, region) {
      return jq.each(function () {
  ...
  $.fn.layout.defaults = {
    ...
    mode: "default",
    //page 页面布局，default
    onExpand: function onExpand(region) {},
    onCollapse: function onCollapse(region) {},
    onAdd: function onAdd(region) {},
    onRemove: function onRemove(region) {}
  };
  ...
  $.fn.layout.paneldefaults = $.extend({}, $.fn.panel.defaults, {
    region: null,
    // possible values are: 'north','south','east','west','center'
    split: false,
    collapsedSize: 32,
    expandMode: 'float',
    // possible values are: 'float','dock',null
    hideExpandTool: false,
    hideCollapsedContent: true,
    collapsedContent: function collapsedContent(title) {
      var p = $(this);
...
(function ($) {
  ...
  function createItem(target, div, options) {
    var item = $(div);
    var itemOpts = $.extend({}, $.parser.parseOptions(item[0], ['id', 'name', 'iconCls', 'href', {
      separator: 'boolean'
    }]), {
    ...
    if (!item.hasClass('menu-sep')) {
      ...
      if (item[0].submenu) {
        $('<div class="menu-rightarrow"><i class="fa fa-caret-right" aria-hidden="true"></i> </div>').appendTo(item); // has sub menu
      }
    }
  }
  ...
  function _findItem(target, param) {
    var result = null;
  ...
  function _appendItem(target, param) {
    var opts = $.data(target, 'menu').options;
    var menu = $(target);
    ...
    if (param.parent) {
      if (!param.parent.submenu) {
        ...
        $('<div class="menu-rightarrow"><i class="fa fa-caret-right" aria-hidden="true"></i> </div>').appendTo(param.parent);
  ...
  $.fn.menu.methods = {
     ...
     * }
     */
    setText: function setText(jq, param) {
      return jq.each(function () {
     ...
     * }
     */
    setIcon: function setIcon(jq, param) {
      return jq.each(function () {
        var item = $(param.target).data('menuitem').options;
     ...
     * }
     */
    getItem: function getItem(jq, itemEl) {
    ...
    findItem: function findItem(jq, text) {
      if (typeof text == 'string') {
        return _findItem(jq[0], function (item) {
     ...
     */
    appendItem: function appendItem(jq, param) {
      return jq.each(function () {
        _appendItem(this, param);
      });
    },
    removeItem: function removeItem(jq, itemEl) {
      return jq.each(function () {
        _removeItem(this, itemEl);
      });
    },
    enableItem: function enableItem(jq, itemEl) {
      return jq.each(function () {
        setDisabled(this, itemEl, false);
      });
    },
    disableItem: function disableItem(jq, itemEl) {
      return jq.each(function () {
        setDisabled(this, itemEl, true);
      });
    },
    showItem: function showItem(jq, itemEl) {
      return jq.each(function () {
        setVisible(this, itemEl, true);
      });
    },
    hideItem: function hideItem(jq, itemEl) {
      return jq.each(function () {
        setVisible(this, itemEl, false);
      });
    },
    resize: function resize(jq, menuEl) {
      return jq.each(function () {
  ...
  $.fn.menu.defaults = {
    ...
    itemHeight: 32,
    duration: 100,
    // Defines duration time in milliseconds to hide when the mouse leaves the menu.
    hideOnUnhover: true,
    // Automatically hides the menu when mouse exits it
    inline: false,
    // true to stay inside its parent, false to go on top of all elements
    fit: false,
    noline: false,
    ...
    position: function position(target, left, top) {
      return {
        left: left,
        top: top
      };
...
(function ($) {
  function init(target) {
    ...
    if (opts.hasDownArrow) {
      ...
      var inner = btn.find('.l-btn-left'); // $('<span></span>').addClass(opts.cls.arrow).appendTo(inner);
    ...
    if (opts.menu) {
      ...
      var _onShow = mopts.onShow;
      var _onHide = mopts.onHide;
      $.extend(mopts, {
        onShow: function onShow() {
  ...
  function bindEvents(target) {
    ...
    t.unbind('.menubutton');
    var timeout = null; // t.bind('click.menubutton', function(){
    // 	if (!isDisabled()){
    // 		showMenu(target);
    // 		return false;
    // 	}
...
(function ($) {
  ...
  function init(target) {
    var button = $('<span class="switchbutton">' + '<span class="switchbutton-inner">' + '<span class="switchbutton-on"></span>' + '<span class="switchbutton-handle"></span>' + '<span class="switchbutton-off"></span>' + '<input class="switchbutton-value" type="checkbox" tabindex="-1">' + '</span>' + '</span>').insertAfter(target);
    var t = $(target);
  ...
  function setSize(target, param) {
    ...
    button.find('.switchbutton-handle')._outerWidth(handleWidth + 2)._outerHeight(h).css({
    ...
    button.find('.switchbutton-off').css({
      width: w - handleWidth / 2 + 'px',
      textIndent: (opts.reversed ? '-' : '') + handleWidth / 2 - 6 + 'px'
    });
  ...
  function createButton(target) {
    ...
    if (opts.checked) {
      button.addClass('switchbutton-handle-on').removeClass('switchbutton-handle-off');
    } else {
      button.addClass('switchbutton-handle-off').removeClass('switchbutton-handle-on');
    }
    ...
    svalue.unbind('.switchbutton').bind('change.switchbutton', function (e) {
      return false;
    });
  ...
  function checkButton(target, checked, animate) {
    ...
    var opts = state.options;
    var inner = state.switchbutton.find('.switchbutton-inner');
    var switchbutton = state.switchbutton;
    var labelOn = inner.find('.switchbutton-on');
    var handle = inner.find('.switchbutton-handle');
    ...
    if (opts.checked != checked) {
      if (!!checked) {
        switchbutton.addClass('switchbutton-handle-on').removeClass('switchbutton-handle-off');
      } else {
        switchbutton.addClass('switchbutton-handle-off').removeClass('switchbutton-handle-on');
      }
  ...
  function setDisabled(target, disabled) {
    ...
    if (disabled) {
      opts.disabled = true; // $(target).add(input).attr('disabled', 'disabled');
      ...
      opts.disabled = false; // $(target).add(input).removeAttr('disabled');
...
(function ($) {
  ...
  function focusEventHandler(e) {
    ...
    state.value = opts.val(target); // state.value = undefined;
    ...
    (function () {
      ...
      if (state.validating) {
        ...
        if (state.value != value) {
          // when the value changed, validate it
          state.value = value;
  ...
  function setReadonly(target, mode) {
    ...
    if (opts.readonly || !opts.editable) {
      $(target).triggerHandler('blur.validatebox'); // $(target).addClass('validatebox-readonly').attr('readonly', 'readonly');
  ...
  $.fn.validatebox.defaults = {
    ...
    // Defines whether to validate after creating the component
    validateOnBlur: false,
    // Defines whether to validate when losing focus
    events: {
    ...
    tipOptions: {
      ...
      onHide: function onHide() {// $(this).tooltip('destroy');
      }
    },
    rules: {
      email: {
    ...
    onValidate: function onValidate(valid) {} // fires when validation completes
...
(function ($) {
  ...
  function validate(target) {
    ...
    input.validatebox($.extend({}, opts, {
      deltaX: function deltaX(position) {
        return $(target).textbox('getTipX', position);
      },
      deltaY: function deltaY(position) {
      ...
      onValidate: function onValidate(valid) {
        var box = $(this);
  ...
  function setDisabled(target, disabled) {
    ...
    if (opts.disabled) {
      ...
      tb.addClass('textbox-disabled'); // ss.attr('disabled', 'disabled');
      ...
      tb.removeClass('textbox-disabled'); // ss.removeAttr('disabled');
  ...
  $.fn.textbox.methods = {
    ...
    setText: function setText(jq, value) {
      return jq.each(function () {
    ...
    initValue: function initValue(jq, value) {
      return jq.each(function () {
        var state = $.data(this, 'textbox'); // state.options.value = '';
    ...
    getIcon: function getIcon(jq, index) {
    ...
    // 	}
    // },
    getTipX: function getTipX(jq, position) {
      var state = jq.data('textbox');
      var opts = state.options;
      var tb = state.textbox;
      var input = tb.find('.textbox-text');
    ...
    getTipY: function getTipY(jq, position) {
      var state = jq.data('textbox');
      var opts = state.options;
      var tb = state.textbox;
      var input = tb.find('.textbox-text');
    ...
    setSelectionRange: function setSelectionRange(jq, param) {
      return jq.each(function () {
  ...
  $.fn.textbox.defaults = $.extend({}, $.fn.validatebox.defaults, {
    ...
    multiline: false,
    icons: [],
    // {iconCls:'icon-clear',disabled:true,handler:function(e){}}
    iconCls: null,
    iconAlign: 'right',
    // 'left' or 'right'
    iconWidth: 18,
    ...
    inputEvents: {
      blur: function blur(e) {
        var t = $(e.data.target);
        var opts = t.textbox('options'); // t.textbox('setValue', opts.value);
    ...
    onChange: function onChange(newValue, oldValue) {},
    onResizing: function onResizing(width, height) {},
    onResize: function onResize(width, height) {},
...
(function ($) {
  function buildBox(target) {
    ...
    if (opts.showEye) {
      icons.push({
        iconCls: 'passwordbox-open fa fa-eye',
  ...
  function _setValue(target, value) {
    ...
    opts.revealed ? icon.addClass('passwordbox-close fa-eye-slash') : icon.removeClass('passwordbox-close fa-eye-slash');
  }
  ...
  $.fn.passwordbox.parseOptions = function (target) {
    return $.extend({}, $.fn.textbox.parseOptions(target), $.parser.parseOptions(target, ['passwordChar', {
...
(function ($) {
  function buildSearchBox(target) {
    ...
    var icons = $.extend(true, [], opts.icons);
    icons.push({
      iconCls: 'searchbox-button fal fa-search',
      handler: function handler(e) {
        var t = $(e.data.target);
...
(function ($) {
  ...
  $.fn.form.methods = {
    ...
    submit: function submit(jq, options) {
      return jq.each(function () {
  ...
  $.fn.form.defaults = {
    ...
    success: function success(data) {},
    onBeforeLoad: function onBeforeLoad(param) {},
...
(function ($) {
  ...
  $.fn.numberbox.defaults = $.extend({}, $.fn.textbox.defaults, {
    ...
    parser: function parser(s) {
      s = s + '';
      var opts = $(this).numberbox('options'); // if (parseFloat(s) != s){
...
(function ($) {
  ...
  function init(target) {
    $(target).addClass('calendar').html('<div class="calendar-header">' + '<div class="edoc2-icon edoc2date-icon calendar-prevmonth">&#xe603;</div>' + '<div class="edoc2-icon edoc2date-icon calendar-nextmonth">&#xe602;</div>' + '<div class="edoc2-icon edoc2date-icon calendar-prevyear">&#xe65a;</div>' + '<div class=" edoc2-icon edoc2date-icon calendar-nextyear">&#xe65b;</div>' + '<div class="calendar-title">' + '<span class="calendar-text"></span>' + '</div>' + '</div>' + '<div class="calendar-body">' + '<div class="calendar-menu">' + '<div class="calendar-menu-year-inner">' + '<i class="fa fa-backward calendar-menu-prev" aria-hidden="true"></i>' + '<span><input class="calendar-menu-year" type="text"></input></span>' + '<i class="fa fa-forward calendar-menu-next" aria-hidden="true"></i>' + '</div>' + '<div class="calendar-menu-month-inner">' + '</div>' + '</div>' + '</div>');
    $(target).bind('_resize', function (e, force) {
      if ($(this).hasClass('easyui-fluid') || force) {
        setSize(target);
      }
  ...
  function bindEvents(target) {
    ...
    $(target).unbind('.calendar').bind('mouseover.calendar', function (e) {
      ...
      if (t.hasClass('calendar-nav') || t.hasClass('calendar-day') && !t.hasClass('calendar-disabled')) {
  ...
  function show(target) {
    ...
    var currentInfo = opts.current ? opts.current.getFullYear() + ',' + (opts.current.getMonth() + 1) + ',' + opts.current.getDate() : ''; // calulate the saturday and sunday index
  ...
  $.fn.calendar.parseOptions = function (target) {
    var t = $(target);
    return $.extend({}, $.parser.parseOptions(target, ['weekNumberHeader', {
      firstDay: 'number',
      fit: 'boolean',
      border: 'boolean',
      showWeek: 'boolean'
  ...
  $.fn.calendar.defaults = {
    ...
    onChange: function onChange(newDate, oldDate) {},
    onNavigate: function onNavigate(year, month) {}
  };
})(jQuery);
...
(function ($) {
  function buildSpinner(target) {
    ...
    if (opts.spinArrow) {
      ...
      arrowIcon.append('<span class="spinner-arrow spinner-button-top">' + '<span class="spinner-arrow-up fal fa-angle-up"></span>' + '</span>' + '<span class="spinner-arrow spinner-button-bottom">' + '<span class="spinner-arrow-down fal fa-angle-down"></span>' + '</span>');
    } else {
      ...
      btnLeft.linkbutton({
        iconCls: opts.reversed ? 'spinner-button-up fa fa-plus' : 'spinner-button-down fa fa-minus',
        onClick: function onClick() {
          doSpin(target, !opts.reversed);
        }
      });
      btnRight.linkbutton({
        iconCls: opts.reversed ? 'spinner-button-down fa fa-minus' : 'spinner-button-up fa fa-plus',
        onClick: function onClick() {
          doSpin(target, opts.reversed);
        }
      });
  ...
  $.fn.spinner.defaults = $.extend({}, $.fn.textbox.defaults, {
    ...
    increment: 1,
    spinAlign: 'right',
    // possible values: 'left','right','horizontal','vertical'
    reversed: false,
    spin: function spin(down) {},
    // the function to implement the spin button clicking
...
(function ($) {
  ...
  function highlight(target, index) {
    ...
    if (range) {
      ...
      $(target).timespinner('setSelectionRange', {
        start: range[0],
        end: range[1]
      });
      tb.focus();
  ...
  function doSpin(target, down) {
    ...
    if (s2 == opts.ampm[0]) {
      ...
      s2 = s2 + opts.increment * (down ? -1 : 1); // s2 = (parseInt(s2,10)||0) + opts.increment*(down?-1:1);
    }
...
(function ($) {
  ...
  function indexOfArray(a, o) {
    return $.easyui.indexOfArray(a, o); // for(var i=0,len=a.length; i<len; i++){
    // 	if (a[i] == o) return i;
    // }
    // return -1;
  }
  ...
  function removeArrayItem(a, o, id) {
    $.easyui.removeArrayItem(a, o, id); // if (typeof o == 'string'){
    // 	for(var i=0,len=a.length; i<len; i++){
    // 		if (a[i][o] == id){
    // 			a.splice(i, 1);
    // 			return;
  ...
  function addArrayItem(a, o, r) {
    $.easyui.addArrayItem(a, o, r); // for(var i=0,len=a.length; i<len; i++){
    // 	if (a[i][o] == r[o]){return;}
    // }
    // a.push(r);
  }
  ...
  function _createStyleSheet(target) {
    ...
    return {
      add: function add(lines) {
        ...
        ss.push('</style>');
        $(ss.join('\n')).appendTo(cc);
        cc.children('style[easyui]:not(:last)').remove(); //				setTimeout(function(){
        //					cc.children('style[easyui]:not(:last)').remove();
        //				}, 0);
      },
      getRule: function getRule(index) {
      ...
      set: function set(selector, width) {
  ...
  function setBodySize(target) {
    ...
    var opts = state.options;
    var dc = state.dc;
    var wrap = state.panel; //支持小表格样式 by tanyo

    if (opts.type == "small") {
      wrap.addClass("datagrid-small");
    } else {
      wrap.addClass("datagrid-big");
    }
    ...
    var header3 = view3.children('div.datagrid-header');
    var table1 = header1.find('table');
    var table2 = header2.find('table');
    var table3 = header3.find('table'); // set view width
    ...
    var headerInner3 = header3.children('div.datagrid-header-inner').show();
    var view3Width = headerInner3.find('table').width();
    view3.width(view3Width);
    ...
    view2.width(innerWidth - view1._outerWidth() - view3._outerWidth());
    view2.css("right", view3Width);

    view1.children()._outerWidth(view1.width());

    view2.children()._outerWidth(view2.width());

    view3.children()._outerWidth(view3.width()); // set header height


    var all = header1.add(header2).add(header3).add(table1).add(table2).add(table3);
    all.css('height', '');
    var hh = Math.max(table1.height(), table2.height(), table3.height());

    all._outerHeight(hh); // set the position of empty message


    view.children('.datagrid-empty').css('top', hh + 'px'); // set body height
    ...
    wrap.children(':not(.datagrid-view,.datagrid-mask,.datagrid-mask-msg,.datagrid-mask-loading)').each(function () {
    ...
    view1.add(view2).add(view3).children('div.datagrid-body').css({
  ...
  function _fixRowHeight(target, index, forceFix) {
    ...
    if (opts.height == 'auto') {
      ...
      height -= parseInt(body2.css('marginTop')) || 0; //不设置body高度
      // body1.height(height);
      // body2.height(height);
    ...
    dc.body2.triggerHandler('scroll'); // set body row or footer row height

    function setHeight(trs1, trs2) {
      for (var i = 0; i < trs2.length; i++) {
        var tr1 = $(trs1[i]);
        var tr2 = $(trs2[i]); // tr1.css('height', 'auto');
        // tr2.css('height', 'auto');
  ...
  function wrapGrid(target, rownumbers, opts) {
    function getColumns() {
      ...
      $(target).children('thead').each(function () {
        ...
        $(this).find('tr').each(function () {
          var cols = [];
          var tempColumns = frozenColumns;

          if (tempColumns.length > 0) {
            tempColumns = frozenRightColumns;
          }
          ...
          opt.frozen ? tempColumns.push(cols) : columns.push(cols);
        });
      });
      return [frozenColumns, columns, frozenRightColumns];
    }

    var mode = opts.mode ? "datagrid-" + opts.mode : "";
    var panel = $('<div class="datagrid-wrap ' + mode + '">' + '<div class="datagrid-view">' + '<div class="datagrid-view1">' + '<div class="datagrid-header">' + '<div class="datagrid-header-inner"></div>' + '</div>' + '<div class="datagrid-body">' + '<div class="datagrid-body-inner"></div>' + '</div>' + '<div class="datagrid-footer">' + '<div class="datagrid-footer-inner"></div>' + '</div>' + '</div>' + '<div class="datagrid-view2">' + '<div class="datagrid-header">' + '<div class="datagrid-header-inner"></div>' + '</div>' + '<div class="datagrid-body"></div>' + '<div class="datagrid-footer">' + '<div class="datagrid-footer-inner"></div>' + '</div>' + '</div>' + '<div class="datagrid-view3">' + '<div class="datagrid-header">' + '<div class="datagrid-header-inner"></div>' + '</div>' + '<div class="datagrid-body"></div>' + '<div class="datagrid-footer">' + '<div class="datagrid-footer-inner"></div>' + '</div>' + '</div>' + //						'<div class="datagrid-resize-proxy"></div>' +
    ...
    var view3 = view.children('div.datagrid-view3');
    return {
      panel: panel,
      frozenColumns: cc[0],
      frozenRightColumns: cc.length == 3 ? cc[2] : [],
      columns: cc[1],
      dc: {
        // some data container
        view: view,
        ...
        header3: view3.children('div.datagrid-header').children('div.datagrid-header-inner'),
        body1: view1.children('div.datagrid-body').children('div.datagrid-body-inner'),
        body2: view2.children('div.datagrid-body'),
        body3: view3.children('div.datagrid-body'),
        ...
        footer3: view3.children('div.datagrid-footer').children('div.datagrid-footer-inner')
      }
    };
  }
  ...
  function buildGrid(target) {
    ...
    createColumnHeader(dc.header3, opts.frozenRightColumns, true, true);
    createColumnStyle();
    dc.header1.add(dc.header2).add(dc.header3).css('display', opts.showHeader ? 'block' : 'none');
    dc.footer1.add(dc.footer2).add(dc.footer3).css('display', opts.showFooter ? 'block' : 'none');
        ...
        if (opts.toolbar.length > 0) {
          ...
          for (var i = 0; i < opts.toolbar.length; i++) {
            ...
            if (btn == '-') {
              ...
              if (btn.menu == true && btn.menuStr) {
                tool.menubutton($.extend({}, btn, {
                  plain: true,
                  menu: btn.menuStr
                }));
              } else {
    ...
    if (opts.ownbar && opts.ownbar.length > 0) {
      var toolbar = $('div.datagrid-toolbar', panel);
      var ownbarDom;

      if (toolbar.length == 0) {
        var oBar = $('<div class="datagrid-toolbar" style="min-height: 40px;"><div class="data-grid-ownbar" ></div></div>').prependTo(panel);
        ownbarDom = oBar.find(".data-grid-ownbar");
      } else {
        ownbarDom = $('<div class="data-grid-ownbar" ></div>').prependTo(toolbar);
        toolbar[0].className = toolbar[0].className + ' datagrid-toolbar-min-height';
      }

      for (var i = 0; i < opts.ownbar.length; i++) {
        var btn = opts.ownbar[i];

        if (btn == "column") {
          var menutool = $('<a href="javascript:;" class="easyui-menu data-grid-menu" attr="' + btn + '"></a>').prependTo(ownbarDom);
          var mm = "<div id='mm'><ul class='menu-check-box'></ul></div>";
          var om = $(mm).prependTo(ownbarDom);
          var ul = om.find("ul");

          if (opts.frozenColumns && opts.frozenColumns[0]) {
            for (var j = 0; j < opts.frozenColumns[0].length; j++) {
              if (opts.frozenColumns[0][j] && opts.frozenColumns[0][j].title && opts.frozenColumns[0][j].field) {
                $('<li class="menu-check-item"><input name="column" value="' + opts.frozenColumns[0][j].field + '" label="' + opts.frozenColumns[0][j].title + '"/></li>').appendTo(ul);
              }
            }
          }

          if (opts.columns && opts.columns[0]) {
            for (var c = 0; c < opts.columns[0].length; c++) {
              if (opts.columns[0][c] && opts.columns[0][c].title && opts.columns[0][c].field) {
                $('<li class="menu-check-item"><input name="column" value="' + opts.columns[0][c].field + '" label="' + opts.columns[0][c].title + '"/></li>').appendTo(ul);
                ;
              }
            }
          }

          if (opts.frozenRightColumns && opts.frozenRightColumns[0]) {
            for (var r = 0; r < opts.frozenRightColumns[0].length; r++) {
              if (opts.frozenRightColumns[0][r] && opts.frozenRightColumns[0][r].title && opts.frozenRightColumns[0][r].field) {
                $('<li class="menu-check-item"><input name="column" value="' + opts.frozenRightColumns[0][r].field + '" label="' + opts.frozenRightColumns[0][r].title + '"/></li>').appendTo(ul);
              }
            }
          }
          ...
          menutool.menubutton({
            plain: true,
            iconCls: 'fal edoc2-icon edoc2-icon-cols',
            menu: '#mm'
          });
          ul.find('input[name="column"]').checkbox({
            labelPosition: "after",
            checked: true,
            onChange: function onChange() {
              var columns = ul.find('.checkbox-f');
              var shows = [];
              var hides = [];

              for (var i = 0; i < columns.length; i++) {
                var options = $(columns[i]).checkbox("options");

                if (options.checked == true) {
                  shows.push(options.value);
                } else {
                  hides.push(options.value);
                }
              }

              for (var i = 0; i < shows.length; i++) {
                $(target).datagrid('showColumn', shows[i]);
              }

              for (var i = 0; i < hides.length; i++) {
                $(target).datagrid('hideColumn', hides[i]);
              }
            }
          });
        } else if (btn == "export") {//导出先不做
          // var printool= $('<a href="javascript:;" class="easyui-linkbutton" attr="'+btn+'"></a>').appendTo(ownbarDom);
          // printool.linkbutton({size:'sm',
          // 	 plain:true,
          // 	 iconCls:"edoc2-icon edoc2-icon-export"
          //    });
        } else if (btn == "print") {
          var exportool = $('<a href="javascript:;" class="easyui-linkbutton" attr="' + btn + '"></a>').appendTo(ownbarDom);
          ;
          exportool.linkbutton({
            size: 'sm',
            plain: true,
            iconCls: "fal edoc2-icon edoc2-icon-print",
            onClick: function onClick() {
              var printWin = window.open('打印窗口', '_blank'),
                  style = "",
                  html = $(dc.header2.html()); //输出表头

              var aStyle = ['<style>', 'body{font-size: 12px; color: #666;}', 'table{width: 100%; border-collapse: collapse; border-spacing: 0;}', 'th,td{line-height: 20px; padding: 9px 15px; border: 1px solid #ccc; text-align: left; font-size: 12px; color: #666;}', 'a{color: #666; text-decoration:none;}'];
              aStyle.push('</style>');
              style = aStyle.join(" ");
              html.append(dc.body2.find("table").html()); //输出表体

              html.append(dc.footer2.find("table").html()); //输出合计行

              html.find(".datagrid-header-check").closest("td").remove();
              html.find(".datagrid-cell-check").closest("td").remove();
    ...
    if (opts.pagination) {
      ...
      if (opts.type == "small") {
        ...
        pagerSettings.layout = ["list", "sep", "prev", "sep", "manual", "sep", "next", "sep", "info"];
      }

      pager.pagination(pagerSettings);
      opts.pageSize = pager.pagination('options').pageSize; // repare the pageSize value
    } ///是否固定右侧列


    function createColumnHeader(container, columns, frozen, isRight) {
      if (!columns) return;
      $(container).show();
      $(container).empty();
      ...
      for (var i = 0; i < columns.length; i++) {
        ...
        for (var j = 0; j < cols.length; j++) {
          ...
          if (col.checkbox) {
            ...
            var ckeckHtml = '<input type="checkbox"/>';

            if (opts.singleSelect) {
              ckeckHtml = "";
            }

            $('<div class="datagrid-header-check"></div>').html(ckeckHtml).appendTo(td);
            td.find('input[type="checkbox"]').checkbox();
          } else if (col.field) {
            td.attr('field', col.field);
            td.append('<div class="datagrid-cell"><span></span><span class="edoc2-table-sort edoc2-inline"><i class="edoc2-edge edoc2-table-sort-asc" title="升序"></i><i class="edoc2-edge edoc2-table-sort-desc" title="降序"></span></div>');
            ...
            cell.css('text-align', col.halign || col.align || ''); // define the cell class
      ...
      if (frozen && opts.rownumbers && !isRight) {
    ...
    function createColumnStyle() {
      ...
      var fields = _getColumnFields(target, true).concat(_getColumnFields(target)).concat(_getColumnFields(target, true, true));

      for (var i = 0; i < fields.length; i++) {
        var col = _getColumnOption(target, fields[i]);
  ...
  function bindEvents(target) {
    ...
    dc.body2.bind('scroll', function () {
      ...
      $(this).scrollTop(stv);
      b1.scrollTop(stv); // b1.scrollTop($(this).scrollTop());
  ...
  function headerClickEventHandler(e) {
    ...
    var ck = $(e.target).closest(".datagrid-header-check").find(".checkbox-f");
    ;
    ...
    if (ck.length) {
      ...
      var options = ck.checkbox("options");
  ...
  function hoverEventHandler(isOver) {
    return function (e) {
      ...
      if (isOver) {
        _highlightRow(target, index); // $(target).datagrid('highlightRow', index);
  ...
  function clickEventHandler(e) {
    ...
    var index = getTrIndex(tr);
    var tt = $(e.target);
    var cellContainer = tt.closest(".datagrid-cell-check");

    if (cellContainer.length > 0) {
      // click the checkbox
      var ck = cellContainer.find(".checkbox-f");
      var checked = ck.checkbox("options").checked;
  ...
  function sortGrid(target, param) {
    ...
    for (var i = 0; i < names.length; i++) {
      var col = _getColumnOption(target, names[i]);
    ...
    if (opts.remoteSort) {
      ...
      _loadData(target, $(target).datagrid('getData'));
    }
  ...
  function _fitColumns(target) {
    ...
    function fillWidth() {
      ...
      var fields = _getColumnFields(target, false);
  ...
  function _autoSizeColumn(target, field) {
    ...
    function setSize(field) {
      ...
      headerCell.css('width', '');
      $(target).datagrid('fixColumnSize', field);
      opts.onResizeColumn.call(target, field, col.width); // get cell width of specified type(body or footer)
  ...
  function _fixColumnSize(target, field) {
    var state = $.data(target, 'datagrid');
    var opts = state.options;
    var dc = state.dc;
    ...
    if (field) {
      ...
      var ff = _getColumnFields(target, true).concat(_getColumnFields(target, false)); // get all fields
  ...
  function fixMergedSize(target, tds) {
    ...
    tds.each(function () {
      ...
      if (colspan > 1) {
        var col = _getColumnOption(target, td.attr('field'));

        var width = col.boxWidth + col.deltaWidth - 1;

        for (var i = 1; i < colspan; i++) {
          td = td.next();
          col = _getColumnOption(target, td.attr('field'));
  ...
  function fixEditableSize(target) {
    ...
    dc.view.find('div.datagrid-editable').each(function () {
      ...
      var col = $(target).datagrid('getColumnOption', field);
      var width = col.boxWidth + col.deltaWidth - 1;
      var isClear = isClearPadding(col.editor);

      if (isClear) {
        cell.closest("td").css("padding", "0px");
        width += 10;
      }
      ...
      ed.actions.gridType = opts.type;

      if (ed.actions.resize) {
        ed.actions.resize(ed.target, width);
      }
    });
  }
  ...
  function _getColumnOption(target, field) {
    ...
    if (!col) {
      col = find(opts.frozenRightColumns);
    }

    return col;
  }

  function getColumnLayout(target, frozen, frozenRight) {
    var opts = $.data(target, 'datagrid').options;
    var columns = frozen ? opts.frozenColumns : opts.columns;

    if (frozenRight) {
      columns = opts.frozenRightColumns;
    }
  ...
  function _getColumnFields(target, frozen, isRight) {
    var aa = getColumnLayout(target, frozen, isRight);
  ...
  function _loadData(target, data) {
    ...
    if (!opts.remoteSort && opts.sortName) {
      ...
      data.rows.sort(function (r1, r2) {
        ...
        for (var i = 0; i < names.length; i++) {
          ...
          var col = _getColumnOption(target, sn);
    ...
    opts.view.render.call(opts.view, target, dc.body3, true, true);
    opts.view.render.call(opts.view, target, dc.body2, false);
    opts.view.render.call(opts.view, target, dc.body1, true);

    if (opts.showFooter) {
      opts.view.renderFooter.call(opts.view, target, dc.footer3, true, true);
    ...
    state.ss.clean(); //		opts.onLoadSuccess.call(target, data);
    ...
    if (data.total === 0) {
      opts.view.renderEmptyRow.call(opts.view, target);
    }
  ...
  function _setSelectionState(target) {
    var state = $.data(target, 'datagrid');
    var opts = state.options;
    var dc = state.dc;
    var chk = dc.header1.add(dc.header2).find('.checkbox-f');
  ...
  function _scrollTo(target, index) {
    var state = $.data(target, 'datagrid');
    var dc = state.dc;
    var opts = state.options;
    ...
    if (tr.length) {
      ...
      var frozenHeight = body2.outerHeight(true) - body2.outerHeight(); // var top = tr.position().top - headerHeight - frozenHeight;
  ...
  function _highlightRow(target, index) {
    var state = $.data(target, 'datagrid');
    var opts = state.options;
    ...
    state.highlightIndex = index; //		scrollTo(target, index);
  }
  /**
  ...
  function _selectRow(target, index, notCheck, notScroll) {
    var state = $.data(target, 'datagrid');
    var opts = state.options;
    var row = opts.finder.getRow(target, index);
    ...
    if (!notCheck && opts.checkOnSelect) {
      _checkRow(target, index, true); // don't select the row again
  ...
  function _unselectRow(target, index, notCheck) {
    var state = $.data(target, 'datagrid');
    var dc = state.dc;
    var opts = state.options;
    var row = opts.finder.getRow(target, index);
    ...
    if (!notCheck && opts.checkOnSelect) {
      _uncheckRow(target, index, true); // don't unselect the row again
    ...
    if (opts.singleSelect) {
      tr.removeClass('datagrid-row-single-selected');
    }
  ...
  function _selectAll(target, notCheck) {
    var state = $.data(target, 'datagrid');
    var opts = state.options;
    var rows = opts.finder.getRows(target);
    ...
    if (!notCheck && opts.checkOnSelect) {
      _checkAll(target, true); // don't select rows again
  ...
  function _unselectAll(target, notCheck) {
    var state = $.data(target, 'datagrid');
    var opts = state.options;
    var rows = opts.finder.getRows(target);
    var selectedRows = $.data(target, 'datagrid').selectedRows;
    ...
    if (!notCheck && opts.checkOnSelect) {
      _uncheckAll(target, true); // don't unselect rows again

    }

    var trs = opts.finder.getTr(target, '', 'selected');
    trs.removeClass('datagrid-row-selected');

    if (opts.singleSelect) {
      trs.removeClass('datagrid-row-single-selected');
    }
  ...
  function _checkRow(target, index, notSelect) {
    var state = $.data(target, 'datagrid');
    var opts = state.options;
    var row = opts.finder.getRow(target, index);
    ...
    if (!notSelect && opts.selectOnCheck) {
      _selectRow(target, index, true); // don't check the row again

    }

    var tr = opts.finder.getTr(target, index).addClass('datagrid-row-checked');
    tr.find('div.datagrid-cell-check .checkbox-f').checkbox('check');
    tr = opts.finder.getTr(target, '', 'checked', 2);

    if (tr.length == opts.finder.getRows(target).length) {
      var dc = state.dc;
      dc.header1.add(dc.header2).find('.checkbox-f').checkbox('check');
    }
  ...
  function _uncheckRow(target, index, notSelect) {
    var state = $.data(target, 'datagrid');
    var opts = state.options;
    var row = opts.finder.getRow(target, index);
    ...
    if (!notSelect && opts.selectOnCheck) {
      _unselectRow(target, index, true); // don't uncheck the row again

    }

    var tr = opts.finder.getTr(target, index).removeClass('datagrid-row-checked');
    tr.find('div.datagrid-cell-check .checkbox-f').checkbox('uncheck');
    var dc = state.dc;
    var header = dc.header1.add(dc.header2);
    header.find('.checkbox-f').checkbox('uncheck');
  ...
  function _checkAll(target, notSelect) {
    var state = $.data(target, 'datagrid');
    var opts = state.options;
    var rows = opts.finder.getRows(target);

    if (!notSelect && opts.selectOnCheck) {
      _selectAll(target, true); // don't check rows again
    ...
    var checkboxs = hck.add(bck).filter(".checkbox-f");
    checkboxs.checkbox("check"); //._propAttr('checked', true);
  ...
  function _uncheckAll(target, notSelect) {
    var state = $.data(target, 'datagrid');
    var opts = state.options;
    var rows = opts.finder.getRows(target);

    if (!notSelect && opts.selectOnCheck) {
      _unselectAll(target, true); // don't uncheck rows again

    }

    var dc = state.dc;
    var hck = dc.header1.add(dc.header2).find('input[type=checkbox]');
    var bck = opts.finder.getTr(target, '', 'checked').removeClass('datagrid-row-checked').find('div.datagrid-cell-check input[type=checkbox]'); //		var bck = opts.finder.getTr(target, '', 'allbody').find('div.datagrid-cell-check input[type=checkbox]');

    var checkboxs = hck.add(bck).filter(".checkbox-f"); //._propAttr('checked', false);

    checkboxs.checkbox("uncheck");
  ...
  function _beginEdit(target, index) {
    var opts = $.data(target, 'datagrid').options;
    var tr = opts.finder.getTr(target, index);
    var row = opts.finder.getRow(target, index);
    ...
    _validateRow(target, index); // validate the row data
  ...
  function _endEdit(target, index, cancel) {
    var state = $.data(target, 'datagrid');
    var opts = state.options;
    ...
    if (!cancel) {
      if (!_validateRow(target, index)) return; // invalid row data
  ...
  function _getEditor(target, param) {
    var editors = _getEditors(target, param.index != undefined ? param.index : param.id);
  ...
  function isClearPadding(editor) {
    var typeStr = "";
    ...
    var types = ['text', 'textbox', 'passwordbox', 'filebox', 'numberbox', 'numberspinner', 'combobox', 'combotree', 'combogrid', 'combotreegrid', 'datebox', 'datetimebox', 'timespinner', 'datetimespinner'];

    if ($.inArray(typeStr, types) > -1) {
      return true;
    }
  ...
  function createEditor(target, index) {
    ...
    tr.children('td').each(function () {
      ...
      if (col && col.editor) {
        ...
        if (editor) {
          ...
          var editorHeight = opts.type == "big" ? $.fn.datagrid.defaults.editorHeight : $.fn.datagrid.defaults.editorSmallHeight;

          if (edittype == "combo" || edittype == "combobox") {
            editoptions.type = opts.type;
          }
    ...
    _fixRowHeight(target, index, true);
  }
  /**
  ...
  function _validateRow(target, index) {
  ...
  function _getChanges(target, state) {
  ...
  function _deleteRow(target, index) {
    ...
    if (opts.height == 'auto') {
      _fixRowHeight(target); // adjust the row height
  ...
  function _insertRow(target, param) {
  ...
  function _updateRow(target, param) {
    var state = $.data(target, 'datagrid');
    var opts = state.options;
  ...
  function _rejectChanges(target) {
    ...
    function doSelect(ids, action) {
      for (var i = 0; i < ids.length; i++) {
        var index = _getRowIndex(target, ids[i]);

        if (index >= 0) {
          (action == 's' ? _selectRow : _checkRow)(target, index, true);
        }
      }
    }
  ...
  $.fn.datagrid = function (options, param) {
    ...
    return this.each(function () {
      ...
      if (state) {
        ...
        if (!opts.frozenRightColumns) opts.frozenRightColumns = wrapResult.frozenRightColumns;
        opts.columns = $.extend(true, [], opts.columns);
        opts.frozenColumns = $.extend(true, [], opts.frozenColumns);
        opts.frozenRightColumns = $.extend(true, [], opts.frozenRightColumns);
        ...
        if (opts.frozenColumns && opts.frozenColumns.length > 0) {
          ...
          if (opts.columns && opts.columns.length > 0) {
            $.each(opts.columns[0], function (i, item) {
              if (item.checkbox) {
                checkboxField = item;
              } else {
                columns.push(item);
          ...
          if (checkboxField) {
            ...
            opts.frozenColumns[0].unshift(checkboxField);
          }
        }
      ...
      return {
        ...
        resize: function resize(target, width) {
          if (isA(target, name)) {
            ...
            var height = $.fn.datagrid.defaults.editorHeight;

            if (this.gridType == "small") {
              height = $.fn.datagrid.defaults.editorSmallHeight;
            }
  ...
  var editors = $.extend({}, getDefaultEditors(['text', 'textbox', 'passwordbox', 'filebox', 'numberbox', 'numberspinner', 'combobox', 'combotree', 'combogrid', 'combotreegrid', 'datebox', 'datetimebox', 'timespinner', 'datetimespinner']), {
    ...
    checkbox: {
      init: function init(container, options) {
        ...
        container.find('input[type="checkbox"]').checkbox({
          value: options.on,
          checked: true
        });
  ...
  $.fn.datagrid.methods = {
    ...
    setSelectionState: function setSelectionState(jq) {
      return jq.each(function () {
        _setSelectionState(this);
      });
    },
    createStyleSheet: function createStyleSheet(jq) {
      return _createStyleSheet(jq[0]);
    },
    ...
    getColumnFields: function getColumnFields(jq, param) {
      //兼容老的api
      var oParam = {};
      ...
      return _getColumnFields(jq[0], oParam.frozen, oParam.frozenRight);
    },
    getColumnOption: function getColumnOption(jq, field) {
    ...
    load: function load(jq, params) {
      return jq.each(function () {
        ...
        opts.pageNumber = 1;
        var pager = $(this).datagrid('getPager'); //				pager.pagination({pageNumber:1});
    ...
    reload: function reload(jq, params) {
      return jq.each(function () {
        var opts = $(this).datagrid('options');
    ...
    reloadFooter: function reloadFooter(jq, footer) {
      return jq.each(function () {
        ...
        if (opts.showFooter) {
          opts.view.renderFooter.call(opts.view, this, dc.footer3, false);
    ...
    loading: function loading(jq) {
      return jq.each(function () {
        ...
        if (!panel.children('div.datagrid-mask-loading').length) {
          var msg = $('<div class="datagrid-mask-loading" style="display:block;left:50%"></div>').html('<i  class="fas fa-spinner datagrid-loading fa-pulse"></i>').appendTo(panel);
    ...
    loaded: function loaded(jq) {
      return jq.each(function () {
        ...
        var panel = $(this).datagrid('getPanel');
        panel.children('div.datagrid-mask-loading').remove();
      });
    },
    ...
    fixColumnSize: function fixColumnSize(jq, field) {
      return jq.each(function () {
        _fixColumnSize(this, field);
      });
    },
    fixRowHeight: function fixRowHeight(jq, index) {
      return jq.each(function () {
        _fixRowHeight(this, index);
      });
    },
    freezeRow: function freezeRow(jq, index) {
      return jq.each(function () {
    ...
    scrollTo: function scrollTo(jq, index) {
      return jq.each(function () {
        _scrollTo(this, index);
      });
    },
    highlightRow: function highlightRow(jq, index) {
      return jq.each(function () {
        _highlightRow(this, index);
    ...
    selectRow: function selectRow(jq, index) {
      return jq.each(function () {
    ...
    selectRecord: function selectRecord(jq, id) {
      return jq.each(function () {
        ...
        if (opts.idField) {
          var index = _getRowIndex(this, id);
    ...
    unselectRow: function unselectRow(jq, index) {
      return jq.each(function () {
        _unselectRow(this, index);
      });
    },
    checkRow: function checkRow(jq, index) {
      return jq.each(function () {
        _checkRow(this, index);
      });
    },
    uncheckRow: function uncheckRow(jq, index) {
      return jq.each(function () {
    ...
    beginEdit: function beginEdit(jq, index) {
      return jq.each(function () {
        _beginEdit(this, index);
      });
    },
    endEdit: function endEdit(jq, index) {
      return jq.each(function () {
        _endEdit(this, index, false);
      });
    },
    cancelEdit: function cancelEdit(jq, index) {
      return jq.each(function () {
        _endEdit(this, index, true);
      });
    },
    getEditors: function getEditors(jq, index) {
      return _getEditors(jq[0], index);
    },
    getEditor: function getEditor(jq, param) {
      // param: {index:0, field:'name'}
      return _getEditor(jq[0], param);
    },
    refreshRow: function refreshRow(jq, index) {
      return jq.each(function () {
        var opts = $.data(this, 'datagrid').options;
        opts.view.refreshRow.call(opts.view, this, index);
      });
    },
    validateRow: function validateRow(jq, index) {
      return _validateRow(jq[0], index);
    },
    updateRow: function updateRow(jq, param) {
      // param: {index:1,row:{code:'code1',name:'name1'}}
      return jq.each(function () {
        _updateRow(this, param); // var opts = $.data(this, 'datagrid').options;
    ...
    insertRow: function insertRow(jq, param) {
      return jq.each(function () {
    ...
    getChanges: function getChanges(jq, state) {
      return _getChanges(jq[0], state); // state: inserted,deleted,updated
    },
    ...
    mergeCells: function mergeCells(jq, param) {
      return jq.each(function () {
        _mergeCells(this, param);
      });
    },
    showColumn: function showColumn(jq, field) {
      return jq.each(function () {
    ...
    hideColumn: function hideColumn(jq, field) {
      return jq.each(function () {
        var col = $(this).datagrid('getColumnOption', field);
    ...
    sort: function sort(jq, param) {
      return jq.each(function () {
        sortGrid(this, param);
      });
    },
    gotoPage: function gotoPage(jq, param) {
      return jq.each(function () {
        var target = this;
        var page, cb;
  ...
  var defaultView = {
    render: function render(target, container, frozen, frozenRight) {
      var rows = $(target).datagrid('getRows');
      $(container).empty().html(this.renderTable(target, 0, rows, frozen, frozenRight));

      if (frozen) {
        if (frozenRight) {
          $(container).closest(".datagrid-view3").addClass("datagrid-frozen-right");
        } else {
          $(container).closest(".datagrid-view1").addClass("datagrid-frozen-left");
        }
      }

      $(container).find('input[type="checkbox"]').checkbox();
    },
    renderFooter: function renderFooter(target, container, frozen, frozenRight) {
      var opts = $.data(target, 'datagrid').options;
      var rows = $.data(target, 'datagrid').footer || [];
      var fields = $(target).datagrid('getColumnFields', frozen, frozenRight);
    ...
    renderTable: function renderTable(target, index, rows, frozen, frozenRight) {
      var state = $.data(target, 'datagrid');
      var opts = state.options;

      if (frozen && !frozenRight) {
      ...
      for (var i = 0; i < rows.length; i++) {
        var row = rows[i]; // get the class and style attributes for this row
        ...
        table.push(this.renderRow.call(this, target, fields, frozen, index, row, frozenRight));
        table.push('</tr>');
        index++;
      }
    ...
    renderRow: function renderRow(target, fields, frozen, rowIndex, rowData, frozenRight) {
      var opts = $.data(target, 'datagrid').options;
      var cc = [];

      if (frozen && opts.rownumbers && !frozenRight) {
    ...
    refreshRow: function refreshRow(target, rowIndex) {
      this.updateRow.call(this, target, rowIndex, {});
    },
    updateRow: function updateRow(target, rowIndex, row) {
      var opts = $.data(target, 'datagrid').options;
      ...
      function _update(frozen, frozenRight) {
        ...
        var checked = false;
        var chk = tr.find('div.datagrid-cell-check .checkbox-f');

        if (chk.length > 0) {
          checked = chk.checkbox("options").checked;
        }

        tr.html(this.renderRow.call(this, target, fields, frozen, rowIndex, rowData));
        tr.find("input[type='checkbox']").checkbox();
      ...
      _update.call(this, true, false);

      _update.call(this, true, true);

      _update.call(this, false);

      $(target).datagrid('fixRowHeight', rowIndex);
    },
    insertRow: function insertRow(target, index, row) {
      var state = $.data(target, 'datagrid');
      var opts = state.options;
      var dc = state.dc;
      var data = state.data;
      ...
      function _incIndex(frozen, frozenRight) {
        var serno = frozen ? 1 : 2;
      ...
      function _insert(frozen, frozenRight) {
        var serno = frozen ? 1 : 2;
        var fields = $(target).datagrid('getColumnFields', {
          frozen: frozen,
          frozenRight: frozenRight
        ...
        var tr = '<tr id="' + rowId + '" class="datagrid-row" datagrid-row-index="' + index + '"></tr>'; //				var tr = '<tr id="' + rowId + '" class="datagrid-row" datagrid-row-index="' + index + '">' + this.renderRow.call(this, target, fields, frozen, index, row) + '</tr>';
      ...
      _incIndex.call(this, true, true);

      _incIndex.call(this, false);

      _insert.call(this, true);

      _insert.call(this, true, true);
    ...
    deleteRow: function deleteRow(target, index) {
      var state = $.data(target, 'datagrid');
      var opts = state.options;
      var data = state.data;
    ...
    onBeforeRender: function onBeforeRender(target, rows) {},
    ...
    setEmptyMsg: function setEmptyMsg(target) {
      var state = $.data(target, 'datagrid');
      var opts = state.options;
      var isEmpty = opts.finder.getRows(target).length == 0;

      if (isEmpty) {// this.renderEmptyRow(target);
      }
    ...
    renderEmptyRow: function renderEmptyRow(target) {
      var opts = $(target).datagrid('options');
      ...
      var body2 = $.data(target, 'datagrid').dc.body2; // body2.html(this.renderTable(target, 0, [{}], false));
      ...
      var emptyHeight = opts.emptyHeight;
      body2.html('<div class="empty-datagrid"><span class="empty-datagrid-desc">' + $.fn.datagrid.defaults.noData + '</span></div>');
      body2[0].style && body2[0].style.height && body2[0].style.height.split('px')[0] > 3 ? emptyHeight = body2[0].style.height.split('px')[0] : emptyHeight;
      body2.parent().prev('.datagrid-view1').removeClass('datagrid-frozen-left');
      body2.find('.empty-datagrid').css({
        height: emptyHeight - 3
      });
      var bodyView = $.data(target, 'datagrid').dc.view;

      if (bodyView.height() < emptyHeight) {
        bodyView.css({
          height: emptyHeight - 3
        });
      }
  ...
  $.fn.datagrid.defaults = $.extend({}, $.fn.panel.defaults, {
    ...
    singleSelect: false,
    ctrlSelect: false,
    // only allows multi-selection when ctrl+click is used
    selectOnCheck: true,
    checkOnSelect: true,
    pagination: false,
    pagePosition: 'bottom',
    ...
    scrollbarSize: 0,
    rownumberWidth: 30,
    // the column width of rownumbers
    // editorHeight: 24,	// the default height of editors
    editorSmallHeight: 30,
    // the small table height of editors
    editorHeight: 38,
    // the big table height of editors
    headerEvents: {
    ...
    rowStyler: function rowStyler(rowIndex, rowData) {},
    ...
    editors: editors,
    finder: {
      getTr: function getTr(target, index, type, serno) {
        type = type || 'body';
        serno = serno || 0;
        var state = $.data(target, 'datagrid');
      ...
      getRow: function getRow(target, p) {
        // p can be row index or tr object
        var index = _typeof(p) == 'object' ? p.attr('datagrid-row-index') : p;
        return $.data(target, 'datagrid').data.rows[parseInt(index)];
      },
      getRows: function getRows(target) {
    ...
    onLoadError: function onLoadError() {},
    onClickRow: function onClickRow(rowIndex, rowData) {},
    onDblClickRow: function onDblClickRow(rowIndex, rowData) {},
    onClickCell: function onClickCell(rowIndex, field, value) {},
    onDblClickCell: function onDblClickCell(rowIndex, field, value) {},
    onBeforeSortColumn: function onBeforeSortColumn(sort, order) {},
    onSortColumn: function onSortColumn(sort, order) {},
    onResizeColumn: function onResizeColumn(field, width) {},
    onBeforeSelect: function onBeforeSelect(rowIndex, rowData) {},
    onSelect: function onSelect(rowIndex, rowData) {},
    onBeforeUnselect: function onBeforeUnselect(rowIndex, rowData) {},
    onUnselect: function onUnselect(rowIndex, rowData) {},
    onSelectAll: function onSelectAll(rows) {},
    onUnselectAll: function onUnselectAll(rows) {},
    onBeforeCheck: function onBeforeCheck(rowIndex, rowData) {},
    onCheck: function onCheck(rowIndex, rowData) {},
    onBeforeUncheck: function onBeforeUncheck(rowIndex, rowData) {},
    onUncheck: function onUncheck(rowIndex, rowData) {},
    onCheckAll: function onCheckAll(rows) {},
    onUncheckAll: function onUncheckAll(rows) {},
    onBeforeEdit: function onBeforeEdit(rowIndex, rowData) {},
    onBeginEdit: function onBeginEdit(rowIndex, rowData) {},
    onEndEdit: function onEndEdit(rowIndex, rowData, changes) {},
    onAfterEdit: function onAfterEdit(rowIndex, rowData, changes) {},
    onCancelEdit: function onCancelEdit(rowIndex, rowData) {},
    onHeaderContextMenu: function onHeaderContextMenu(e, field) {},
    onRowContextMenu: function onRowContextMenu(e, rowIndex, rowData) {}
  });
})(jQuery);
...
(function ($) {
  ...
  var groupview = $.extend({}, $.fn.datagrid.defaults.view, {
    render: function render(target, container, frozen) {
      var table = [];
    ...
    renderGroup: function renderGroup(target, groupIndex, group, frozen) {
      var state = $.data(target, 'datagrid');
      var opts = state.options;
      ...
      function parseCss(css, cls) {
        ...
        return 'class="' + cls + (classValue ? ' ' + classValue : '') + '" ' + 'style="' + styleValue + '"';
      }
    },
  ...
  $.extend($.fn.datagrid.methods, {
    ...
    expandGroup: function expandGroup(jq, groupIndex) {
      return jq.each(function () {
        var opts = $(this).datagrid('options');
    ...
    collapseGroup: function collapseGroup(jq, groupIndex) {
      return jq.each(function () {
        var opts = $(this).datagrid('options');
        var view = $.data(this, 'datagrid').dc.view;
        var group = view.find(groupIndex != undefined ? 'div.datagrid-group[group-index="' + groupIndex + '"]' : 'div.datagrid-group');
    ...
    scrollToGroup: function scrollToGroup(jq, groupIndex) {
      return jq.each(function () {
        var state = $.data(this, 'datagrid');
        var dc = state.dc;
  ...
  $.extend(groupview, {
    refreshGroupTitle: function refreshGroupTitle(target, groupIndex) {
      var state = $.data(target, 'datagrid');
      var opts = state.options;
      var dc = state.dc;
    ...
    resizeGroup: function resizeGroup(target, groupIndex) {
      var state = $.data(target, 'datagrid');
      var dc = state.dc;
      var ht = dc.header2.find('table');
      var fr = ht.find('tr.datagrid-filter-row').hide(); // var ww = ht.width();
    ...
    updateRow: function updateRow(target, index, row) {
      var opts = $.data(target, 'datagrid').options;
    ...
    setGroupIndex: function setGroupIndex(target) {
      var index = 0;
  ...
  $.fn.propertygrid.defaults = $.extend({}, $.fn.datagrid.defaults, {
    ...
    groupView: groupview,
    groupField: 'group',
    groupStyler: function groupStyler(value, rows) {
      return '';
    },
...
(function ($) {
    ...
    $(target).datagrid($.extend({}, opts, {
      ...
      onSortColumn: function onSortColumn(sort, order) {
        ...
        if (opts.remoteSort) {
          ...
          _loadData(target, null, data);
        }

        opts.onSortColumn.call(target, sort, order);
      },
      onClickCell: function onClickCell(index, field) {
        opts.onClickCell.call(target, field, _find(target, index));
      },
      onDblClickCell: function onDblClickCell(index, field) {
        opts.onDblClickCell.call(target, field, _find(target, index));
      },
      onRowContextMenu: function onRowContextMenu(e, index) {
        opts.onContextMenu.call(target, e, _find(target, index));
      }
    }));
  ...
  function setRowHeight(target, idValue) {
    ...
    if (!dc.body1.is(':empty') && (!opts.nowrap || opts.autoRowHeight)) {
      if (idValue != undefined) {
        var children = _getChildren(target, idValue);
  ...
  function clickEventHandler(e) {
    ...
    if (tt.hasClass('tree-hit')) {
      _toggle(target, nodeId);
    } else if (tt.hasClass('tree-checkbox')) {
      _checkNode(target, nodeId);
    } else {
      var opts = $(target).datagrid('options');
  ...
  function _checkNode(target, idValue, checked, nocallback) {
    var state = $.data(target, 'treegrid');
    var checkedRows = state.checkedRows;
    var opts = state.options;
    ...
    if (!nocallback) {
      if (opts.onBeforeCheckNode.call(target, row, checked) == false) {
        return;
      }
    }
  ...
  function setParentCheckbox(target, row) {
    ...
    var prow = _getParent(target, row[opts.idField]);
  ...
  function adjustCheck(target, idValue) {
    ...
    if (opts.view.hasCheckbox(target, row)) {
      ...
      if (row.checkState == 'checked') {
        _checkNode(target, idValue, true, true);
      } else if (row.checkState == 'unchecked') {
        _checkNode(target, idValue, false, true);
      } else {
  ...
  function _loadData(target, parentId, data, append, nocallback) {
    var state = $.data(target, 'treegrid');
    var opts = state.options;
    var dc = state.dc;
    data = opts.loadFilter.call(target, data, parentId);

    var node = _find(target, parentId);
  ...
  function request(target, parentId, params, append, callback) {
    ...
    if (parentId == undefined && opts.queryParams) {
    ...
    var row = _find(target, parentId);

    if (opts.onBeforeLoad.call(target, row, param) == false) return; //		if (!opts.url) return;
    ...
    var result = opts.loader.call(target, param, function (data) {
      ...
      _loadData(target, parentId, data, append);
  ...
  function _getParent(target, idValue) {
    var row = _find(target, idValue);
  ...
  function _getChildren(target, parentId) {
    var data = $.data(target, 'treegrid').data;

    if (parentId) {
      var pnode = _find(target, parentId);
  ...
  function _getLevel(target, idValue) {
    // if (!idValue) return 0;
    var opts = $.data(target, 'treegrid').options;
    var tr = opts.finder.getTr(target, idValue);
  ...
  function _collapse(target, idValue) {
    var opts = $.data(target, 'treegrid').options;
  ...
  function _expand2(target, idValue) {
    var opts = $.data(target, 'treegrid').options;
    var tr = opts.finder.getTr(target, idValue);
    var hit = tr.find('span.tree-hit');
    ...
    if (subtree.length) {
      ...
      var subtree = tr.next('tr.treegrid-tr-tree');
      var cc = subtree.children('td').children('div');
      cc.hide(); //			var params = opts.queryParams || {};
  ...
  function _toggle(target, idValue) {
    var opts = $.data(target, 'treegrid').options;
    var tr = opts.finder.getTr(target, idValue);
    var hit = tr.find('span.tree-hit');

    if (hit.hasClass('tree-expanded')) {
      _collapse(target, idValue);
    } else {
      _expand2(target, idValue);
    }
  }

  function _collapseAll(target, idValue) {
    var opts = $.data(target, 'treegrid').options;

    var nodes = _getChildren(target, idValue);

    if (idValue) {
      nodes.unshift(_find(target, idValue));
    }

    for (var i = 0; i < nodes.length; i++) {
      _collapse(target, nodes[i][opts.idField]);
    }
  }

  function _expandAll(target, idValue) {
    var opts = $.data(target, 'treegrid').options;
    ...
    for (var i = 0; i < nodes.length; i++) {
      _expand2(target, nodes[i][opts.idField]);
    }
  }

  function _expandTo(target, idValue) {
    var opts = $.data(target, 'treegrid').options;
    var ids = [];

    var p = _getParent(target, idValue);
    ...
    for (var i = 0; i < ids.length; i++) {
      _expand2(target, ids[i]);
    }
  }

  function _append(target, param) {
    var state = $.data(target, 'treegrid');
    var opts = state.options;
    ...
    _loadData(target, param.parent, param.data, state.data.length > 0, true);
  }

  function _insert(target, param) {
    var ref = param.before || param.after;
    var opts = $.data(target, 'treegrid').options;
  ...
  function _remove(target, idValue) {
    var state = $.data(target, 'treegrid');
    var opts = state.options;

    var prow = _getParent(target, idValue);
  ...
  $.fn.treegrid.methods = {
    ...
    fixRowHeight: function fixRowHeight(jq, idValue) {
      return jq.each(function () {
    ...
    loadData: function loadData(jq, data) {
      return jq.each(function () {
        _loadData(this, data.parent, data);
      });
    },
    load: function load(jq, params) {
      return jq.each(function () {
    ...
    reload: function reload(jq, id) {
      return jq.each(function () {
        var opts = $(this).treegrid('options'); //				var params = typeof id == 'object' ? id : $.extend({},opts.queryParams,{id:id});
    ...
    setSelectionState: function setSelectionState(jq) {
      return jq.each(function () {
        ...
        for (var i = 0; i < state.tmpIds.length; i++) {
          _checkNode(this, state.tmpIds[i], true, true);
        }

        state.tmpIds = [];
      });
    },
    getCheckedNodes: function getCheckedNodes(jq, state) {
      state = state || 'checked';
      var rows = [];
    ...
    checkNode: function checkNode(jq, id) {
      return jq.each(function () {
        _checkNode(this, id, true);
      });
    },
    uncheckNode: function uncheckNode(jq, id) {
      return jq.each(function () {
        _checkNode(this, id, false);
      });
    },
    clearChecked: function clearChecked(jq) {
      return jq.each(function () {
        ...
        $.map($(target).treegrid('getCheckedNodes'), function (row) {
          _checkNode(target, row[opts.idField], false, true);
        });
      });
    }
  };
  ...
  var defaultView = $.extend({}, $.fn.datagrid.defaults.view, {
    ...
    renderFooter: function renderFooter(target, container, frozen) {
      var opts = $.data(target, 'treegrid').options;
    ...
    renderRow: function renderRow(target, fields, frozen, depth, row) {
      var state = $.data(target, 'treegrid');
      var opts = state.options;
      var cc = [];
    ...
    hasCheckbox: function hasCheckbox(target, row) {
      var opts = $.data(target, 'treegrid').options;
    ...
    refreshRow: function refreshRow(target, id) {
      this.updateRow.call(this, target, id, {});
    },
    updateRow: function updateRow(target, id, row) {
      var opts = $.data(target, 'treegrid').options;
    ...
    onBeforeRender: function onBeforeRender(target, parentId, data) {
      if ($.isArray(parentId)) {
        data = {
      ...
      if (data.length == undefined) {
        ...
        var setParent = function setParent(children, parentId) {
          for (var i = 0; i < children.length; i++) {
            var row = children[i];
    ...
    sort: function sort(target, data) {
      var opts = $.data(target, 'treegrid').options;
    ...
    transfer: function transfer(target, parentId, data) {
      var opts = $.data(target, 'treegrid').options;
  ...
  $.fn.treegrid.defaults = $.extend({}, $.fn.datagrid.defaults, {
    ...
    loadFilter: function loadFilter(data, parentId) {
      return data;
    },
    finder: {
      getTr: function getTr(target, id, type, serno) {
        type = type || 'body';
        serno = serno || 0;
        ...
        if (serno == 0) {
          ...
          if (type == 'body') {
            ...
            return (serno == 1 ? dc.body1 : dc.body2).find('tr.datagrid-row-checked'); //						return (serno==1?dc.body1:dc.body2).find('tr.datagrid-row:has(div.datagrid-cell-check input:checked)');
          } else if (type == 'last') {
      ...
      getRow: function getRow(target, p) {
        // p can be the row id or tr object
    ...
    onBeforeLoad: function onBeforeLoad(row, param) {},
    ...
    onClickCell: function onClickCell(field, row) {},
    onDblClickCell: function onDblClickCell(field, row) {},
    onContextMenu: function onContextMenu(e, row) {},
    onBeforeEdit: function onBeforeEdit(row) {},
    onAfterEdit: function onAfterEdit(row, changes) {},
    onCancelEdit: function onCancelEdit(row) {},
    onBeforeCheckNode: function onBeforeCheckNode(row, checked) {},
    onCheckNode: function onCheckNode(row, checked) {}
  });
})(jQuery);
...
(function ($) {
  ...
  var listview = $.extend({}, $.fn.datagrid.defaults.view, {
    ...
    groupRows: function groupRows(target, rows) {
      var state = $.data(target, 'datagrid');
      var opts = state.options;
      var groups = [];
  ...
  $.fn.datalist.parseOptions = function (target) {
    return $.extend({}, $.fn.datagrid.parseOptions(target), $.parser.parseOptions(target, ['valueField', 'textField', 'groupField', {
      checkbox: 'boolean',
      lines: 'boolean'
    }]));
  };
  ...
  $.fn.datalist.defaults = $.extend({}, $.fn.datagrid.defaults, {
    ...
    groupField: '',
    view: listview,
    textFormatter: function textFormatter(value, row) {
      return value;
    },
    groupFormatter: function groupFormatter(fvalue, rows) {
      return fvalue;
...
(function ($) {
  ...
  function buildCombo(target) {
    ...
    if (!state.panel) {
      state.panel = $('<div class="combo-panel  combo-' + opts.type + '"></div>').appendTo('body');
      state.panel.panel({
        ...
        doSize: false,
        closed: true,
        cls: 'combo-p edoc2-anim edoc2-anim-upbit',
        style: {
          position: 'absolute',
          zIndex: 10
        },
        ...
        onClose: function onClose() {
          var target = $(this).panel('options').comboTarget;
          var state = $(target).data('combo'); // var state = $.data(target, 'combo');
  ...
  function togglePanel(target) {
    ...
    if (panel.is(':visible')) {
      ...
      _hidePanel(comboTarget);

      $(target).combo('textbox').blur();
  ...
  function inputClickHandler(e) {
    var target = e.data.target;
    var state = $.data(target, 'combo');
    var opts = state.options; //if (!opts.editable){

    togglePanel(target); // } else {
    // 	var p = $(target).closest('div.combo-p').children('.combo-panel');	// the parent combo panel
    // 	// $('div.combo-panel:visible').not(panel).not(p).panel('close');
  ...
  function _showPanel(target) {
    ...
    function getLeft() {
      ...
      if (left + panel._outerWidth() > $(window)._outerWidth() + $(document).scrollLeft()) {
        left = $(window)._outerWidth() + $(document).scrollLeft() - panel._outerWidth() - 8;
      }
    ...
    function getTop() {
      if (opts.panelValign == 'top') {
        ...
        var top = combo.offset().top + combo._outerHeight() + 5;
      } else {
  ...
  function _getValues(target) {
    var state = $.data(target, 'combo');
    var opts = state.options; // var combo = state.combo;
  ...
  function _setValues(target, values) {
    var state = $.data(target, 'combo');
    var combo = state.combo; // var opts = state.options;
  ...
  function initValue(target) {
    ...
    if (opts.multiple) {
      _setValues(target, opts.value ? opts.value : []);
    } else {
      _setValue(target, opts.value); // set initialize value
  ...
  $.fn.combo.methods = {
    ...
    combo: function combo(jq) {
      if (jq.hasClass("combo-panel")) {
        return jq.panel('options').comboTarget;
      }
    ...
    setValues: function setValues(jq, values) {
      return jq.each(function () {
...
(function ($) {
  ...
  function _scrollTo(target, value) {
  ...
  function _select(target, value, remainText) {
    var opts = $.data(target, 'combobox').options;
    ...
    if ($.inArray(value + '', values) == -1) {
      ...
      _setValues2(target, values, remainText);
    }
  }
  /**
  ...
  function _unselect(target, value) {
    var opts = $.data(target, 'combobox').options;
    var values = $(target).combo('getValues');
  ...
  function _setValues2(target, values, remainText) {
    var opts = $.data(target, 'combobox').options;
    var panel = $(target).combo('panel');
    ...
    if (opts.showItemIcon) {
      ...
      tb.removeClass('textbox-bgicon fa ' + opts.textboxIconCls);

      if (theRow && theRow.iconCls) {
        tb.addClass('textbox-bgicon fa ' + theRow.iconCls);
  ...
  function _loadData(target, data, remainText) {
    var state = $.data(target, 'combobox');
    var opts = state.options;
    ...
    if (opts.multiple) {
      _setValues2(target, vv, remainText);
    } else {
      _setValues2(target, vv.length ? [vv[vv.length - 1]] : [], remainText);
    }
  ...
  function doQuery(target, q) {
    ...
    function _setValues(vv) {
      if (opts.reversed) {
        ...
        _setValues2(target, opts.multiple ? q ? vv : [] : vv, true);
      }
    }
  }
  ...
  function create(target) {
    ...
    $(target).combo($.extend({}, opts, {
      onShowPanel: function onShowPanel() {
        ...
        _setValues2(this, $(this).combobox('getValues'), true);
  ...
  $.fn.combobox = function (options, param) {
    ...
    return this.each(function () {
      ...
      if (state.options.data) {
        _loadData(this, state.options.data);
      } else {
  ...
  $.fn.combobox.methods = {
    ...
    select: function select(jq, value) {
      return jq.each(function () {
        _select(this, value);
      });
    },
    unselect: function unselect(jq, value) {
      return jq.each(function () {
        _unselect(this, value);
      });
    },
    scrollTo: function scrollTo(jq, value) {
      return jq.each(function () {
        _scrollTo(this, value);
      });
    }
  };
  ...
  $.fn.combobox.parseOptions = function (target) {
    var t = $(target);
    return $.extend({}, $.fn.combo.parseOptions(target), $.parser.parseOptions(target, ['valueField', 'textField', 'groupField', 'groupPosition', 'mode', 'method', 'url', {
  ...
  var defaultView = {
    render: function render(target, container, data) {
      var state = $.data(target, 'combobox');
      var opts = state.options;
      var prefixId = $(target).attr('id') || '';
      COMBOBOX_SERNO++;
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        if (opts.showItemIcon && row.iconCls) {
          dd.push('<i class="combobox-icon fa ' + row.iconCls + '"></i>');
        }
  ...
  $.fn.combobox.defaults = $.extend({}, $.fn.combo.defaults, {
    ...
    // or 'sticky'
    groupField: null,
    groupFormatter: function groupFormatter(group) {
      return group;
    },
    mode: 'local',
    // or 'remote'
    ...
    showItemIcon: false,
    limitToList: false,
    // limit the inputed values to the listed items
    unselectedValues: [],
    mappingRows: [],
    view: defaultView,
    keyHandler: {
    ...
    onUnselect: function onUnselect(record) {},
...
(function ($) {
  ...
  function create(target) {
    ...
    tree.tree($.extend({}, opts, {
      ...
      onLoadSuccess: function onLoadSuccess(node, data) {
        ...
        _setValues(target, values, state.remainText);
  ...
  function retrieveValues(target) {
    ...
    _setValues(target, vv, state.remainText);
  }

  function _setValues(target, values, remainText) {
    var state = $.data(target, 'combotree');
    var opts = state.options;
    var tree = state.tree;
    ...
    topts.onBeforeCheck = topts.onCheck = topts.onBeforeSelect = topts.onSelect = function () {}; // topts.onBeforeCheck = topts.onCheck = topts.onSelect = function(){};
    ...
    $.map(vv, function (v) {
      ...
      if (node) {
        tree.tree('check', node.target).tree('select', node.target); // ss.push(node.text);
    ...
    function findText(value, a) {
      var item = $.easyui.getArrayItem(a, 'id', value); // return item ? item.text : undefined;
  ...
  $.fn.combotree.methods = {
    ...
    clone: function clone(jq, container) {
...
(function ($) {
  ...
  function create(target) {
    ...
    $(target).addClass('combogrid-f').combo($.extend({}, opts, {
      onShowPanel: function onShowPanel() {
        _setValues2(this, $(this).combogrid('getValues'), true);
    ...
    function onLoadSuccess(data) {
      ...
      _setValues2(comboTarget, values, state.remainText);
    ...
    function retrieveValues() {
      ...
      vv = vv.concat(opts.unselectedValues); // don't scroll the datagrid when setting selected records

      var body2 = dg.data('datagrid').dc.body2;
      var scrollTop = body2.scrollTop();

      _setValues2(comboTarget, vv, state.remainText);
  ...
  $.fn.combogrid = function (options, param) {
    if (typeof options == 'string') {
      ...
      if (method) {
        ...
        return this.combo(options, param); //				return $.fn.combo.methods[options](this, param);
      }
    }
...
(function ($) {
  ...
  function retrieveValues(target) {
    ...
    _setValues2(target, vv);
  }

  function _setValues2(target, values) {
    var state = $.data(target, 'combotreegrid');
    var opts = state.options;
    var grid = state.grid;
    ...
    $.map(vv, function (v) {
      ...
      if (row) {
        ...
        ss.push(getText(row)); // ss.push(row[opts.treeField]);
      } else {
        ss.push(findText(v, opts.mappingRows) || v);
        opts.unselectedValues.push(v);
      }
    ...
    function findText(value, a) {
      var item = $.easyui.getArrayItem(a, opts.idField, value);
      return item ? getText(item) : undefined; // return item ? item[opts.treeField] : undefined;
    }
  ...
  function doQuery(target, q) {
    ...
    if (opts.mode == 'remote') {
      ...
      var vv = []; // var qq = opts.multiple ? q.split(opts.separator) : [q];
  ...
  $.fn.combotreegrid.parseOptions = function (target) {
    var t = $(target);
    return $.extend({}, $.fn.combo.parseOptions(target), $.fn.treegrid.parseOptions(target), $.parser.parseOptions(target, ['mode', {
      limitToGrid: 'boolean'
    }]));
  };
  ...
  $.fn.combotreegrid.defaults = $.extend({}, $.fn.combo.defaults, $.fn.treegrid.defaults, {
    ...
    singleSelect: true,
    limitToGrid: false,
    // limit the inputed values to the listed grid rows
    unselectedValues: [],
    mappingRows: [],
    mode: 'local',
    // or 'remote'
...
(function ($) {
  function create(target) {
    ...
    $(target).addClass('tagbox-f').combobox($.extend({}, opts, {
      ...
      onResizing: function onResizing(width, height) {
        ...
        input.css('margin', 0); // tb._size({width: opts.width}, $(this).parent());
    ...
    function buildTag() {
      ...
      $.map($(target).tagbox('getValues'), function (value, index) {
        ...
        $('<a href="javascript:;" class="tagbox-remove fal fa-times"></a>').appendTo(label);
      });
      resizeLabel(target);
  ...
  function doEnter(target) {
    var t = $(target);
    var opts = t.tagbox('options'); // if (!$(target).tagbox('isValid')){return;}
  ...
  $.fn.tagbox.defaults = $.extend({}, $.fn.combobox.defaults, {
    ...
    tagFormatter: function tagFormatter(value, row) {
      var opts = $(this).tagbox('options');
      return row ? row[opts.textField] : value;
    },
    tagStyler: function tagStyler(value, row) {
      return '';
    },
    onClickTag: function onClickTag(value) {},
    onBeforeRemoveTag: function onBeforeRemoveTag(value) {},
...
(function ($) {
  ...
  function createBox(target) {
    ...
    $(target).addClass('datebox-f').combo($.extend({}, opts, {
      onShowPanel: function onShowPanel() {
        ...
        _setValue(this, $(this).datebox('getText'), true);
    ...
    if (!state.calendar) {
      ...
      $.extend(state.calendar.calendar('options'), {
        ...
        onSelect: function onSelect(date) {
          ...
          _setValue(target, opts.formatter.call(target, date));
    ...
    function setButtons(target) {
      ...
      var button = $('<div class="datebox-button"><table cellspacing="0" cellpadding="0" style="position: absolute;right: 10px;"><tr></tr></table></div>').appendTo(panel);
  ...
  function _setValue(target, value, remainText) {
    var state = $.data(target, 'datebox');
    var opts = state.options;
  ...
  $.fn.datebox.defaults = $.extend({}, $.fn.combo.defaults, {
    ...
    buttons: [{
      ...
      handler: function handler(target) {
        var opts = $(target).datebox('options');
        var now = new Date();
...
(function ($) {
  function createBox(target) {
    ...
    $(target).removeClass('datebox-f').addClass('datetimebox-f'); // override the calendar onSelect event, don't close panel when selected
    ...
    if (!state.spinner) {
      ...
      var p = $('<div><input></div>').insertAfter(panel.children('div.datebox-calendar-inner'));
  ...
  $.fn.datetimebox.defaults = $.extend({}, $.fn.datebox.defaults, {
    ...
    hour12: false,
    panelEvents: {
      mousedown: function mousedown(e) {// e.preventDefault();
        // e.stopPropagation();
      }
    },
    keyHandler: {
    ...
    buttons: [{
      ...
      handler: function handler(target) {
        ...
        _setValue(target, opts.formatter.call(target, new Date()));
...
(function ($) {
  function init(target) {
    var slider = $('<div class="slider">' + '<div class="slider-inner">' + '<a href="javascript:;" class="slider-handle"></a>' + '<span class="slider-tip"></span><span class="slider-fill"></span>' + '</div>' + '<div class="slider-rule"></div>' + '<div class="slider-rulelabel"></div>' + '<div style="clear:both"></div>' + '<input type="hidden" class="slider-value">' + '</div>').insertAfter(target);
    var t = $(target);
  ...
  function showRule(target) {
    ...
    function _build(aa) {
      ...
      for (var i = 0; i < aa.length; i++) {
        ...
        span.css(opts.mode == 'h' ? 'left' : 'top', distance); // show the labels
  ...
  function buildSlider(target) {
    ...
    inner.html('<a href="javascript:;" class="slider-handle"></a>' + '<span class="slider-tip"></span><span class="slider-fill"></span>');

    if (opts.range) {
      inner.append('<a href="javascript:;" class="slider-handle"></a>' + '<span class="slider-tip"></span><span class="slider-fill"></span>');
    }
  ...
  function value2pos(target, value) {
    ...
    return pos; // return pos.toFixed(0);
  }
  /**
  ...
  function pos2value(target, pos) {
    ...
    return value; // return value.toFixed(0);
  }
  ...
  $.fn.slider.methods = {
    ...
    clear: function clear(jq) {
      return jq.each(function () {
        ...
        _setValues(this, opts.range ? [opts.min, opts.max] : [opts.min]);
      });
    },
    reset: function reset(jq) {
      return jq.each(function () {
  ...
  $.fn.slider.defaults = {
    ...
    converter: {
      toPosition: function toPosition(value, size) {
        var opts = $(this).slider('options');
    ...
    onSlideStart: function onSlideStart(value) {},
...
(function ($) {
  function buildToolbar(target) {
    ...
    var pager = $(target).addClass('pagination').addClass("pagination-size-" + opts.size).html('<table cellspacing="0" cellpadding="0" border="0"><tr></tr></table>');
    ...
    for (var index = 0; index < aa.length; index++) {
      ...
      if (item == 'list') {
        ...
        for (var i = 0; i < opts.pageList.length; i++) {
          data.push({
            text: opts.pageList[i] + $.fn.datagrid.defaults.listText,
            val: opts.pageList[i]
          });
        }

        $('<td></td>').append($("<div class='pagination-page'/>").append(ps)).appendTo(tr);
        $(ps).combobox({
          panelValign: "auto",
          height: opts.size == "big" ? 30 : 22,
          valueField: 'val',
          value: opts.pageSize,
          type: opts.size == "big" ? "small" : "lsmall",
          textField: 'text',
          panelHeight: opts.size == "big" ? 150 : 120,
          width: opts.size == "big" ? 100 : 86,
          editable: false,
          data: data,
          onChange: function onChange(newValue) {
            opts.pageSize = parseInt(newValue);
            opts.onChangePageSize.call(target, opts.pageSize);
            selectPage(target, opts.pageNumber);
          }
        });
      } else if (item == 'sep') {//$('<td><div class="pagination-btn-separator"></div></td>').appendTo(tr);
      } else if (item == 'first') {
        bb.first = createButton('first', $.fn.datagrid.defaults.firstText);
      } else if (item == 'prev') {
        bb.prev = createButton('prev', $.fn.datagrid.defaults.prevText);
      } else if (item == 'next') {
        bb.next = createButton('next', $.fn.datagrid.defaults.nextText);
      } else if (item == 'last') {
        bb.last = createButton('last', $.fn.datagrid.defaults.lastText);
      } else if (item == 'manual') {
        $('<span  class="pagination-before"></span>').html(opts.beforePageText ? opts.beforePageText : $.fn.pagination.defaults.beforePageText).appendTo(tr).wrap('<td></td>');
        var numDom = $('<input class="pagination-num" type="text" value="1" size="2"/>');
        bb.num = numDom.appendTo(tr).wrap('<td></td>');
        numDom.textbox({
          height: opts.size == "big" ? 30 : 22,
          width: 40,
          inputEvents: $.extend({}, $.fn.textbox.defaults.inputEvents, {
            ...
            focus: function focus(e) {
              $(e.target).val($(e.target).siblings('input').val());
            },
            // ggj:end
            keydown: function keydown(a, c, d) {
              if (event.keyCode == 13) {
        ...
        target.numDom = numDom; // ggj:end

        bb.after = $('<span class="pagination-after"></span>').appendTo(tr).wrap('<td></td>');
    ...
    if (opts.buttons) {
      ...
      if ($.isArray(opts.buttons)) {
        for (var i = 0; i < opts.buttons.length; i++) {
          var btn = opts.buttons[i]; // if (btn == '-') {
          // 	$('<td><div class="pagination-btn-separator"></div></td>').appendTo(tr);
          // } else {

          var td = $('<td></td>').appendTo(tr);
          var a = $('<a href="javascript:;" key="custom"></a>').appendTo(td);
    ...
    function createButton(name, text) {
      ...
      var a = $('<a href="javascript:;" key="' + name + '"></a>').appendTo(tr);
      a.wrap('<td></td>');
      var settings = {
        plain: true,
        height: opts.size == "big" ? 30 : 22
      ...
      if (!text) {
        settings.iconCls = btn.iconCls;
      } else {
        settings.text = text;
      }

      a.linkbutton(settings).unbind('.pagination').bind('click.pagination', function () {
  ...
  function refreshData(target, param) {
    ...
    var pinfo = opts.displayMsg; //只要总记录数就可以
    ...
    $(target).find('div.pagination-info').html(pinfo); //		bb.first.add(bb.prev).linkbutton({disabled: (opts.pageNumber == 1)});
    ...
    setLoadStatus(target, opts.loading); // ggj

    if (target.numDom) {
      var inputs = target.numDom.siblings('span.textbox').children('input');
      inputs.val(opts.pageNumber);
    } // ggj:end
  ...
  $.fn.pagination.methods = {
    ...
    refresh: function refresh(jq, options) {
      return jq.each(function () {
        refreshData(this, options);
      });
    },
    select: function select(jq, page) {
      return jq.each(function () {
  ...
  $.fn.pagination.defaults = {
    ...
    onBeforeRefresh: function onBeforeRefresh(pageNumber, pageSize) {},
    onRefresh: function onRefresh(pageNumber, pageSize) {},
...
(function ($) {
  $.extend($.fn.datagrid.defaults, {
    dragSelection: false,
    onBeforeDrag: function onBeforeDrag(row) {},
    // return false to deny drag
    onStartDrag: function onStartDrag(row) {},
    onStopDrag: function onStopDrag(row) {},
    onDragEnter: function onDragEnter(targetRow, sourceRow) {},
    // return false to deny drop
    onDragOver: function onDragOver(targetRow, sourceRow) {},
    // return false to deny drop
    onDragLeave: function onDragLeave(targetRow, sourceRow) {},
    onBeforeDrop: function onBeforeDrop(targetRow, sourceRow, point) {},
    onDrop: function onDrop(targetRow, sourceRow, point) {} // point:'append','top','bottom'

  });
  $.extend($.fn.datagrid.methods, {
    _appendRow: function _appendRow(jq, row) {
      return jq.each(function () {
        var dg = $(this);
    ...
    _insertRow: function _insertRow(jq, param) {
      return jq.each(function () {
        var dg = $(this);
    ...
    _deleteRow: function _deleteRow(jq, row) {
      return jq.each(function () {
        var dg = $(this);
        var rows = $.isArray(row) ? row : [row];
        $.map(rows, function (row) {
  ...
  $.extend($.fn.datagrid.methods, {
    enableDnd: function enableDnd(jq, index) {
      return jq.each(function () {
        var target = this;
        var state = $.data(this, 'datagrid');
        var dg = $(this);
        ...
        var draggableOptions = {
          ...
          onBeforeDrag: function onBeforeDrag(e) {
            ...
            $.map($.isArray(draggingRow) ? draggingRow : [draggingRow], function (row) {
              ...
              opts.finder.getTr(target, index).droppable({
                accept: 'no-accept'
              });
            });
          },
...
(function ($) {
  ...
  $.extend($.fn.treegrid.methods, {
    ...
    enableDnd: function enableDnd(jq, id) {
      if (!$('#treegrid-dnd-style').length) {
        $('head').append('<style id="treegrid-dnd-style">' + '.treegrid-row-top td{border-top:1px solid red}' + '.treegrid-row-bottom td{border-bottom:1px solid red}' + '.treegrid-row-append .tree-title{border:1px solid red}' + '</style>');
      }
      ...
      return jq.each(function () {
        ...
        function setValid(id, valid) {
          ...
          tr.next('tr.treegrid-tr-tree').find('tr[node-id]').droppable({
            accept: accept
          });
        }
      });
...
(function ($) {
  ...
  function buildBox(target) {
    ...
    cvalue.unbind('.checkbox').bind('change.checkbox', function (e) {
      return false;
    });
  ...
  function setChecked(target, checked) {
    ...
    if (checked) {
      ...
      inner.closest(".checkbox").addClass("ck-checked");
    } else {
      inner.removeClass('checkbox-checked');
      inner.closest(".checkbox").removeClass("ck-checked");
    }
  ...
  $.fn.checkbox.parseOptions = function (target) {
    var t = $(target);
    return $.extend({}, $.parser.parseOptions(target, ['label', 'labelPosition', 'labelAlign', {
      labelWidth: 'number'
    }]), {
      value: t.val() || undefined,
      checked: t.attr('checked') ? true : undefined,
...
(function ($) {
  ...
  function insertChar(target, c) {
    ...
    if (start != -1) {
      ...
      if (r.test(c)) {
        ...
        $(target).maskedbox('setSelectionRange', {
          start: start,
          end: start
        });
      }
  ...
  $.extend($.fn.textbox.methods, {
    inputMask: function inputMask(jq, param) {
      return jq.each(function () {
        var target = this;
...
(function ($) {
  ...
  function init(target) {
    var button = $('<span class="edoc2-form-radio scss-anim inputbox">' + '<span class="edoc2-anim edoc2-icon radiobutton-inner">' + "&#xe63f;" + '</span>' + '<input type="radio" class="radiobutton-value">' + '</span>').insertAfter(target);
    var t = $(target);
  ...
  function buildButton(target) {
    ...
    rvalue.unbind('.radiobutton').bind('change.radiobutton', function (e) {
      return false;
    });
  ...
  function setChecked(target, checked) {
    ...
    function _checked(b, c) {
      ...
      var input = button.find('.radiobutton-value');
      button.find('.radiobutton-inner').html(c ? '&#xe643;' : '&#xe63f;');

      input._propAttr('checked', c);

      button.removeClass("edoc2-form-radioed");
      button.find(".radiobutton-inner").removeClass("edoc2-anim-scaleSpring");

      if (c) {
        button.addClass("edoc2-form-radioed");
        button.find('.radiobutton-inner').addClass("edoc2-anim-scaleSpring");
      }
  ...
  function setReadonly(target, mode) {
    ...
    var opts = state.options;
    opts.readonly = mode == undefined ? true : mode;
    state.radiobutton.removeClass('radiobutton-readonly').addClass(opts.readonly ? 'radiobutton-readonly' : '');
  }
  ...
  function selectItem(target, itemId, triggered) {
    ...
    forTrees(target, function (t) {
      ...
      if (node) {
        ...
        selectedNode = node; // opts.onSelect.call(target, node);
      }
    });
...
(function ($) {
  ...
  var $,
    ...
    config: {
      skin: "layer-ext-edoc2"
    },
    end: {},
    btn: ['&#x786E;&#x5B9A;', '&#x53D6;&#x6D88;'],
    //原有还是保持为靠右，但宽度只占50%
    //扩展靠左按钮，默认为空，haibin.long
    btnLeft: [],
    //按钮栏自定义接口
    btnCustome: "",
    //固定内容高度
    contentHeight: 0,
    //提示的信息设置，数组：0：提示内容；1：提示信息的图片编号，暂时
    tipsInfo: ['', 0],
    //五种原始层模式
    type: ['dialog', 'page', 'iframe', 'loading', 'tips'],
    //setwin right top 坐标定位
    setwinArea: [],
    titleHidden: false
  }; //默认内置方法。
  ...
  var layer = {
    ...
    config: function config(options, fn) {
      ...
      layer.path = _ready.config.path || layer.path; //options.skin="layer-ext-edoc2";

      typeof options.extend === 'string' && (options.extend = [options.extend]); // layer.use('skin/layer.css', (options.extend && options.extend.length > 0) ? (function loop(){
    ...
    hint: function hint(options) {
      ...
      this.msg('', {
        type: 1,
        tipsInfo: [options.info, options.type],
        time: options.time,
        maxWidth: options.maxWidth || 360,
        offset: options.offset || '60px'
      });
    },
  ...
  Class.pt = Class.prototype; //缓存常用字符
  ...
  doms["extends"] = ['layui-layer-btnleft', 'layui-layer-btnright', 'layui-layer-btnleft0', 'layui-layer-loading-imporation']; //edoc2 extend
  //缓存按照图标

  doms.btnImgs = ['layui-layer-faceimg', 'layui-layer-linkauthimg']; //缓存Tips的图标信息

  doms.tipsImgs = ['', 'layui-layer-tip01', 'layui-layer-tip02', 'layui-layer-tip03', 'layui-layer-tip04']; //默认配置
  ...
  Class.pt.config = {
    ...
    tips: 2,
    moveType: 1
  }; //在type为1的时候，设置内容为空的默认值

  Class.pt.getContent = function () {
    var that = this;
    var config = that.config;
    if (_typeof(config.tipsInfo) == ( true ? "undefined" : undefined)) return config.content || '';
    var emptyTxt = '';

    if (config.tipsInfo[0].length > 0) {
      if (config.area[0].length > 0) {
        //tipInfo 垂直居中提示
        emptyTxt = '<div class="layui-layer-tips" ) >' + (config.tipsInfo[1] == 0 ? '' : '<span class="' + doms.tipsImgs[config.tipsInfo[1]] + '" ></span> ') + '<span>' + config.tipsInfo[0] + '</span></div>';
      } else {
        //tipInfo 小面板提示
        emptyTxt = '<span class="' + doms.tipsImgs[config.tipsInfo[1]] + '" >' + config.tipsInfo[0] + '</span> ';
      }
    }

    return config.content || emptyTxt;
  }; //容器
  ...
  Class.pt.vessel = function (conType, callback) {
    ...
    '<div class="' + doms[0] + ' ' + (doms.anim[config.shift] || '') + (' layui-layer-' + _ready.type[config.type]) + ((config.type == 0 || config.type == 2) && !config.shade ? ' layui-layer-border' : '') + ' ' + (config.skin || '') + '" id="' + doms[0] + times + '" type="' + _ready.type[config.type] + '" times="' + times + '" showtime="' + config.time + '" conType="' + (conType ? 'object' : 'string') + '" style="z-index: ' + zIndex + '; width:' + config.area[0] + ';height:' + config.area[1] + (config.fix ? '' : ';position:absolute;') + '">' + (config.titleHidden || conType && config.type != 2 ? '' : titleHTML) + '<div class="layui-layer-content' + (config.type == 4 ? ' layui-layer-fourpadding' : '') + (config.type == 0 && config.icon !== -1 ? ' layui-layer-padding' : '') + (config.type == 3 ? ' layui-layer-loading' + config.icon + ' ' + doms["extends"][3] : '') + '" ' + (config.contentHeight > 0 ? '" style="height:' + config.contentHeight + 'px; overflow-y:auto;"' : '') + ' >' + (config.type == 0 && config.icon !== -1 ? '<i class="layui-layer-ico layui-layer-ico' + config.icon + '"></i>' : '') + (config.type == 1 && conType ? '' : that.getContent()) + '</div>' + '<span class="layui-layer-setwin" ' + function () {
      // Modify:2015-12-29 haibin.long 增加自定义setwin层坐标
      if (_typeof(config.setwinArea) == ( true ? "undefined" : undefined) || config.setwinArea.length == 0) {
        return '';
      } else {
        return 'style="right:' + config.setwinArea[0] + 'px; top:' + config.setwinArea[1] + 'px; "';
      }
    }() + ' >' + function () {
      var closebtn = ismax ? '<a class="layui-layer-min" href="javascript:;"><cite></cite></a><a class="layui-layer-ico layui-layer-max edoc2-icon edoc2-icon-close" href="javascript:;"></a>' : '';
      config.closeBtn && (closebtn += '<a class="layui-layer-ico edoc2-icon edoc2-icon-close ' + doms[7] + ' ' + doms[7] + (config.title ? config.closeBtn : config.type == 4 ? '1' : '2') + '" href="javascript:;"></a>');
      return closebtn;
    }() + '</span>' + (_typeof(config.btnCustome) === ( true ? "undefined" : undefined) || config.btnCustome.length == 0 ? that.createBtn(config) : '<div class="' + doms[6] + '">' + config.btnCustome + '</div>') + '</div>'], titleHTML);
    return that;
  }; //包裹btn haibin.long


  Class.pt.createBtn = function (config) {
    return config.btnLeft ? function () {
      var button = '';
      typeof config.btnLeft === 'string' && (config.btnLeft = [config.btnLeft]);

      for (var i = 0, len = config.btnLeft.length; i < len; ++i) {
        button += '<a class="' + doms["extends"][0] + i + ' ' + doms.btnImgs[config.btnLeft[i]] + '"></a>';
      }

      button = '<div class="' + doms["extends"][0] + '">' + button + '</div>';
      var rightBtn = "";
      typeof config.btn === 'string' && (config.btn = [config.btn]);

      for (var i = 0, len = config.btn.length; i < len; ++i) {
        rightBtn += '<a class="' + doms[6] + i + '">' + config.btn[i] + '</a>';
      }

      rightBtn = '<div class="' + doms["extends"][1] + '">' + rightBtn + '</div>';
      return '<div class="' + doms[6] + ' clear ">' + button + rightBtn + '</div>';
    }() : function () {
  ...
  Class.pt.creat = function () {
    ...
    var content = config.content,
        conType = _typeof(content) === 'object';
    ...
    switch (config.type) {
      ...
      case 2:
        ...
        config.content = '<iframe ' + (config.contentHeight > 0 ? 'height="' + config.contentHeight + '"' : '') //iframe height
        + ' scrolling="' + (config.content[1] || 'auto') + '" allowtransparency="true" id="' + doms[4] + '' + times + '" name="' + doms[4] + '' + times + '" onload="this.className=\'\';" class="layui-layer-load" frameborder="0" src="' + config.content[0] + '"></iframe>';
        break;
  ...
  Class.pt.auto = function (index) {
    ...
    function setHeight(elem) {
      if (config.contentHeight > 0) // haibin.long 2015-12-02
        return;
      ...
      var timeout = setTimeout(function () {
        ...
        var padtop = parseFloat(elem.css("paddingTop")) || 0,
            padbottom = parseFloat(elem.css("paddingBottom")) || 0;
        var pad = padtop + padbottom;
        elem.height(area[1] - titHeight - btnHeight - pad);
        clearTimeout(timeout);
      }, 200);
    }
  ...
  Class.pt.callback = function () {
    ...
    layero.find('.' + doms[7]).on('click', cancel); //点遮罩关闭
  ...
  Class.pt.IE6 = function (layero) {
    var that = this,
        _ieTop = layero.offset().top; //ie6的固定与相对定位
  ...
  function getContent(html) {
    ...
    if (html > 0) {
      //tipInfo 垂直居中提示
      emptyTxt = '<div class="layui-layer-tips" ) >' + (config.tipsInfo[1] == 0 ? '' : '<span class="' + doms.tipsImgs[1] + '" ></span> ') + '<span>' + config.tipsInfo[0] + '</span></div>';
    }
  ...
  layer.setContent = function (html, index, empInfo) {
    if (_typeof(empInfo) == ( true ? "undefined" : undefined)) empInfo = '';
    var empHtml = '<span class="layui-layer-tip01" >' + empInfo + '</span>';
    $('#' + doms[0] + (index || layer.index) + ' .' + doms[5]).html(html.length > 0 ? html : empHtml);
  }; //获取子iframe的DOM
  ...
  $.layer = (_$$layer = {
    setContent: function setContent(html, index, empInfo) {
      return layer.setContent(html, index, empInfo);
    },
    getChildFrame: function getChildFrame(selector, index) {
      return layer.getChildFrame(selector, index);
    },
    getFrameIndex: function getFrameIndex(name) {
      return layer.getFrameIndex(name);
    },
    iframeAuto: function iframeAuto(index) {
      return layer.iframeAuto(index);
    },
    iframeSrc: function iframeSrc(index, url) {
      return layer.iframeSrc(index, url);
    },
    photos: function photos(options, loop, key) {
      return layer.photos(options, loop, key);
    },
    prompt: function prompt(options, yes) {
    ...
    config: function config(options) {
      debugger;
      return layer.config(options);
    },
    ready: function ready(path, callback) {
      return layer.ready(path, callback);
    },
    ...
    style: function style(index, cssStyle) {
      return layer.style(index, cssStyle);
    },
    title: function title(_title, index) {
      return layer.title(_title, index);
    }
  }, _defineProperty(_$$layer, "getChildFrame", function getChildFrame(selector, index) {
    return layer.getChildFrame(selector, index);
  }), _defineProperty(_$$layer, "getFrameIndex", function getFrameIndex(windowName) {
    return layer.getFrameIndex(windowName);
  }), _defineProperty(_$$layer, "iframeAuto", function iframeAuto(index) {
    return layer.iframeAuto(index);
  }), _defineProperty(_$$layer, "iframeSrc", function iframeSrc(index, url) {
    return layer.iframeSrc(index, url);
  }), _defineProperty(_$$layer, "setTop", function setTop(layero) {
    return layer.setTop(layero);
  }), _defineProperty(_$$layer, "full", function full() {
    return layer.full(layero);
  }), _defineProperty(_$$layer, "min", function min() {
    return layer.min(layero);
  }), _defineProperty(_$$layer, "restore", function restore() {
    return layer.restore();
  }), _defineProperty(_$$layer, "prompt", function prompt(options, yes) {
    return layer.prompt(options, yes);
  }), _defineProperty(_$$layer, "tab", function tab(options) {
    return layer.tab(options);
  }), _defineProperty(_$$layer, "photos", function photos(options) {
    return layer.photos(options);
  }), _$$layer);
  $.layer.defaults = {// type: 0,
    // title: '&#x4FE1;&#x606F;',
    // content:'',
    // skin:'',
    // area:'auto',
...
(function ($) {
  ...
  function Menu(target) {
    this.settings = {
      filter: "inbizadmin-system-side-menu",
      THIS: 'inbiz-this',
      MOD_NAME: "element",
      SHOW: "inbiz-show",
      NAV_ELEM: '.inbiz-nav',
      NAV_ITEM: 'inbiz-nav-item',
      NAV_BAR: 'inbiz-nav-bar',
      NAV_TREE: 'inbiz-nav-tree',
      NAV_CHILD: 'inbiz-nav-child',
      NAV_MORE: 'inbiz-nav-more',
      NAV_ANIM: 'inbiz-anim inbiz-anim-upbit',
      NAV_FIRST_MORE: '.first-menu-more',
      NAV_PLAIN: 'inbiz-nav-plain',
      NAV_PANEL: 'inbiz-nav-panel',
      target: target,
      ie: function () {
        //ie版本
  ...
  function createFirst(target, options, isSide) {
    var firstHtmls = "";

    if (options && options.menus && options.menus.length > 0) {
      var navClass = "";

      if (isSide) {
        navClass = "inbiz-nav-tree";
      }

      if (!isSide && options.MenuType === 'plain') {
        navClass = "inbiz-nav-plain";
      } else if (!isSide && options.MenuType === 'panel') {
        navClass = "inbiz-nav-panel";
      } else if (!isSide && options.MenuType === 'default') {
        navClass = "inbiz-nav-default";
      }

      firstHtmls += '<ul class="inbiz-nav ' + navClass + '">';
      $.each(options.menus, function (i, item) {
        firstHtmls += '<li level="1" class="inbiz-nav-item navItem';

        if (item.selected) {
          if (item.childrenCount > 0) {
            firstHtmls += ' inbiz-nav-itemed"';
          } else {
            firstHtmls += ' inbiz-this"';
          }
        } else {
          firstHtmls += '"';
        }

        firstHtmls += " count='" + item.childrenCount + "' ";

        if (item.id) {
          firstHtmls += " menuid='" + item.id + "' ";
        }

        firstHtmls += '">';
        firstHtmls += '<a ';

        if (isSide && item.childrenCount > 0) {
          item.url = "javascript:void(0);";
        }

        firstHtmls += " inbiz-tips='" + item.name + "'";

        if (item.url) {
          firstHtmls += ' href="' + item.url + '"';
        }

        if (item.target && item.childrenCount == 0) {
          firstHtmls += ' target="' + item.target + '"';
        }

        firstHtmls += '>';
        firstHtmls = addIconAndColor(item.icon, firstHtmls);
        firstHtmls += '<cite>' + item.name + '</cite>';

        if (item.childrenCount > 0) {
          firstHtmls += '<span class="inbiz-nav-more"></span>';
        }

        firstHtmls += '</a>';

        if (item.childrenCount > 0 && item.childrens && item.childrens.length > 0) {
          firstHtmls += createChilds(null, item.childrens, 2, isSide, true);
        }
      ...
      if (!isSide) {
        firstHtmls += ' <li class="inbiz-nav-item navItem first-menu-more"> <span> ... </span><ul id="menuOverflow"></ul> </li>';
      }

      firstHtmls += '</ul>';
      $(target).find(".nav").append(firstHtmls);
    }
  }

  function addIconAndColor(icon, html) {
    if (icon) {
      var color = "";

      if (icon.indexOf("#") > -1) {
        //处理图标选择框带的color
        var temp = icon.substring(icon.indexOf("#"));

        if (temp.indexOf(" ") > -1) {
          temp = icon.substring(0, icon.indexOf(" "));
        }
      ...
      html += '<i class="' + icon + '" style="' + color + '"></i>';
    }
  ...
  function createChilds(target, menus, level, isSide, def) {
    var childsHtmls = "";
    ...
    if (menus && menus.length > 0) {
      ...
      //    isShow=" inbiz-show";
      // }
      childsHtmls += '<dl class="inbiz-nav-child inbiz-anim inbiz-anim-upbit" level="' + level + '">';
      $.each(menus, function (i, item) {
        childsHtmls += '<dd level="' + level + '" class="navItem';

        if (item.selected) {
          if (item.childrenCount > 0) {
            childsHtmls += ' inbiz-nav-itemed"';
          } else {
            childsHtmls += ' inbiz-this"';
          }
        } else {
          childsHtmls += '"';
        }

        childsHtmls += " count='" + item.childrenCount + "' ";

        if (item.id) {
          childsHtmls += " menuid='" + item.id + "' ";
        }
        ...
        if (item.url) {
          childsHtmls += ' href="' + item.url + '"';
        }

        if (item.target && item.childrenCount == 0) {
          childsHtmls += ' target="' + item.target + '"';
        }

        childsHtmls += '>';
        childsHtmls = addIconAndColor(item.icon, childsHtmls);
        childsHtmls += '<cite>' + item.name + '</cite>';

        if (item.childrenCount > 0) {
          childsHtmls += '<span class="inbiz-nav-more"></span>';
        }

        childsHtmls += '</a>';

        if (item.childrenCount > 0 && item.childrens && item.childrens.length > 0) {
          childsHtmls += createChilds(null, item.childrens, level + 1, isSide, true);
        }

        childsHtmls += '</dd>';
      });
      childsHtmls += '</dl>'; //如果是默认创建2级菜单，则不追加到dom

      if (!def) {
        $(target).append(childsHtmls);
      }
    }
  ...
  function shrink(sk, opts, that) {
    var isShrink = false;

    if (sk.hasClass("edoc2-icon-spread-left")) {
      sk.removeClass("edoc2-icon-spread-left").addClass("edoc2-icon-shrink-right");
      $("body").removeClass("inbiz-side-shrink");
    } else {
      sk.removeClass("edoc2-icon-shrink-right").addClass("edoc2-icon-spread-left");
      $("body").addClass("inbiz-side-shrink");
      isShrink = true;
    }

    if (opts.onShrink) {
      opts.onShrink(isShrink, that);
    }
  }
  ...
  Menu.prototype.init = function () {
    ...
    this.timerMore = {};
    this.timeEnd = {};
    that = this; //套个闭包，多个菜单同时用会有问题

    (function (that) {
      var opts = $.data(that.settings.target, 'navigation').options;

      if (opts.flexible) {
        if (opts.flexible.seletor) {
          var sk = $(opts.flexible.seletor);
          sk.addClass("edoc2-icon");

          if (opts.flexible.shrink) {
            sk.addClass("edoc2-icon-spread-left");
            $("body").addClass("inbiz-side-shrink");
          } else {
            sk.addClass("edoc2-icon-shrink-right");
          }

          sk.off("click").on("click", function () {
            shrink(sk, opts, that);
          });
        }
      }

      var isSide = $(that.settings.target).hasClass("inbiz-side");
      createFirst(that.settings.target, opts, isSide);

      if (!isSide) {
        setTimeout(function () {
          navigationResize(that.settings.target);

          if ($(that.settings.target).hasClass("inbiz-header")) {
            $('.inbiz-header').css({
              overflow: 'visible'
            });
          }
        }, 0);
        currentMenu = that.settings.target;
      }

      var elem = $(that.settings.NAV_ELEM, that.settings.target),
          bar = $('<span class="' + that.settings.NAV_BAR + '"></span>');

      if (this.target) {
        elem = $(this.target).find(that.settings.NAV_ELEM);
      }

      elem.each(function (index) {
        var othis = $(this),
            itemElem = othis.find('.' + that.settings.NAV_ITEM);
        var menuid = othis.attr("menuid"); //Hover滑动效果

        if (!othis.find('.' + that.settings.NAV_BAR)[0]) {
          othis.append(bar);
          itemElem.on('mouseenter', function (event) {
            var showMoreFirstMenu = $(this).parent('#menuOverflow').length;
            $('.vertical-menu .' + that.settings.NAV_BAR).show();
            that.follow.call(this, bar, othis, index, that, showMoreFirstMenu);
            event.stopPropagation();
          }).on('mouseleave', function (event) {
            $('.vertical-menu .' + that.settings.NAV_BAR).hide();

            (function (othis, that) {
              if (!othis.hasClass(that.settings.NAV_TREE)) {
                clearTimeout(that.timerMore[index]);
                that.timerMore[index] = setTimeout(function () {
                  othis.find('.' + that.settings.NAV_CHILD).removeClass(that.settings.SHOW);
                  othis.find('.' + that.settings.NAV_MORE).removeClass(that.settings.NAV_MORE + 'd');
                }, that.TIME);
              }
            })(othis, that);
          ...
          othis.on('mouseleave', function () {
            clearTimeout(that.timer[index]);
            that.timeEnd[index] = setTimeout(function () {
              if (othis.hasClass(that.settings.NAV_TREE)) {
                bar.css({
                  height: 0,
        ...
        itemElem.find('a').each(function () {
          var thisA = $(this),
              parent = thisA.parent(),
              child = thisA.siblings('.' + that.settings.NAV_CHILD); //输出小箭头

          if (child[0] && !thisA.children('.' + that.settings.NAV_MORE)[0]) {
            thisA.append('<span class="' + that.settings.NAV_MORE + '"></span>');
          }

          thisA.off('click').on('click', function (event) {
            var callData = {
              menuid: parent.attr("menuid"),
              count: parent.attr("count"),
              target: parent
            };
            that.clickItem.call(this, that, callData);
            event.stopPropagation();
          }); //点击菜单
        });
      }); //tip初始化
      ...
      if (isSide) {
        $(that.settings.target).find("*[inbiz-tips]").on('mouseenter', function () {
          var othis = $(this);
          if (othis.parent().hasClass('inbiz-nav-item') && othis.closest(".inbiz-side-shrink").length == 0) return;
          var tips = othis.attr('inbiz-tips'),
              offset = othis.attr('inbiz-offset'),
      ...
      $(that.settings.target).on("mouseenter", "li .navItem", function (event) {
        var showMoreFirstMenu = $(this).parents('#menuOverflow').length; //进入当前的时候，清除下未清干净的

        var othis = $(this);
        othis.parent().find("dl[level='3']").removeClass(that.settings.SHOW);
        that.follow.call(this, null, null, 0, that, showMoreFirstMenu);
        event.stopPropagation();
      }).on('mouseleave', "li .navItem", function (event) {
        var othis = $(this);
        var index = $(this).closest(that.settings.NAV_ELEM).index();
      ...
      $(that.settings.target).off("click").on("click", "li .navItem", function (event) {
        var callData = {
  ...
  Menu.prototype.clickItem = function (that, callData) {
    var opts = $.data(that.settings.target, 'navigation').options;
    var othis = $(callData.target),
        parents = othis.parents(that.settings.NAV_ELEM),
        parent = othis.parent(),
        child = othis.siblings('.' + that.settings.NAV_CHILD),
        unselect = typeof parent.attr('lay-unselect') === 'string',
        isSide = $(that.settings.target).hasClass("inbiz-side"); // 当传了这个事件后，返回true才执行内部方法

    var showMore;

    if (parents.hasClass(that.settings.NAV_TREE)) {
      showMore = function showMore() {
        if ($("body").hasClass("inbiz-side-shrink")) {
          if (parseInt($(othis).attr("count")) > 0) {
            shrink($(opts.flexible.seletor), opts, that);

            if ($(othis).hasClass("inbiz-nav-itemed")) {
              return;
            }
          }
        }

        child = $(othis).find('.' + that.settings.NAV_CHILD);

        if (!(othis.attr('href') !== 'javascript:;' && othis.attr('target') === '_blank') && !unselect) {
          if (!child[0]) {
            parents.find('.' + that.settings.THIS).removeClass(that.settings.THIS);
            othis.addClass(that.settings.THIS);
            $('.vertical-menu .' + that.settings.NAV_BAR).hide();
          }
        }

        child.removeClass(that.settings.NAV_ANIM); //如果有子菜单，则展开

        if (child[0]) {
          othis[child.css('display') === 'none' ? 'addClass' : 'removeClass'](that.settings.NAV_ITEM + 'ed');

          if ($(that.settings.target).attr('lay-shrink') === 'all') {
            othis.siblings().removeClass(that.settings.NAV_ITEM + 'ed');
          }
        }
      };
    } else {
      showMore = function showMore() {
        if (!child[0]) {
          var count = parseInt(othis.attr("count"));

          if (count == 0 || othis.attr("showChild") == "0") {
            parents.find('.' + that.settings.THIS).removeClass(that.settings.THIS);
            parents.find('.inbiz-nav-itemed').removeClass("inbiz-nav-itemed");
            parents.find('.inbiz-nav-manymore').removeClass("inbiz-nav-manymore");

            if (othis.attr("level") == "1") {
              othis.addClass("inbiz-nav-itemed");
            } else {
              if (count == 0) {
                othis.addClass(that.settings.THIS);
              }
            ...
            var items = $(othis).parents(".navItem");
            $.each(items, function (i, item) {
              $(item).addClass("inbiz-nav-itemed");

              if ($(item).attr("count") > 0) {
                $(item).find(".inbiz-nav-more:eq(0)").addClass("inbiz-nav-manymore");
              }
            });
          }
        }
    ...
    if (opts.onClickItem && !opts.onClickItem(othis, that)) {
      showMore();
      return;
    } //如果是垂直菜单
    ...
    if (parents.hasClass(that.settings.NAV_TREE)) {
      var count = callData.count;
      var level = $(othis).attr("level");

      if ($(othis).find("dl[level]").length == 0 && count > 0) {
        if (opts.onLoadData) {
          opts.onLoadData(callData).then(function (data) {
            createChilds(othis, data, parseInt(level) + 1, isSide);
            showMore();
          });
        }
      } else {
  ...
  Menu.prototype.follow = function (bar, nav, index, that, dropDownMenu) {
    var itemDom = this;
    var isSide = $(that.settings.target).hasClass("inbiz-side");
    var opts = $.data(that.settings.target, 'navigation').options;
    var othis = $(this),
        child = $(othis.children('.' + that.settings.NAV_CHILD).get(0));
    ...
    if (isSide) {
      ...
      if (dropDownMenu != 1) {
        child.addClass(that.settings.NAV_ANIM);
        bar && bar.css({
          left: othis.position().left + parseFloat(othis.css('marginLeft')),
          top: othis.position().top + othis.height() - bar.height() - 2
        });
        ...
        }, that.settings.ie && that.settings.ie < 10 ? 0 : that.TIME);
      }

      clearTimeout(that.timeEnd[index]);
      ...
      var showMore = function showMore() {
        var level = parseInt($(othis).attr("level"));
        var plainStyle = $(othis).parents(that.settings.NAV_ELEM).hasClass(that.settings.NAV_PLAIN);
        var panelStyle = $(othis).parents(that.settings.NAV_ELEM).hasClass(that.settings.NAV_PANEL);
        var leftDistance = $(othis).offset().left;
        var windowWidth = $(window).width();

        if (plainStyle) {
          // 抽屉
          if (level == 1 && +dropDownMenu !== 1) {
            var headerLeft = 0;
            var plainWidth;

            if ($('.inbiz-header').find('.plugin-nav-left').length > 0) {
              headerLeft = $('.inbiz-header .plugin-nav-left').offset().left;
              plainWidth = document.body.offsetWidth - 2 - headerLeft;
            } else {
              plainWidth = document.body.offsetWidth - 2;
            }

            var inbizMoban3 = $('body').hasClass('layout-default') && $('.Menu').hasClass('nav-container-fixed');
            ...
            if ($('body').hasClass('layout-left') && $('.layout-left').find('.leftMenu').length > 0) {
              headerLeft = $('body').find('.leftMenu').width();
            }

            child.css({
              left: -leftDistance + headerLeft,
              width: plainWidth,
              height: 347,
              overflowX: 'auto',
              overflowY: 'auto'
            });
            child.find('.inbiz-nav-child').show().css({
              position: 'static'
            });
            child.find('.navItem').each(function (i) {
              $(this).addClass('plain-second-menu' + parseInt($(this).attr("level")));
            });
          }
        } else if (panelStyle) {
          // 面板
          var childCount = child.find('.navItem').length;
          var twoMenuHeight = parseInt(child.parents('#menuOverflow').height());
          child.find('.navItem').each(function (i) {
            if (parseInt($(this).attr("level")) == 2 && parseInt($(this).attr("count")) > 0) {
              $(this).addClass('panel-two-level');
            }

            if (parseInt($(this).attr("level")) == 3 && parseInt($(this).attr("count")) > 0) {
              $(this).addClass('panel-three-nav');
            }
          });

          if (level == 1 && parseInt(dropDownMenu) == 1) {
            // 更多下拉菜单处理
            child.addClass('panel-two-menu');

            if (parseInt(childCount) > 0 && parseInt(childCount) < 11) {
              var rightPanelWidth = 200;
              child.css({
                left: -rightPanelWidth - 2,
                top: -(othis.position().top + 1),
                width: rightPanelWidth + 'px',
                minHeight: twoMenuHeight + 'px'
              });
            } else if (parseInt(childCount) > 10) {
              var rightPanelWidth = 600;
              child.css({
              ...
              child.find('.navItem').each(function (i) {
                if (parseInt($(this).attr("level")) == 2) {
                  $(this).addClass('three-line-menu');
                }
              });
            }
          } else if (level == 2) {
            var twoMenuHeight = parseInt(child.parents('.' + that.settings.NAV_CHILD).height());
            child.find('.' + that.settings.NAV_MORE).hide();
            child.find('.' + that.settings.NAV_CHILD).addClass('nav-panel-fourth');
            child.find('.navItem').each(function (i) {
              if (parseInt($(this).attr("level")) == 3 && parseInt($(this).attr("count")) > 0) {
                $(this).addClass('panel-three-level');
              }
            });
            ...
            if (parseInt(childCount) > 0 && parseInt(childCount) < 11) {
              ...
              if (windowWidth - leftDistance - othis.width() < rightPanelWidth) {
                child.css({
                  left: -rightPanelWidth - 2
                });
              } else {
              ...
              if (windowWidth - leftDistance - othis.width() < leftDistance) {
                child.css({
                  left: -rightPanelWidth - 2
                });
              } else {
        ...
        that.timerMore[index] = setTimeout(function () {
          if (othis.is(":hover")) {
            //避免二级菜单重叠，只显示当前hover元素的二级菜单
            child.addClass(that.settings.SHOW);
          }

          $(othis.find('.' + that.settings.NAV_MORE).get(0)).addClass(that.settings.NAV_MORE + 'd');
        }, 200);
      };

      var count = othis.attr("count");
      var level = othis.attr("level");
      var levelDom = othis.find("dl[level]");

      if (levelDom.length == 0 && count > 0) {
        if (opts.onLoadData) {
          opts.onLoadData({
            menuid: othis.attr("menuid")
          }).then(function (data) {
            if (data || data.length == 0) {
              othis.attr("showChild", "0");
            }

            createChilds(itemDom, data, parseInt(level) + 1, false);
            showMore();
          });
        }
      } else {
  ...
  function navigationResize(target) {
    $(target).find('.first-menu-more').before($(target).find('#menuOverflow>li'));
    var plainStyle = $(target).find('.inbiz-nav').hasClass('inbiz-nav-plain');
    var panelStyle = $(target).find('.inbiz-nav').hasClass('inbiz-nav-panel');
    var inbizLogoWidth = $('.inbiz-header .plugin-nav-left .nav-logo').is(':visible') ? parseInt($('.inbiz-header .plugin-nav-left .nav-logo').width()) : 0;
    var inbizShrinkWidth = $('.inbiz-header .plugin-nav-left .nav-Shrink').is(':visible') ? parseInt($('.inbiz-header .plugin-nav-left .nav-Shrink').width()) : 0;
    var inbizPersonWidth = $('.inbiz-header .nav-tool .small-person-view').is(':visible') ? parseInt($('.inbiz-header .nav-tool .small-person-view').width() + 20) : $('.inbiz-header .nav-tool .simple-person-view').is(':visible') ? parseInt($('.inbiz-header .nav-tool .simple-person-view').width() + 20) : 0;
    var inbizLangWidth = $('.inbiz-header .nav-multilanguage').is(':visible') ? parseInt($('.inbiz-header .nav-multilanguage').width() + 10) : 0;
    var navItemWidth = 120 + inbizLogoWidth + inbizShrinkWidth + inbizPersonWidth + inbizLangWidth;
    var windowWidth;

    if ($('.inbiz-header').find('.plugin-nav-left').length > 0) {
      windowWidth = $(window).width() - $('.inbiz-header .plugin-nav-left').offset().left;
      $('.plugin-nav-left .horizontal-menu').css({
        "width": $(window).width() - $('.inbiz-header .plugin-nav-left').offset().left - (inbizLogoWidth + inbizShrinkWidth + inbizPersonWidth + inbizLangWidth)
      });
    } else {
    ...
    if (inbizMoban3) {
      windowWidth = $('.nav-container-fixed .plugin-nav').width() - 60;
      $('.plugin-nav-left .horizontal-menu').css({
        "width": windowWidth - (inbizLogoWidth + inbizShrinkWidth + inbizPersonWidth + inbizLangWidth)
      });
    }

    var navItems = $(target).find('.' + that.settings.NAV_ITEM);
    navItems = navItems.not($(that.settings.NAV_FIRST_MORE));
    navItems.each(function () {
      navItemWidth += $(this).outerWidth(true);
    });
    var firstMore = $(target).find(that.settings.NAV_FIRST_MORE);
    navItemWidth > windowWidth ? firstMore.show() : firstMore.hide();
    ...
    if (plainStyle) {
      // 抽屉菜单
      var leftDistance = firstMore.offset().left;
      var headerLeft = 0;
      var plainWidth;
      ...
      var overflow = firstMore.children('#menuOverflow');
      overflow.css({
        left: -leftDistance + headerLeft,
        width: plainWidth,
        height: 347,
      ...
      overflow.find('.navItem').each(function (i) {
        $(this).addClass('plain-overflow-menu' + parseInt($(this).attr("level")));
      });
    } else if (panelStyle) {
      // 面板菜单
      var overflow = firstMore.children('#menuOverflow');
      overflow.find('.navItem').each(function (i) {
        $(this).addClass('panel-overflow-menu' + parseInt($(this).attr("level")));
      });
    }
  }
  ...
  $.fn.navigation.defaults = {
    ...
    menus: [] //竖向菜单收缩选择器
    // menus:[
    // {id:parseInt(Math.random()*10000000),name:"家用电器", url:"", selected:false,childrenCount:12,target:"_blank",childrens:[
    //   {id:parseInt(Math.random()*10000000),name:"名字超长的样子二级菜单冰箱不都顶顶顶", url:"", selected:true,childrenCount:8,target:"_blank",
    //   childrens:[
    //     {id:parseInt(Math.random()*10000000),name:"三级菜单定频名字过长很长的样子", url:"#784444", selected:false,childrenCount:0},
    //     {id:parseInt(Math.random()*10000000),name:"定频三级菜单", url:"#784444", selected:false,childrenCount:0},
    //     {id:parseInt(Math.random()*10000000),name:"定三级菜单频", url:"#784444", selected:false,childrenCount:0},
    //     {id:parseInt(Math.random()*10000000),name:"shuangmen三级菜单定频名字过长很长的样子", url:"#123456", selected:true,childrenCount:6,childrens:[
    //       {id:parseInt(Math.random()*10000000),name:"四级菜单名字过长ceshi测试顶顶顶直冷-1", url:"#789456", selected:false,childrenCount:0},
    //       {id:parseInt(Math.random()*10000000),name:"直冷-2", url:"#45258", selected:false,childrenCount:0},
    //       {id:parseInt(Math.random()*10000000),name:"直冷-3", url:"#78951", selected:false,childrenCount:0},
    //       {id:parseInt(Math.random()*10000000),name:"直冷-4", url:"#111555", selected:false,childrenCount:0},
    //       {id:parseInt(Math.random()*10000000),name:"直冷-5", url:"#888663", selected:false,childrenCount:0},
    //       {id:parseInt(Math.random()*10000000),name:"风冷（无霜）", url:"", selected:false,childrenCount:0,target:"_blank"}
    //     ]},
    //     {id:parseInt(Math.random()*10000000),name:"对开门", url:"#2222222222", selected:false,childrenCount:2,childrens:[
    //       {id:parseInt(Math.random()*10000000),name:"定频", url:"#784444", selected:false,childrenCount:0},
    //       {id:parseInt(Math.random()*10000000),name:"定频ddd", url:"", selected:false,childrenCount:0,target:"_blank"}
    //     ]},
    //     {id:parseInt(Math.random()*10000000),name:"三门", url:"#2222222222", selected:false,childrenCount:2,childrens:[
    //       {id:parseInt(Math.random()*10000000),name:"百度4-1", url:"#777777777", selected:false,childrenCount:0},
    //       {id:parseInt(Math.random()*10000000),name:"百度4-2", url:"", selected:false,childrenCount:0,target:"_blank"}
    //     ]},
    //     {id:parseInt(Math.random()*10000000),name:"多门", url:"#2222222222", selected:false,childrenCount:2,childrens:[
    //       {id:parseInt(Math.random()*10000000),name:"百度4-1", url:"#777777777", selected:false,childrenCount:0},
    //       {id:parseInt(Math.random()*10000000),name:"百度4-2", url:"", selected:false,childrenCount:0,target:"_blank"}
    //     ]},
    //     {id:parseInt(Math.random()*10000000),name:"单门", url:"", selected:false,childrenCount:2,target:"_blank",childrens:[
    //       {id:parseInt(Math.random()*10000000),name:"百度4-1", url:"#777777777", selected:false,childrenCount:0},
    //       {id:parseInt(Math.random()*10000000),name:"百度4-2", url:"", selected:false,childrenCount:0,target:"_blank"}
    //     ]}
    //   ]},
    //   {id:parseInt(Math.random()*10000000),name:"冰箱", url:"", selected:false,childrenCount:2,target:"_blank",
    //   childrens:[
    //     {id:parseInt(Math.random()*10000000),name:"多门", url:"#2222222222", selected:false,childrenCount:2,childrens:[
    //       {id:parseInt(Math.random()*10000000),name:"百度4-1", url:"#777777777", selected:false,childrenCount:0},
    //       {id:parseInt(Math.random()*10000000),name:"百度4-2", url:"", selected:false,childrenCount:0,target:"_blank"}
    //     ]},
    //     {id:parseInt(Math.random()*10000000),name:"三门", url:"", selected:false,childrenCount:2,target:"_blank",childrens:[
    //       {id:parseInt(Math.random()*10000000),name:"百度4-1", url:"#777777777", selected:false,childrenCount:0},
    //       {id:parseInt(Math.random()*10000000),name:"百度4-2", url:"", selected:false,childrenCount:0,target:"_blank"}
    //     ]}
    //   ]},
    ...
    //     ]}
    //   ]},
    //   {id:parseInt(Math.random()*10000000),name:"个护健康", url:"", selected:false,childrenCount:2,target:"_blank",
    //   childrens:[
    //     {id:parseInt(Math.random()*10000000),name:"剃须刀", url:"#2222222222", selected:false,childrenCount:2,childrens:[
    //       {id:parseInt(Math.random()*10000000),name:"百度4-1", url:"#777777777", selected:false,childrenCount:0},
    //       {id:parseInt(Math.random()*10000000),name:"百度4-2", url:"", selected:false,childrenCount:0,target:"_blank"}
    //     ]},
    //     {id:parseInt(Math.random()*10000000),name:"电动牙刷", url:"", selected:false,childrenCount:2,target:"_blank",childrens:[
    //       {id:parseInt(Math.random()*10000000),name:"百度4-1", url:"#777777777", selected:false,childrenCount:0},
    //       {id:parseInt(Math.random()*10000000),name:"百度4-2", url:"", selected:false,childrenCount:0,target:"_blank"}
    //     ]}
    //   ]},
    //   {id:parseInt(Math.random()*10000000),name:"电视", url:"", selected:false,childrenCount:2,target:"_blank",
    //   childrens:[
    //     {id:parseInt(Math.random()*10000000),name:"超薄电视", url:"#2222222222", selected:false,childrenCount:2,childrens:[
    //       {id:parseInt(Math.random()*10000000),name:"百度4-1", url:"#777777777", selected:false,childrenCount:0},
    //       {id:parseInt(Math.random()*10000000),name:"百度4-2", url:"", selected:false,childrenCount:0,target:"_blank"}
    //     ]},
    //     {id:parseInt(Math.random()*10000000),name:"全面屏电视", url:"", selected:false,childrenCount:2,target:"_blank",childrens:[
    //       {id:parseInt(Math.random()*10000000),name:"百度4-1", url:"#777777777", selected:false,childrenCount:0},
    //       {id:parseInt(Math.random()*10000000),name:"百度4-2", url:"", selected:false,childrenCount:0,target:"_blank"}
    //     ]}
    //   ]},
    //   {id:parseInt(Math.random()*10000000),name:"空调", url:"", selected:false,childrenCount:2,target:"_blank", childrens:[
    //     {id:parseInt(Math.random()*10000000),name:"空调挂机", url:"#2222222222", selected:false,childrenCount:0},
    //     {id:parseInt(Math.random()*10000000),name:"中央空调", url:"", selected:false,childrenCount:2,target:"_blank",childrens:[
    //       {id:parseInt(Math.random()*10000000),name:"百度4-1", url:"#777777777", selected:false,childrenCount:0},
    //       {id:parseInt(Math.random()*10000000),name:"百度4-2", url:"", selected:false,childrenCount:0,target:"_blank"}
    //     ]}
    //   ]},
    //   {id:parseInt(Math.random()*10000000),name:"洗衣机", url:"", selected:false,childrenCount:2,target:"_blank",
    //   childrens:[
    //     {id:parseInt(Math.random()*10000000),name:"迷你洗衣机", url:"#2222222222", selected:false,childrenCount:2,childrens:[
    //       {id:parseInt(Math.random()*10000000),name:"百度4-1", url:"#777777777", selected:false,childrenCount:0},
    //       {id:parseInt(Math.random()*10000000),name:"百度4-2", url:"", selected:false,childrenCount:0,target:"_blank"}
    //     ]},
    //     {id:parseInt(Math.random()*10000000),name:"滚筒洗衣机", url:"", selected:false,childrenCount:2,target:"_blank",childrens:[
    //       {id:parseInt(Math.random()*10000000),name:"百度4-1", url:"#777777777", selected:false,childrenCount:0},
    //       {id:parseInt(Math.random()*10000000),name:"百度4-2", url:"", selected:false,childrenCount:0,target:"_blank"}
    //     ]}
    //   ]},
    //   {id:parseInt(Math.random()*10000000),name:"生活电器", url:"", selected:false,childrenCount:6,target:"_blank",childrens:[
    //     {id:parseInt(Math.random()*10000000),name:"取暖电器", url:"#777777777", selected:false,childrenCount:0},
    //     {id:parseInt(Math.random()*10000000),name:"加湿器", url:"", selected:false,childrenCount:0,target:"_blank"},
    //     {id:parseInt(Math.random()*10000000),name:"空气净化器", url:"", selected:false,childrenCount:0,target:"_blank"},
    //     {id:parseInt(Math.random()*10000000),name:"吸尘器", url:"", selected:false,childrenCount:0,target:"_blank"},
    //     {id:parseInt(Math.random()*10000000),name:"扫地机器人", url:"", selected:false,childrenCount:0,target:"_blank"},
    //     {id:parseInt(Math.random()*10000000),name:"除湿器", url:"", selected:false,childrenCount:0,target:"_blank"},
    //   ]}
    // ]},
    // {id:parseInt(Math.random()*10000000),name:"新闻", url:"#111111111",selected:false,childrenCount:2,childrens:[
    //   {id:parseInt(Math.random()*10000000),name:"百度2", url:"", selected:false,childrenCount:2,target:"_blank",
    //   childrens:[
    //     {id:parseInt(Math.random()*10000000),name:"百度3-1", url:"#2222222222", selected:false,childrenCount:2,childrens:[
    //       {id:parseInt(Math.random()*10000000),name:"百度4-1", url:"#777777777", selected:false,childrenCount:0},
    //       {id:parseInt(Math.random()*10000000),name:"百度4-2", url:"", selected:false,childrenCount:0,target:"_blank"}
    //     ]},
    //     {id:parseInt(Math.random()*10000000),name:"百度3-2", url:"", selected:false,childrenCount:2,target:"_blank",childrens:[
    //       {id:parseInt(Math.random()*10000000),name:"百度4-1", url:"#777777777", selected:false,childrenCount:0},
    //       {id:parseInt(Math.random()*10000000),name:"百度4-2", url:"", selected:false,childrenCount:0,target:"_blank"}
    //     ]}
    //   ]},
    //   {id:parseInt(Math.random()*10000000),name:"百度2", url:"", selected:false,childrenCount:2,target:"_blank", childrens:[
    //     {id:parseInt(Math.random()*10000000),name:"百度3-1", url:"#2222222222", selected:false,childrenCount:0},
    //     {id:parseInt(Math.random()*10000000),name:"百度3-2", url:"", selected:false,childrenCount:2,target:"_blank",childrens:[
    //       {id:parseInt(Math.random()*10000000),name:"百度4-1", url:"#777777777", selected:false,childrenCount:0},
    //       {id:parseInt(Math.random()*10000000),name:"百度4-2", url:"", selected:false,childrenCount:0,target:"_blank"}
    //     ]}
    ...
    //     ]}
    //   ]},
    //   {id:parseInt(Math.random()*10000000),name:"百度wwwwwwwwwwwww", url:"", selected:false,childrenCount:0,target:"_blank"}
    // ]},
    //  {id:parseInt(Math.random()*10000000),name:"淘宝", url:"",selected:false,childrenCount:0
    //   },
    //   {id:parseInt(Math.random()*10000000),name:"新闻", url:"#ssssd",selected:false,childrenCount:2,childrens:[
    //     {id:parseInt(Math.random()*10000000),name:"百度1", url:"", selected:false,childrenCount:2,target:"_blank",
    //     childrens:[
    //       {id:parseInt(Math.random()*10000000),name:"百度1-1", url:"", selected:false,childrenCount:0,target:"_blank"},
    //       {id:parseInt(Math.random()*10000000),name:"百度1-2", url:"", selected:false,childrenCount:0,target:"_blank"},
    //       {id:parseInt(Math.random()*10000000),name:"百度1-1", url:"", selected:false,childrenCount:0,target:"_blank"},
    //       {id:parseInt(Math.random()*10000000),name:"百度1-2", url:"", selected:false,childrenCount:0,target:"_blank"}
    //     ]},
    //     {id:parseInt(Math.random()*10000000),name:"百度2", url:"", selected:false,childrenCount:3,target:"_blank",
    //     childrens:[
    //       {id:parseInt(Math.random()*10000000),name:"百度1-1", url:"", selected:false,childrenCount:0,target:"_blank"},
    //       {id:parseInt(Math.random()*10000000),name:"百度1-2", url:"", selected:false,childrenCount:0,target:"_blank"},
    //       {id:parseInt(Math.random()*10000000),name:"百度1-1", url:"", selected:false,childrenCount:0,target:"_blank"},
    ...
    //     }
    //   ]},
    //   {id:parseInt(Math.random()*10000000),name:"百度", url:"", selected:false,childrenCount:0,target:"_blank"},
    //   {id:parseInt(Math.random()*10000000),name:"百度", url:"", selected:false,childrenCount:0,target:"_blank"},
    //   {id:parseInt(Math.random()*10000000),name:"百度", url:"", selected:false,childrenCount:0,target:"_blank"},
    //   {id:parseInt(Math.random()*10000000),name:"百度", url:"", selected:false,childrenCount:0,target:"_blank"},
    //   {id:parseInt(Math.random()*10000000),name:"百度", url:"", selected:false,childrenCount:0,target:"_blank"},
    //   {id:parseInt(Math.random()*10000000),name:"家用电器", url:"", selected:false,childrenCount:12,target:"_blank",childrens:[
    //     {id:parseInt(Math.random()*10000000),name:"冰箱", url:"", selected:false,childrenCount:5,target:"_blank",
    //     childrens:[
    //       {id:parseInt(Math.random()*10000000),name:"双门", url:"#123456", selected:false,childrenCount:6,childrens:[
    //         {id:parseInt(Math.random()*10000000),name:"直冷-1", url:"#789456", selected:false,childrenCount:0},
    //         {id:parseInt(Math.random()*10000000),name:"直冷-2", url:"#45258", selected:false,childrenCount:0},
    //         {id:parseInt(Math.random()*10000000),name:"直冷-3", url:"#78951", selected:false,childrenCount:0},
    //         {id:parseInt(Math.random()*10000000),name:"直冷-4", url:"#111555", selected:false,childrenCount:0},
    //         {id:parseInt(Math.random()*10000000),name:"直冷-5", url:"#888663", selected:false,childrenCount:0},
    ...
    //   {id:parseInt(Math.random()*10000000),name:"百度", url:"", selected:false,childrenCount:0,target:"_blank"},
    //   {id:parseInt(Math.random()*10000000),name:"百度", url:"", selected:false,childrenCount:0,target:"_blank"},
    //   {id:parseInt(Math.random()*10000000),name:"测试菜单名字过长的测试测试一下", url:"#ssssd",selected:false,childrenCount:2,childrens:[
    //     {id:parseInt(Math.random()*10000000),name:"百度1334", url:"", selected:false,childrenCount:2,target:"_blank",
    //     childrens:[
    //       {id:parseInt(Math.random()*10000000),name:"百度1-1", url:"#xxxxs", selected:false,childrenCount:0},
    //       {id:parseInt(Math.random()*10000000),name:"百度1-2", url:"", selected:false,childrenCount:0,target:"_blank"}
    //     ]},
    //     {id:parseInt(Math.random()*10000000),name:"百度2", url:"", selected:false,childrenCount:0,target:"_blank"}
    //   ]},
    //   {id:parseInt(Math.random()*10000000),name:"新闻228", url:"#ssssd",selected:false,childrenCount:2,childrens:[
    //     {id:parseInt(Math.random()*10000000),name:"百度156677", url:"", selected:false,childrenCount:2,target:"_blank",childrens:[
    //       {id:parseInt(Math.random()*10000000),name:"百度1-1", url:"#777777777", selected:false,childrenCount:0},
    //       {id:parseInt(Math.random()*10000000),name:"百度1-2", url:"", selected:false,childrenCount:0,target:"_blank"}
    //     ]},
    //     {id:parseInt(Math.random()*10000000),name:"百度2", url:"", selected:false,childrenCount:0,target:"_blank"}
    //   ]},
    //   {id:parseInt(Math.random()*10000000),name:"家用电器", url:"", selected:false,childrenCount:6,target:"_blank",childrens:[
    //     {id:parseInt(Math.random()*10000000),name:"冰箱", url:"", selected:false,childrenCount:2,target:"_blank",
    //     childrens:[
    //       {id:parseInt(Math.random()*10000000),name:"多门", url:"#2222222222", selected:false,childrenCount:2,childrens:[
    //         {id:parseInt(Math.random()*10000000),name:"百度4-1", url:"#777777777", selected:false,childrenCount:0},
