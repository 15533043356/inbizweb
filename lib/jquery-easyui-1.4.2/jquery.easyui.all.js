...
(function($){
	function buildToolbar(target){
		...
		var pager = $(target).addClass('pagination').html('<table cellspacing="0" cellpadding="0" border="0" class="pageOpt"><tr></tr></table>');
		...
		for (var index = 0; index < aa.length; index++) {
			...
			if (item == 'list'){
			    ...
			    //$('<td></td>').append(ps).appendTo(tr);//easyui原版
			    //pager.append(ps);
				$(target).find('.pageOpt').before(ps);//下拉框放在table前面 by cq.yanglulian 2017/6/28
			} else if (item == 'sep'){
				$('<td><div class="pagination-btn-separator"></div></td>').appendTo(tr);
			} else if (item == 'first'){
				bb.first = createButton('first');
			} else if (item == 'prev'){
			    //bb.prev = createButton('prev');//原版
			    ////V5轻量版'上一页'样式 by cq.yanglulian 2017/6/28
			    var pbtn = opts.nav['prev'];
			    var prevdiv = $('<div class="pagination-prev-div" >'+ $.fn.datagrid.defaults.prevText +'</div>').appendTo(tr);
			    prevdiv.wrap('<td></td>');
			    prevdiv.unbind('.pagination').bind('click.pagination', function () {
			        pbtn.handler.call(target);
			    });
			} else if (item == 'next'){
			    //bb.next = createButton('next');
			    //V5轻量版'下一页'样式 by cq.yanglulian 2017/6/28
			    var nbtn = opts.nav['next'];
                                                    //发现easyui.all.js与easyui.min.js内容不同步,easyui.min比较新,使用easyui.min.js
			    var nextdiv = $('<div class="pagination-next-div" >'+ $.fn.datagrid.defaults.nextText +'</div>').appendTo(tr);
			    nextdiv.wrap('<td></td>');
			    nextdiv.unbind('.pagination').bind('click.pagination', function () {
			        nbtn.handler.call(target);
			    });
			} else if (item == 'last'){
...
//		bb.next.add(bb.last).linkbutton({disabled: (opts.pageNumber == pageCount)});
	...
	$.fn.pagination.defaults = {
		...
		showRefresh: true,
		links: 10,
	    //layout: ['list','sep','first','prev','sep','manual','sep','next','last','sep','refresh'],//原生easyui分页信息排序
		layout: ['list', 'prev', 'sep', 'next',  'sep', 'sep', 'manual'],//V5设计图分页简化轻量版 去掉首末页及刷新按钮
	...
	function bindTreeEvents(target){
		...
		$(target).unbind().bind('mouseover', function(e){
			...
			if (tt.hasClass('tree-hit')){
				...
				opts.onClick.call(target, getNode(target, node[0]), e);
			}
			e.stopPropagation();
			...
			if (!node.length){return;}
			selectNode(target, node[0]);
			opts.onDblClick.call(target, getNode(target, node[0]),e);
			e.stopPropagation();
...
(function($){
    ...
    /**
	 * haibin.long 于 2017-07-31 增加，用于控制窗口居中后，向top方向回拉10%;
	 */
	function vcenterPullBack(target, tomove) {
	    var opts = $.data(target, 'window').options;
	    var pp = $(target).window('panel');
	    var height = pp._outerHeight();
	    if (opts.inline) {
	        var parent = pp.parent();
	        var halfHeight = (parent.height() - height) / 2;
	        halfHeight -= height * 0.1;
	        opts.top = Math.ceil(halfHeight + parent.scrollTop());
	    } else {
            // ( 屏幕H - 窗口h ) / 2 - 窗口h * 0.1
	        var halfHeight = ($(window)._outerHeight() - height) / 2;
	        halfHeight -= height * 0.1;
	        opts.top = Math.ceil( halfHeight + $(document).scrollTop());
	    }
	    if (tomove) { moveWindow(target); }
	}
	...
	function create(target){
        ...
        //20170731haibin.long add pull back 40%.
		if (opts.center) {
		    hcenter(target);
		    vcenterPullBack(target);
		} else {
...
//					$(this).appendTo('body');
	...
	$.fn.window.methods = {
		...
		center: function (jq) {
		    //201612mikahuang,the window is still centered after resizing the browser
		    $(window).off("resize.jeasyui_window").on("resize.jeasyui_window", function () {
		        if (jq && $.data(jq[0], 'window')) {
		            jq.window('center');
		        }
		        else {
		            $(window).off("resize.jeasyui_window");
		        }
		    });
			return jq.each(function(){
			    hcenter(this);
                //20170731haibin.long pull back 10%;
				vcenterPullBack(this); //vcenter(this);
				moveWindow(this);
			});
		}
	};
...
(function($){
	...
	function validate(target){
		...
		function doValidate(vtype, vparam) {
		    ...
		    //201610 mikahuang, add alwaysValidate rule
			if ((rule && value) || (rule && opts["alwaysValidate"])) {
...
(function($){
	...
	function validate(target){
		...
		//mikahuang add, have this api but not achieved
		var _onBeforeValidate = opts.onBeforeValidate;
		var _onValidate = opts.onValidate;
		input.validatebox($.extend({}, opts, {
			...
			onValidate: function(valid){
			    ...
			    //201610 mikahuang,valid value add to _onValidate function param
				_onValidate.call(this, valid, box.val());
			}
		}));
	}
...
//				}, 0);
	...
	function setBodySize(target){
		...
		var isfixheig = target.getAttribute('tit') == undefined ? false : true;
		if (innerWidth == 0 && isfixheig) {
		    innerWidth = state.options.minWidth;
		}
		if (innerHeight == 0 && isfixheig) {
		    innerHeight = state.options.minHeight;
		}
		var view = dc.view;
		var view1 = dc.view1;
		var view2 = dc.view2;
		...
		if (isfixheig) {
		    if (view2.height()==0) { view2.height(innerHeight); }
		}
...
//						col.cellSelector = '.' + col.cellClass;
	...
	function bindEvents(target) {
		...
		header.find('input[type=checkbox]').unbind('.datagrid').bind('click.datagrid', function(e){
			...
			if ($(this).is(':checked')) {
                //全选checkbox样式
			    $(this).parent("div").css("background", "transparent url('Content/images/checkbox-select.png') no-repeat center center");
				checkAll(target);
			} else {
			    //非全选checkbox样式
			    $(this).parent("div").css("background", "transparent url('Content/images/checkbox.png') no-repeat center center");
				uncheckAll(target);
			}
			e.stopPropagation();
		});
...
//		opts.onLoadSuccess.call(target, data);
	...
	function setSelectionState(target){
		...
		dc.header1.add(dc.header2).find('input[type=checkbox]').parent("div").css("background", "transparent url('Content/images/checkbox.png') no-repeat center center");
		if (opts.idField){
...
//		scrollTo(target, index);
	...
	function checkRow(target, index, notSelect){
		...
		tr.find('div.datagrid-cell-check input[type=checkbox]').parent("div").css("background", "transparent url('Content/images/checkbox-select.png') no-repeat center center");
		tr = opts.finder.getTr(target, '', 'checked', 2);
		if (tr.length == opts.finder.getRows(target).length){
			var dc = state.dc;
			dc.header1.add(dc.header2).find('input[type=checkbox]')._propAttr('checked', true);
			dc.header1.add(dc.header2).find('input[type=checkbox]').parent("div").css("background", "transparent url('Content/images/checkbox-select.png') no-repeat center center");
		}
		if (opts.idField){
	...
	function uncheckRow(target, index, notSelect){
		...
		tr.find('div.datagrid-cell-check input[type=checkbox]').parent("div").css("background", "transparent url('Content/images/checkbox.png') no-repeat center center");
		var dc = state.dc;
		var header = dc.header1.add(dc.header2);
		header.find('input[type=checkbox]')._propAttr('checked', false);
	    //头部checkbox样式为非全选
		header.find('input[type=checkbox]').parent("div").css("background", "transparent url('Content/images/checkbox.png') no-repeat center center");
		if (opts.idField){
	...
	function checkAll(target, notSelect){
	    ...
	    //头部checkbox样式为全选
		hck.add(bck).parent("div").css("background", "transparent url('Content/images/checkbox-select.png') no-repeat center center");
		if (opts.idField){
			for(var i=0; i<rows.length; i++){
...
//		var bck = opts.finder.getTr(target, '', 'allbody').find('div.datagrid-cell-check input[type=checkbox]');
	    ...
	    //头部checkbox样式为非全选
		hck.add(bck).parent("div").css("background", "transparent url('Content/images/checkbox.png') no-repeat center center");
		if (opts.idField){
			for(var i=0; i<rows.length; i++){
