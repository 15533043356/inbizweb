...
(function($){
	...
	function createButton(target) {
		var opts = $.data(target, 'linkbutton').options;
		var t = $(target).empty();
		//增加是否是主要按钮,默认为重要按钮，否则为次要按钮 by tanyo
		var isImportant= opts.important===undefined?true:opts.important;
		...
		//modify: 2017-09-04 haibin.long 增加扩展属性的绑定
		if(opts.extend){ //扩展自定义属性
			$.each(opts.extend, function(key, val){
				t.attr(key, val);
			});
		}
		if(opts.extendCls){ //扩展自定义样式类
		...
		if (opts.text){
			var textHtml='<span class="l-btn-text l-btn-istext"></span>';
			if(opts.iconCls)
				textHtml='<span class="l-btn-text"></span>'
			$(textHtml).html(opts.text).appendTo(inner);
		} else {
...
(function($){
	function buildToolbar(target){
		...
		for (var index = 0; index < aa.length; index++) {
			...
			if (item == 'list'){
			    ...
			    //$('<td></td>').append(ps).appendTo(tr);//easyuiԭ
				//pager.append(ps);
				$(target).find('.pageOpt').before(ps);

                //把选择框换成combobox后无法执行事件解决 by tanyo
				ps.combobox({
			        ptype: opts.type,
			        height: boxHeight,
			        panelHeight: 'auto',
			        ...
			        onSelect: function(selectObj) {
						var newValue = selectObj.text
						//这里不能用onchange，不然下面的赋值操作会触发，而在onChangePageSize、onSelectPage
						//中有更新分页的操作的话，会导致pageNumber进行重新赋值，然后又会触发onchange，从而造成死循环
			            opts.pageSize = parseInt(newValue);
						opts.onChangePageSize.call(target, opts.pageSize);
			            selectPage(target, opts.pageNumber);
			        }
			    });
				ps.next(".combo").addClass("pagination-page-combo");
			} else if (item == 'sep'){
				if(!tr.find("td:last").is(':empty')){
					$('<td style="width:20px"></td>').appendTo(tr);
				}
			} else if (item == 'first'){
				...
				if(!$.fn.datagrid.defaults.prevText)
					$.fn.datagrid.defaults.prevText="prev";
				var prevdiv = $('<a class="pagination-prev-div" >'+$.fn.datagrid.defaults.prevText+'</a>').appendTo(tr);
				prevdiv.linkbutton({important:false,height:boxHeight});
			    prevdiv.wrap('<td></td>');
				...
				if(!$.fn.datagrid.defaults.nextText)
					$.fn.datagrid.defaults.nextText="next";
				var nextdiv = $('<a class="pagination-next-div" >'+$.fn.datagrid.defaults.nextText+'</a>').appendTo(tr);
				nextdiv.linkbutton({important:false,height:boxHeight});
			    nextdiv.wrap('<td></td>');
				...
				$('<span></span>').html(opts.beforePageText).appendTo(tr).wrap('<td></td>');
				var num=$('<input class="pagination-num" type="text" value="1" size="2">').appendTo(tr);
				bb.num = num.wrap('<td></td>');
				//换成textbox后事件不执行解决,by tanyo
				num.textbox({width:44,
							ptype:opts.type,
					        height:boxHeight,
					        inputEvents: $.extend({},$.fn.textbox.defaults.inputEvents,{
							keyup: function(event){ 
								if (event.keyCode === 13) { //key enter code
							        var n = parseInt($(this).val());
							        var pageNumber = n > 0 ? n : 1;
				...
				bb.after = $('<span style="padding-right:6px; padding-left:10px;"></span>').appendTo(tr).wrap('<td></td>');
	...
	function selectPage(target, page){
		...
		if(!(opts.pageNumber===0||opts.pageSize===0)){
	...
	function refreshData(target, param){
		...
		if (ps.length){
			...
			ps.combobox("setValue",opts.pageSize+'');
		}
		...
		if (bb.num) {
		    ...
		    bb.num.textbox("setValue", opts.pageNumber);//由于原有的input标签换为了textbox //by hao.huagn@macrowinng.com
		}
...
//		bb.next.add(bb.last).linkbutton({disabled: (opts.pageNumber == pageCount)});
		...
		changeButtonVisible($(target), opts.total,opts.pageSize, opts.pageNumber);
     ...
     * @returns {} 
     */
	function changeButtonVisible(target, total, pageSize, pageNumber) {
	    var totalCount = Math.ceil(total / pageSize);
	    if ((totalCount === pageNumber && totalCount === 1)||total===0) {//只有一页或无数据
	        target.find(".pagination-next-div").parent().hide();
	        target.find(".pagination-next-div").parent().next().hide();
	        target.find(".pagination-prev-div").parent().hide();
	        target.find(".pagination-prev-div").parent().next().hide();
	    } else if (totalCount === pageNumber) {//最后一页
	        target.find(".pagination-next-div").parent().hide();
	        target.find(".pagination-next-div").parent().next().hide();
	        target.find(".pagination-prev-div").parent().show();
	        target.find(".pagination-prev-div").parent().next().show();
	    } else if (pageNumber === 1) { //第一页
	        target.find(".pagination-next-div").parent().show();
	        target.find(".pagination-next-div").parent().next().show();
	        target.find(".pagination-prev-div").parent().hide();
	        target.find(".pagination-prev-div").parent().next().hide();
	    } else {
	        target.find(".pagination-next-div").parent().show();
	...
	$.fn.pagination.defaults = {
		...
		loading: false,
		buttons: null,
		type:"big",//big small by tanyo
		showPageList: true,
		showRefresh: true,
		links: 10,
	    //layout: ['list','sep','first','prev','sep','manual','sep','next','last','sep','refresh'],//ԭ��easyui��ҳ��Ϣ����
		layout: ['list', 'prev', 'sep', 'next',  'sep', 'sep', 'manual'],//V5���ͼ��ҳ�������� ȥ����ĩҳ��ˢ�°�ť
...
//		param = param || {};
	...
	function expandNode(target, nodeEl, callback){
		...
		if (ul.length){
			...
			if(node.hasChildren){
                hit.removeClass("tree-leaf").addClass("tree-expanded");
            } else {
...
//			hideTip(target, e);
	...
	function showTip(target, e){
		var $target=$(target);
		var isTextBox=$target.hasClass("textbox-text");
		var isValidBox=$target.hasClass("validatebox-text");
		var isNavTip=$target.hasClass("navtip");
		var textBoxCls="";
		if(isValidBox || isTextBox){
            textBoxCls="textbox-tooltip";
		}
		if(isNavTip)
		{
			textBoxCls+=' nav-tooltip';
		}		
		var state = $.data(target, 'tooltip');
		var opts = state.options;
		var tip = state.tip;
		if (!tip){
			tip = $(
				'<div tabindex="-1" class="tooltip '+textBoxCls+'">' +
...
(function($){
	...
	function buildDialog(target){
		...
		if (opts.buttons){
			...
			if ($.isArray(opts.buttons)){
				...
				btnDiv=$('<div class="dialog-button"></div>');
				var buttons = btnDiv.appendTo(win);
				for(var i=0; i<opts.buttons.length; i++){
					var p = opts.buttons[i];
					var button = $('<a href="javascript:void(0)"></a>').appendTo(buttons);
					if (p.handler) {
						//modify: 2017-08-30 haibin.long 增加dialog中按钮作用域指向指定上下文的能力
						if(opts.cxt){
							button[0].onclick = p.handler.bind(opts.cxt);
						}else{
				...
				btnDiv=$(opts.buttons);
				btnDiv.addClass('dialog-button').appendTo(win);
				btnDiv.show();
			}
			if(opts.btnPosition=='center'){
	...
	$.fn.dialog = function(options, param) {
		...
		return this.each(function(){
			...
			//扩展setscroll方法方便重写 bytanyo
			var method=$.fn.dialog.methods["setScroll"];
			if(method)
				method(this,opts.scrollble);
		});
	};
...
(function($){
	...
	function createDialog(options){
		...
		if (options.buttons && options.buttons.length){
			...
			var i=0;
			//调整messagebutton left 24px by tanyo
			$.map(options.buttons, function(btn){
				var left="24px";
				if(i==0)
				   left="0px";
				$('<a href="javascript:void(0)" style="margin-left:'+left+'"></a>').appendTo(tb).linkbutton(btn);
				i++;
			});
		}
		win.window('window').addClass('messager-window');
		win.window('resize');
		var msgButton=win.children('div.messager-button');
		//调整没有icon的alert by tanyo
		if(options.btnCls){
			msgButton.addClass(options.btnCls);
		}
		msgButton.children('a:first').focus();
		return win;
	}
	...
	$.messager = {
		...
		alert: function(title, msg, icon, fn) {
			...
			var btnCls="";
			if(cls==""){
				//调整没有icon的alert bytanyo
				cls="messager-icon-null";
				btnCls="messager-icon-null-button"
			}
			opts = $.extend({}, $.messager.defaults, {
				content: '<div class="' + cls + '"></div>'
						+ '<div class="messager-text">' + opts.msg + '</div>'
						+ '<div style="clear:both;"/>',
				//控制没有icon的alert bytanyo
				btnCls:btnCls,
				buttons: [{
		...
		prompt: function(title, msg, fn) {
			//调整prompt 样式 by tanyo
			var opts = typeof title == 'object' ? title : {title:title, msg:msg, fn:fn};
			opts = $.extend({}, $.messager.defaults, {
				content: '<div class="messager-icon messager-prompt-icon messager-question"></div>'
						+ '<div class="messager-prompt-text">' + opts.msg + '</div>'
						+ '<br/>'
						+ '<div style="clear:both;"/>'
						+ '<div><input class="messager-input messager-prompt-input" type="text"/></div>',
				btnCls:"messager-prompt-btton",
				buttons: [{
					text: $.messager.defaults.ok,
					onClick: function(){
						win.window('close');
	...
	function createPanel(container, pp, options){
		...
		pp.panel($.extend({}, {
			...
			onBeforeExpand: function(){
				...
				//设置滚动条 by tanyo 
				var method=$.fn.accordion.methods["setScroll"];
				if(method)
				  method(this,opts.scrollble);
			},
			onExpand: function(){
...
(function($){
	...
	function setScrollers(container) {
		...
		if (tabsWidth > cWidth) {
			...
			//判断right by tanyo，增加tools到tab后面
			} else if(opts.toolPosition == 'right') {
				tool.css({
					left: '',
	...
	function addTools(container){
		...
		if (opts.tools) {
			if (typeof opts.tools == 'string'){
				//如果是head 增加tools可以紧跟tab后面的tools by tanyo
				var isHead=header.hasClass("tabs-header");
				if(isHead&&opts.toolPosition=="last"){
					var lis = header.find(".tabs li");
					var len=lis.length;
					var lWidth = (len*$(lis[0]).width());
					$(opts.tools).css({left:lWidth});
					$(opts.tools).addClass("tabls-tool-set");
	...
	function wrapTabs(container) {
		...
		//过滤掉隐藏的tab
		if (state.options.titles && state.options.titles.length>0) {
            $.each(state.options.titles, function (i, item) {
                if (item.hide) {
                    cc.find("div[title='" + item.key + "']").remove();
                }
            });
        }
		cc.children('div').each(function(){
		...
		$(container).children('div.tabs-header').unbind().bind('click', function(e){
			if ($(e.target).hasClass('tabs-scroller-left')){
				...
				if (a.length){
					...
					//删除tab页后tools变动bytanyo
					var tools=$(container).find(".tabls-tool-set");
					if(tools.length){
						var tabs=$(container).find(".tabs li");
						var lWidth= $(tabs[0]).width();
						var cWidth= tabs.length*lWidth;
						tools.css({left:cWidth+15});
					}
				} else if (li.length){
//					selectTab(container, getLiIndex(li));
	...
	function createTab(container, options, pp) {
		...
		if(pp){
			title= pp.attr("title");
			item = getItem(state.options.titles, title);
		}
		...
		if (options.index >= tabs.length){
			tab.appendTo(ul);
			if(item){
				panels.find("div[title='" + title + "']").attr("title", item.value);
			}
			pp.appendTo(panels);
			tabs.push(pp);
		} else {
...
//		}
	...
	$.fn.tabs.defaults = {
		...
		pill: false,
		tools: null,
		toolPosition: 'last',	// left,right,last(新增：跟随），modify: 2017-09-13 haibin.long 原默认为right
...
(function($){
	...
	function buildTextBox(target){
		...
		*/
		/* modify by cq.taiqi 增加样式扩展*/
		if (opts.multiline){
			if(opts.cls){
				$('<textarea class="textbox-text '+ opts.cls+' " autocomplete="off"></textarea>').prependTo(tb);
			}else{
				$('<textarea class="textbox-text" autocomplete="off"></textarea>').prependTo(tb);
			}
		} else {
			if(opts.cls){
				$('<input type="'+opts.type+'" class="textbox-text '+opts.cls+' " autocomplete="off">').prependTo(tb);
			}
			else{
				/* modify by cq.wuguangyun 增加样式扩展,个人设置--修改密码自动填充密码*/
                if (opts.validType == "__PasswordUpdate_ConfirmPasswordValidate" || opts.validType == "__PasswordUpdate_NewPasswordValidate" || opts.validType == "__PasswordUpdate_OldPasswordValidate") {
                    $('<input type="' + opts.type + '" class="textbox-text" autocomplete="new-password">').prependTo(tb);
				} else if (target.id =="shareName") {
                    $('<input type="' + opts.type + '" maxlength="65" class="textbox-text" autocomplete="off">').prependTo(tb);
                }
				else {
                    $('<input type="' + opts.type + '" class="textbox-text" autocomplete="off">').prependTo(tb);
                }
	...
	function validate(target){
		...
		input.validatebox($.extend({}, opts, {
            deltaX: 14, //$(target).textbox('getTipX'), modify: 2018-07-14 haibin.long 由于v5.easyui中对textbox设置为content-box了，影响此处理改为0行
			onBeforeValidate: function(){
				var box = $(this);
				if (!box.is(':focus')){
	...
	function bindEvents(target){
		...
		if (!opts.disabled && !opts.readonly){
			input.bind('blur.textbox', function(e){
				...
				var addon=textbox.find(".textbox-addon a");
		...
		addon.unbind().bind('click', {target:target}, function(e){
			var a=$(e.target);
			var icon = a.closest('a.textbox-icon:not(.textbox-icon-disabled)');
			if (icon.length){
		...
		addon.find('.textbox-icon').each(function(index){
			...
			//如果是numberspinner禁用状态隐藏上下箭头 by tanyo
			var nbox= $(this).closest(".numberbox");
			if(nbox.length>0 && nbox.hasClass("textbox-disabled")){
				var isNumspinner=nbox.prev().hasClass("easyui-numberspinner");
				if(isNumspinner){
					$(this).closest(".textbox-addon").hide();	
				}
			}
			var icon = $(this);
	...
	function setReadonly(target, mode){
		...
		if (opts.readonly || !opts.editable){
			...
			input.attr('unselectable','on');//兼容ie
		}
	}
...
//	}
	...
	$.fn.textbox.defaults = $.extend({}, $.fn.validatebox.defaults, {
		...
		prompt:'',
		value:'',
		ptype:"big",//big small主题大小 by tanyo
		type:'text',
		multiline:false,
		editable:true,
		disabled:false,
		...
		buttonIcon:null,
		buttonAlign:'right',
		cls:null,//extends: Add a CSS class to the textbox. Available since version 1.5.1. 
		inputEvents:{
			blur: function(e){
				var t = $(e.data.target);
...
//				}, 0);
	...
	function setBodySize(target){
		...
		// set view width
		//为实现分页 边框加在dataview上，所以需要减去2 边框的宽度 by tanyo
		...
		var hght=(isNaN(parseInt(opts.height)) ? '' : (innerHeight-fixedHeight));
		if(hght>0){
			...
			if(state.options.pagination){
				//如果开启了分页 减去分页空间离内容区的12px by tanyo 
				view.height(view2.height()-12);
				view2.children('div.datagrid-body').height(hght-12);
			}else{
				view.height(view2.height());
				view2.children('div.datagrid-body').height(hght);
			}
		}
	}
			...
			for(var i=0; i<columns.length; i++) {
				...
				for(var j=0; j<cols.length; j++){
					...
					if (col.checkbox){
						...
						//修改checkbox样式 by tanyo
						var id="s_"+$.parser.genUUID()+"_99999";
						$('<div class="datagrid-header-check"></div>')
						.html('<input type="checkbox" id="'+id+'" class="regular-checkbox"/><label for="'+id+'"></label>')
						.appendTo(td);
					} else if (col.field){
						td.attr('field', col.field);
...
//			dc.view1.children('div.datagrid-body').scrollTop($(this).scrollTop());
			...
			refreshCustomScroll(); //modify: 2017-12-13 haibin.long 数据load完成后，再刷新滚动条
		});

		function refreshCustomScroll(){
			//设置滚动条 combox by tanyo
			var method=$.fn.datagrid.methods["setScroll"];
			if(method){
				method(dc.view2.children('div.datagrid-body'),state.options.scrollble);
			}
		}
	}
	...
	function clickEventHandler(e){
		...
		var tt = $(e.target);
		if (!tr){return;}
		var extend= tt.attr("extend")=="true"?true:false;
	
		 //modify: 2018-07-13 haibin.long 由于label关联checkbox会触发两次click事件，以下判断有问题
		/*if(extend){
			if(!islabel)
			 islabel=tt.is("label");
			tt=tt.prev("input");
		}*/
        if (extend && tt.is("label")) {
            return;
        }
		var target = getTableTarget(tr);
		var opts = $.data(target, 'datagrid').options;
		...
		if (tt.parent().hasClass('datagrid-cell-check')) {	// click the checkbox
			...
			if (td.length){
				...
				//modify: 2018-01-15 haibin.long 增加事件对象的传递
				opts.onClickCell.call(target, index, field, row[field], e);
			}
			...
			//modify: 2018-01-15 haibin.long 增加事件对象的传递
			opts.onClickRow.apply(target, getArguments(target, [index, row, e]));
		}
	}
	...
	function createEditor(target, index){
		...
		tr.children('td').each(function(){
			...
			if (col && col.editor){
				...
				if(editoptions)
				{
					editoptions.ptype= col.editor.ptype || "small";
				}else{
					editoptions={};
					editoptions.ptype=col.editor.ptype || "small";
				}
...
//			endEdit(target, i, true);
			...
			return {
				...
				resize: function(target, width){
					if (isA(target, name)){
						...
						$(target)._outerWidth(width)._outerHeight(26); //note: 2017-09-19 haibin.long 这里把高度固定了
					}
				}
			}
		}
	...
	var editors = $.extend({}, 
		...
		validatebox: {
			...
			resize: function(target, width){
				$(target)._outerWidth(width)._outerHeight(26);
			}
		}
	});
		...
		renderRow: function(target, fields, frozen, rowIndex, rowData){
			...
			for(var i=0; i<fields.length; i++){
				...
				if (col){
					...
					cc.push('>');
					if (col.checkbox) {
						var id= "s_"+this.uuid+"_"+rowIndex;
						cc.push('<input type="checkbox" extend="true" class="regular-checkbox" ' + (rowData.checked ? 'checked="checked"' : ''));
						cc.push(' name="' + field + '" value="' + (value!=undefined ? value : '') + '" id="'+id+'" />');
						cc.push('<label for="'+id+'" extend="true"></label>');
...
//				var tr = '<tr id="' + rowId + '" class="datagrid-row" datagrid-row-index="' + index + '">' + this.renderRow.call(this, target, fields, frozen, index, row) + '</tr>';
		...
		setEmptyMsg:function(target){
			...
			if(opts.emptyMsg){
				dg.dc.view.children(".datagrid-empty").remove();
				if(isEmpty){
					var h=dg.dc.header2.parent().outerHeight();
					var d=$("<div class=\"datagrid-empty\"></div>").appendTo(dg.dc.view);
...
(function($){
	...
	function buildGrid(target){
		...
		$(target).datagrid($.extend({}, opts, {
			...
			onClickCell:function(index,field, rowData,e){
				opts.onClickCell.call(target, field, find(target, index),e);
			},
...
(function($){
	...
	function inputEventHandler(e){
		...
		case 40:	// down
			...
			//防止down操作后当前Input已经失去焦点 by tanyo
			$(e.target).focus();
			break;
		case 37:	// left
	...
	function showPanel(target){
		...
		if (palOpts.closed){
			...
			if(!panel.pHeight){
				panel.pHeight= panel.outerHeight() + 10;
			}
			//解决IE下和默认高 不留太多的空白。 by tanyo
			var height=($.browser.msie||$.browser.edge || opts.panelHeight==null)?( (opts.panelHeight == 'auto'|| opts.panelHeight==null )&&panel.outerHeight() ? panel.pHeight : opts.panelHeight):opts.panelHeight;
			if(!height || height <= 0 ){
				//如果当前无高默认为2
				height=200;
			} else if(height > 300){
			...
			panel.panel('resize', {
				...
				//by cq.taiqi 2018.01.09 处理IE系浏览器combobox出现滚动条问题
				height:height
			});
		...
		//设置滚动条 combox by tanyo
		var method=$.fn.combo.methods["setScroll"];
		if(method){
			method(panel,state.options.scrollble);
		}
		(function(){
			if (panel.is(':visible')){
				panel.panel('move', {
	...
	function hidePanel(target){
		//add by cq.taiqi 2017年12月13日 16:10:33 datagrid表格里面，添加combobox时，选择某项后，会出现取不到panel bug
		if(!$.data(target,'combo')) return;
		var panel = $.data(target, 'combo').panel;
		panel.panel('close');
	}
...
//	}
	...
	$.fn.combo = function(options, param){
		if (typeof options == 'string'){
			...
			if (method){
				...
				//这个简化用问题，会影响combo调用validatebox相关访问，不知道为什么这样写，牛X啊
				return this.textbox(options, param);
			}
		}
	...
	$.fn.combo.methods = {
		...
		//设置滚动条扩展方法
		setScroll: function(jq,scrollble){
		},
		cloneFrom: function(jq, from){
			return jq.each(function(){
...
//		param = param || {};
	...
	function create(target){
		...
		var panel=$(target).combo('panel');
		if(opts.ptype=="small")//支持小皮肤 by tanyo
			panel.addClass("combobox-small");
...
(function($){
	...
	function create(target){
		...
		//监控面板
		panel.off("mouseenter").off("mouseleave").on("mouseenter", function () {
			opts.flaghasEnter = true;
		}).on("mouseleave", function () {
			opts.flaghasEnter=true;
		    if (opts.flaghasEnter ) {
		        $(target).combo('hidePanel')
		    }
			opts.flaghasEnter = false;
		});
	}
...
(function($){
	...
	function create(target){
		...
		$(target).addClass('combogrid-f').combo($.extend({}, opts, {
			scrollble: false, //modify:2018-07-16 haibin.long 在combogrid情况下，combo的滚动条应该隐藏 
			onShowPanel: function(){
				...
				if(method){
					var timeIndex = window.setTimeout(function(){
						var dc =$.data(grid[0],'datagrid').dc;
						method(dc.view2.children('div.datagrid-body'), opts.scrollble);
					},0);
				}
			}
		}));
...
(function($){
   ...
   $.browser = (function () {
        ...
        var browser = {
            edge:/Edge/i.test(ua),//MS Edge浏览器
            trident: /Trident/i.test(ua), //IE内核
            msie: /Trident/i.test(ua), //IE内核
            msie11: /Trident/i.test(ua) && /rv:11\.0/i.test(ua),
            msie10: /MSIE 10\.0/i.test(ua),
            msie9: /MSIE 9\.0/i.test(ua),
            msie8: /MSIE 8\.0/i.test(ua),
            msie7: /MSIE 7\.0/i.test(ua),
            msie6: /MSIE 6\.0/i.test(ua),
            presto: /Presto/i.test(ua),  //opera内核
            webkit: /AppleWebKit/i.test(ua),  //苹果、谷歌内核
            gecko: /Gecko/i.test(ua) && /KHTML/i.test(ua), //火狐内核
            ...
            webapp: /Safari/i.test(ua), //web应该程序，没有头部与底部
            language: (navigator.browserLanguage || navigator.language).toLowerCase()
        };
        browser.version = "0";
        if (browser.msie) {
            browser.msie11 && (browser.version = "11");
            browser.msie10 && (browser.version = "10");
            browser.msie9 && (browser.version = "9");
            browser.msie8 && (browser.version = "8");
            browser.msie7 && (browser.version = "7");
            browser.msie6 && (browser.version = "6");
        }
        return browser;
    })();
//扩展表格的滚动条
        ...
        setScroll: function(jq,scrollble){
            if(scrollble){
                ...
                if($.fn.perfectScrollbar){
                 $(jq).perfectScrollbar().perfectScrollbar("update");
               }
            }
        }
    });
