...
//			}
	...
	function collapsePanel(container, region, animateSpeed){
		if (animateSpeed == undefined){animateSpeed = 'fast';}
		var panels = $.data(container, 'layout').panels;
		...
		p.panel('panel').animate(copts.collapse, 0, function(){
		...
		function createExpandPanel(dir){
            ...
            //add by whimco 增加已有竖条的处理
			var p;
			if( $(".layout-expand-"+dir,container).length > 0)
			{
				p = $(".layout-expand-"+dir,container);
			}else{
				p = $('<div></div>').appendTo(container);
			}
	...
	function expandPanel(container, region){
		...
		if (panels[expandP]){
            ...
            //add by whimco 添加0参数，去掉延时效果
			p.panel('panel').animate(eopts.expand,0, function(){
				setSize(container);
			});
		}
